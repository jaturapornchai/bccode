...
describe("login screen security", () => {
  it("uses the public Demo login instead of a loopback-gated Dev Login", () => {
    expect(source).toContain('fetch("/api/auth/demo-login"');
    expect(source).toContain("ทดลองใช้ระบบ (Demo)");
    expect(source).not.toContain("/api/auth/dev-login");
    expect(source).not.toMatch(/isLoopbackHostname|BCAI_DEV_LOGIN_SECRET|X-BC-Dev-Login-Secret/);
  });
