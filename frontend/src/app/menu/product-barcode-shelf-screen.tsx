...
const text = {
  th: {
    ...
    subtitle:
      "เลือกสินค้าจากฐานข้อมูลจริง กำหนดจำนวนป้าย แล้วพิมพ์ตามหน้าจอ Flutter เดิม",
  ...
  en: {
    ...
    subtitle:
      "Select real products, set label copies, and print using the legacy Flutter workflow.",
...
export function ProductBarcodeShelfScreen({
...
}: ProductBarcodeShelfScreenProps) {
    ...
    async (
      ...
      try {
        ...
        const preferredProduct = preferred
          ...
          : undefined;
        if (preferredProduct) {
          const key = productIdentity(preferredProduct);
          setSelected({ [key]: { product: preferredProduct, copies: 1 } });
        }
      } catch (error) {
        setNotice({
          type: "error",
  ...
  function toggleProduct(product: ProductForLabel) {
    setSelected((current) => {
      ...
      else next[key] = { product, copies: 1 };
      return next;
    });
  }

  function updateCopies(key: string, delta: number) {
    setSelected((current) => {
  ...
  function addVisible() {
    setSelected((current) => {
      ...
      for (const product of products) {
        const key = productIdentity(product);
        if (!next[key]) next[key] = { product, copies: 1 };
      }
      return next;
    });
  }
        ...
        <div className="flex min-w-0 flex-wrap items-start justify-between gap-2">
          ...
          <div className="flex flex-wrap gap-2">
            <Badge variant="outline">
              {dictionary.total}: {products.length.toLocaleString()}
            </Badge>
            <Badge variant="success">
              {dictionary.selected}: {selectedList.length.toLocaleString()} /{" "}
          ...
          {selectedList.flatMap(({ product, copies }) =>
            Array.from({ length: copies }, (_, index) => (
              <div
                ...
                key={`${productIdentity(product)}-${index}`}
              >
  ...
  if (embedded)
    return (
      <section className="grid w-full min-w-0 gap-3 product-label-screen">
        {content}
      </section>
    );
  return (
    <main className="min-h-dvh w-full bg-background p-3 text-foreground product-label-screen">
      {content}
    </main>
  );
}
...
function productIdentity(product: ProductForLabel): string {
...
function normalizeProducts(value: unknown): ProductForLabel[] {
  ...
  return value
    ...
    .filter((item): item is ProductForLabel =>
