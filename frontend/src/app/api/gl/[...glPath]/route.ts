...
export async function GET(request: Request, context: Context) {
  ...
  try {
    ...
    const allowed = ["q", "page", "limit", "from", "to", "fiscalyear", "accountcode", "branchcode", "departmentcode", "projectcode", "bookcode", "status", "kind", "snapshot"];
...
export async function POST(request: Request, context: Context) {
  ...
  if (![...GL_RESOURCES, "processes"].includes(body.resource as never) || !["create", "update", "delete", "post", "reverse", "lock", "unlock", "close", "year-end", "recalculate", "reprocess"].includes(String(body.action))) return bad("ประเภทการทำรายการไม่ถูกต้อง");
  ...
  for (const key of ["resource", "id", "action", "requestid", "version", "reason", "date", "docno", "targetyear", "account", "fiscalyear", "master", "journal", "statementtemplate"]) {
...
function hasNumericAmount(value: unknown): boolean {
  ...
  return Object.entries(value).some(([key, child]) => ["amount", "debit", "credit"].includes(key) ? typeof child !== "string" : hasNumericAmount(child));
}
