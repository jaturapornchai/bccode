...
export async function POST(request: Request, context: Context) {
  ...
  for (const key of ["resource", "id", "action", "requestid", "version", "reason", "date", "docno", "targetyear", "account", "fiscalyear", "master", "journal", "budget", "statementtemplate", "review"]) {
...
function hasNumericAmount(value: unknown): boolean {
  ...
  // Budget lines carry money as periods[] (12 monthly amounts) and total (annual amount to spread).
  return Object.entries(value).some(([key, child]) => ["amount", "debit", "credit", "balance_after"].includes(key) ? typeof child !== "string"
    : key === "periods" ? !Array.isArray(child) || child.some((month) => typeof month !== "string")
    : key === "lines" && Array.isArray(child) && child.some((line) => isRecord(line) && "total" in line && typeof line.total !== "string") ? true
    : hasNumericAmount(child));
}
