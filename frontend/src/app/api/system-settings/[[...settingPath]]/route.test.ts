...
describe("system settings API route security", () => {
  ...
  it("rejects cross-tenant requests before proxying", async () => {
    ...
    const response = await GET(
      new Request("http://localhost/api/system-settings/permission_definition?shopid=SHOP002", {
        headers: { Authorization: `Bearer ${signJwt({ username: "user@example.com", shopid: "SHOP001" })}` },
      }),
  ...
  it("uses a generated password when creating a username login user", async () => {
    ...
    const response = await POST(
      new Request("http://localhost/api/system-settings/user?shopid=SHOP001", {
        method: "POST",
        headers: {
  ...
  it("proxies product unit deletes to the legacy unit guid endpoint", async () => {
    ...
    const response = await DELETE(
      new Request("http://localhost/api/system-settings/productunit/UNIT-GUID?shopid=SHOP001", {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
