...
export function ProductGroupTreeView({
  ...
  const getPointerDropCandidate = (clientX: number, clientY: number, activeDrag: DragState) => {
    ...
    const childDropZone = element.closest<HTMLElement>("[data-group-child-drop-guid]");
    const childTargetGuid = childDropZone?.dataset.groupChildDropGuid || "";
    if (childTargetGuid) {
    ...
    const rootZone = element.closest("[data-group-root-drop='true']");
    ...
    const findNearestRowCandidate = () => {
      ...
      const rowElements = Array.from(root.querySelectorAll<HTMLElement>("[data-group-row-guid]"));
    ...
    const rowElement = element.closest<HTMLElement>("[data-group-row-guid]");
    const targetGuid = rowElement?.dataset.groupRowGuid || "";
                ...
                <div className="flex min-w-0 flex-1 flex-wrap items-center gap-x-2 gap-y-1 pr-2">
                  ...
                  {canAcceptChildDrop ? (
                    <span
                      ...
                      data-group-child-drop-guid={node.detail.guidfixed}
