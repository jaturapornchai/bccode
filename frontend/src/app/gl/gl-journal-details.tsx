...
type Input = {key:string;label:string;type?:"date"|"amount"|"check"|"account"|"line";options?:[string,string][];lookup?:GLSupportKind;side?:number};
...
function newRow(key:Key,date:string,branch:string):Row {
  ...
  const defaults:Record<Key,Row>={partners:{partner_code:"",name_th:"",is_customer:true,is_supplier:false,is_active:true},bank_accounts:{bank_account_code:"",bank_name:"",account_name:"",account_number:"",gl_account_code:"",currency_code:"THB",is_active:true},documents:{id,ledger:"ar",partner_code:"",document_no:"",document_date:date,due_date:date,branch_code:branch,document_kind:1,balance_side:1,amount:"0",currency_code:"THB",control_account_code:""},allocations:{id,ledger:"ar",document_id:"",line_number:1,amount:"0"},settlements:{id,ledger:"ar",partner_code:"",debt_document_id:"",payment_document_id:"",settlement_date:date,amount:"0"},bank_lines:{line_number:1,bank_account_code:"",direction:1},statement_lines:{id,bank_account_code:"",source_key:"",transaction_date:date,direction:1,amount:"0"},matches:{id,statement_line_id:"",line_number:1,amount:"0"},withdrawals:{kind:"match",id:"",reason:""}};
  return defaults[key];
}
...
export function GLJournalDetailsPanel({value={},original={},onChange,lines,accounts,date,branch,journalId,editable,posted,onBusyChange,scale=2}:{value?:GLJournalDetails;original?:GLJournalDetails;onChange:(value:GLJournalDetails)=>void;lines:GLLine[];accounts:GLAccount[];date:string;branch:string;journalId?:string;editable:boolean;posted:boolean;onBusyChange?:(busy:boolean)=>void;scale?:number}) {
  ...
  function renderInput(key:Key,index:number,row:Row,field:Input,canEdit:boolean) {
    ...
    if(field.type==="amount")return <AmountInput value={String(current)} onChange={v=>update(key,index,{[field.key]:v})} scale={scale} allowNegative={false} disabled={!canEdit} ariaLabel={label}/>;
  ...
  return <section aria-label={tr("gl_details_ui_18","รายละเอียดตรวจสอบลูกหนี้ เจ้าหนี้ และธนาคาร")} className="grid gap-3 rounded-xl border border-border bg-card p-3 text-[0.95rem] leading-relaxed shadow-sm">
    ...
    {sections.map(([key,title])=>{const rows=detailRows(value,key),canAdd=editable&&(!posted||postedKeys.includes(key));return <details key={key} className="rounded-lg border border-border p-3" open={rows.length>0||undefined}>
      ...
      <div className="grid gap-3">{rows.map((row,index)=>{const saved=detailRows(original,key).some(old=>row.id?old.id===row.id:old.line_number===row.line_number&&old.bank_account_code===row.bank_account_code);const canEdit=canAdd&&(!posted||!saved);return <div key={String(row.id||index)} className="grid gap-2 rounded-lg border border-border bg-muted/20 p-3">
