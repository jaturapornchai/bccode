...
import { type GLJournalDetails, type GLSupportKind, type GLSupportRow, defaultVatTaxType, parseStatementCsv, supportLabel, vatPeriodChoices, vatPeriodPatch, vatPeriodValue } from "@/lib/gl-journal-details";
...
type Input = {key:string;label:string;type?:"date"|"amount"|"check"|"account"|"line"|"period";options?:[string,string][];lookup?:GLSupportKind;side?:number;placeholder?:string;maxLength?:number;numeric?:boolean;show?:(row:Row)=>boolean};
...
const fields:Record<Key,Input[]>={
  ...
  vats:[
    ...
    {key:"partner_tax_id",label:tr("gl_detail_vats_partner_tax_id","เลขประจำตัวผู้เสียภาษี (13 หลัก)"),maxLength:13,numeric:true},
    {key:"partner_branch_no",label:tr("gl_detail_vats_partner_branch_no","สาขา (5 หลัก, 00000 = สำนักงานใหญ่)"),maxLength:5,numeric:true},
    ...
    {key:"vat_rate",label:tr("gl_detail_vats_vat_rate","อัตราภาษี (%)"),type:"amount"},
...
function newRow(key:Key,date:string,branch:string,bookcode?:string):Row {
  const id=crypto.randomUUID();
  const taxType=defaultVatTaxType(bookcode);
...
export function GLJournalDetailsPanel({value={},original={},onChange,lines,accounts,date,branch,bookcode,journalId,editable,posted,onBusyChange,scale=2}:{value?:GLJournalDetails;original?:GLJournalDetails;onChange:(value:GLJournalDetails)=>void;lines:GLLine[];accounts:GLAccount[];date:string;branch:string;bookcode?:string;journalId?:string;editable:boolean;posted:boolean;onBusyChange?:(busy:boolean)=>void;scale?:number}) {
  const tr=useGLText();
  ...
  return <section aria-label={tr("gl_details_ui_18","รายละเอียดตรวจสอบลูกหนี้ เจ้าหนี้ และธนาคาร")} className="grid gap-3 rounded-xl border border-border bg-card p-3 text-[0.95rem] leading-relaxed shadow-sm">
    ...
    {sections.map(([key,title])=>{const rows=detailRows(value,key),canAdd=editable&&(!posted||postedKeys.includes(key));return <details key={key} className="rounded-lg border border-border p-3" open={rows.length>0||undefined}>
      ...
      <div className="grid gap-3">{rows.map((row,index)=>{const saved=detailRows(original,key).some(old=>row.id?old.id===row.id:old.line_number===row.line_number&&old.bank_account_code===row.bank_account_code);const canEdit=canAdd&&(!posted||!saved||taxKeys.includes(key));return <div key={String(row.id||index)} className="grid gap-2 rounded-lg border border-border bg-muted/20 p-3">
        ...
        <div className="grid gap-3 sm:grid-cols-2">{fields[key].filter(field=>field.show?.(row)??true).map(field=><Field key={field.key} label={tr(`gl_detail_${key}_${field.key}`,field.label)}>{renderInput(key,index,row,field,canEdit)}</Field>)}</div>
      </div>;})}</div>
      ...
      {canAdd&&<Button type="button" variant="outline" className="mt-3 min-h-11" onClick={()=>onChange({...value,[key]:[...rows,newRow(key,date,branch,bookcode)]})}>{tr("gl_details_add","เพิ่ม")}{tr(`gl_details_section_${key}`,title)}</Button>}
