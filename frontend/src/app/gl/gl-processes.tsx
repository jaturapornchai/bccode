...
export function GLProcesses({ route, action }: { route: string; action: "close" | "year-end" | "recalculate" | "reprocess" }) {
  ...
  return <section className={`${panel} grid gap-3`}><Notice text={tr(...config.text)} /><Notice error text={error || refs.error} /><Notice text={message} />
    ...
    {year && <p className="text-[0.95rem]">{tr("gl_range_to_period_info", "{0} ถึง {1} · {2} · {3}").replace("{0}", String(year.startdate)).replace("{1}", String(year.enddate)).replace("{2}", String(year.currency)).replace("{3}", String(year.closed ? tr("gl_year_closed", "ปิดปีแล้ว") : tr("gl_active_fiscal_year", "ปีบัญชีเปิดใช้งาน")))}</p>}
