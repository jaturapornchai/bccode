...
describe("backend URL helpers", () => {
  ...
  it("accepts any host (allowlist removed for on-prem flexibility)", () => {
    expect(validateBackendUrl("http://example.com/goapi").mainApiUrl).toBe("http://example.com:8888");
    expect(validateBackendUrl("http://192.168.1.20:8888/goapi").mainApiUrl).toBe("http://192.168.1.20:8888");
