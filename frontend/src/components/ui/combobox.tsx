 ...
 * - React Portal integration (`createPortal` to `document.body`) to escape any `overflow-y-auto`,
 *   stacking contexts (`opacity-95`, `transform`), or parent card clipping
 */
function ComboboxInner<T extends string | number = string | number>(
  ...
  // Compute fixed dropdown coordinates to float directly above/below trigger
  ...
  // Update position on open, window resize, or scroll (including scroll in overflow containers)
  React.useEffect(() => {
    if (!isOpen) return;
    updatePosition();
  ...
  // Close on outside click (inspecting both trigger and portal dropdown)
  React.useEffect(() => {
    if (!isOpen) return;
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className={cn(
          "group flex min-h-[2.6em] w-full items-center justify-between gap-2 rounded-xl border border-input bg-background px-3 py-1.5 text-[0.95rem] leading-normal text-foreground select-none text-left cursor-pointer",
      ...
      {/* Floating Panel portaled to document.body to completely escape overflow-y-auto and parent clipping */}
      {mounted && isOpen && !disabled && createPortal(
        <div
          ref={dropdownRef}
          style={dropdownStyle}
          className={cn(
            "overflow-hidden rounded-xl border border-border/80 bg-popover text-popover-foreground shadow-[0_16px_40px_rgba(0,0,0,0.25),0_4px_16px_rgba(0,0,0,0.15)] dark:shadow-[0_16px_40px_rgba(0,0,0,0.85)] backdrop-blur-md animate-in fade-in-50 zoom-in-95 duration-100",
            dropdownClassName
          )}
          onKeyDown={handleKeyDown}
        >
