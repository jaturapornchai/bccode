...
class ProductGroupRepository {
  Future<ApiResponse> getProductGroupManyByCode(List<String> codeList) async {
    ...
    try {
      ...
      final response =
          await client.get('/product/group/by-code?codes=$codeJson');
      try {
  ...
  Future<ApiResponse> getProductGroupList({
    ...
    try {
      String query =
          "/product/group/list?offset=$offset&limit=$limit&q=$search&sort=code:1";
  ...
  Future<ApiResponse> updateProductGroup(
      String guid, ProductGroupModel categoryModel) async {
    Dio client = Client().init();
    final data = categoryModel.toJson();
    try {
