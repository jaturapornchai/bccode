...
class MasterGradeRepository {
  Future<ApiResponse> getGradeManyByCode(List<String> codeList) async {
    ...
    try {
      ...
      final response =
          await client.get('/aicloud/grade/by-code?codes=$codeJson');
      try {
  ...
  Future<ApiResponse> getGradeList({
    ...
    try {
      String query =
          "/aicloud/grade/list?offset=$offset&limit=$limit&q=$search&sort=code:1";
  ...
  Future<ApiResponse> updateGrade(
      String guid, MasterGradeModel gradeModel) async {
    Dio client = Client().init();
    final data = gradeModel.toJson();
    try {
