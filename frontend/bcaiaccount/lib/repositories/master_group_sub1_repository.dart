...
class MasterGroupSub1Repository {
  Future<ApiResponse> getGroupSub1ManyByCode(List<String> codeList) async {
    ...
    try {
      ...
      final response =
          await client.get('/aicloud/groupsubone/by-code?codes=$codeJson');
      try {
  ...
  Future<ApiResponse> getGroupSub1List({
    ...
    try {
      String query =
          "/aicloud/groupsubone/list?offset=$offset&limit=$limit&q=$search&sort=code:1";
  ...
  Future<ApiResponse> deleteGroupSub1Many(List<String> guidList) async {
    ...
    try {
      ...
      final response =
          await client.delete('/aicloud/groupsubone', data: guidStrings);
      try {
        return ApiResponse.fromMap(response.data);
      } catch (ex) {
        throw Exception(ex);
  ...
  Future<ApiResponse> updateGroupSub1(
      String guid, MasterGroupSub1Model groupSub1Model) async {
    Dio client = Client().init();
    final data = groupSub1Model.toJson();
    try {
      final response =
          await client.put('/aicloud/groupsubone/$guid', data: data);
      try {
        return ApiResponse.fromMap(response.data);
      } catch (ex) {
        throw Exception(ex);
