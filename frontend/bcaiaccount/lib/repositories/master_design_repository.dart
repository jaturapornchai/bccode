...
class MasterDesignRepository {
  Future<ApiResponse> getDesignManyByCode(List<String> codeList) async {
    ...
    try {
      ...
      final response =
          await client.get('/aicloud/design/by-code?codes=$codeJson');
      try {
  ...
  Future<ApiResponse> getDesignList({
    ...
    try {
      String query =
          "/aicloud/design/list?offset=$offset&limit=$limit&q=$search&sort=code:1";
  ...
  Future<ApiResponse> deleteDesignMany(List<String> guidList) async {
    ...
    try {
      ...
      final response =
          await client.delete('/aicloud/design', data: guidStrings);
      try {
        return ApiResponse.fromMap(response.data);
      } catch (ex) {
        throw Exception(ex);
  ...
  Future<ApiResponse> updateDesign(
      String guid, MasterDesignModel designModel) async {
    Dio client = Client().init();
    final data = designModel.toJson();
    try {
