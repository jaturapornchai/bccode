...
/// หน้าจอแสดงรายการใบสั่งซื้อ (Purchase Order List)
...
class PurchaseOrderListScreen extends StatefulWidget {
  ...
  @override
  State<PurchaseOrderListScreen> createState() => PurchaseOrderListScreenState();
}

class PurchaseOrderListScreenState extends State<PurchaseOrderListScreen> with global.ThemeRefreshMixin {
  final TextEditingController _searchController = TextEditingController();
  final ScrollController _scrollController = ScrollController();
  final FocusNode _searchFocusNode = FocusNode(skipTraversal: true);

  List<TransactionModel> _poList = [];
  String _searchText = '';
  ...
  // Cubit สำหรับจัดการรายชื่อเจ้าหนี้
  ...
  void dispose() {
    ...
    // ล้าง debouncer เพื่อป้องกัน memory leak
    _debouncer.dispose();
    super.dispose();
  }
  ...
  void _silentRefresh() {
    // ไม่ทำ silent refresh ถ้ากำลัง loading หรือมีข้อมูลมากกว่า 1 page (กำลัง paginate)
    // เพื่อป้องกันการ reset list ขณะ user กำลัง scroll ดูข้อมูล
    if (_isLoading || _poList.length > 20) {
      return;
    }
  ...
  void _loadData({bool silent = false, bool loadMore = false}) {
    if (_isLoading && _searchText == _currentSearchRequest && !loadMore) {
      return;
    }

    _currentSearchRequest = _searchText;

    // ต้อง set _isLoading = true เสมอเมื่อโหลดข้อมูล (ยกเว้น silent refresh)
    if (!silent) {
      setState(() => _isLoading = true);
    }

    final offset = loadMore ? _poList.length : 0;
    _currentOffset = offset;

    // แปลง DateTime เป็น String format "YYYY-MM-DD" สำหรับ backend
    String? fromDateStr;
    String? toDateStr;
    ...
    AppLogger.info('🔍 [PO List] _loadData called - fromDate: $fromDateStr, toDate: $toDateStr, minAmount: $_filterMinAmount, maxAmount: $_filterMaxAmount, custCodes: $_filterCreditorCodes');
    ...
    context.read<TransBloc>().add(
      TransLoad(
        ...
        ispos: '',
        dateorder: 1,
        // ส่ง filter parameters ไป backend
  ...
  void _onScroll() {
    if (_scrollController.position.pixels >= _scrollController.position.maxScrollExtent - 200) {
      if (!_isLoading && _poList.length < _totalRecords) {
  ...
  Future<void> _loadApprovalStatuses() async {
    if (_poList.isEmpty) return;

    // Copy list เพื่อป้องกัน ConcurrentModificationException
    // เมื่อ loadMore เพิ่มข้อมูลขณะ iterate
    final poListSnapshot = List<TransactionModel>.from(_poList);

    for (final po in poListSnapshot) {
      if (po.docno.isEmpty) continue;
      try {
        final result = await ApprovalApiService.getPOApprovalStatus(po.docno);
        if (result.isSuccess && result.data != null && mounted) {
          setState(() {
            _approvalStatusMap[po.docno] = result.data!;
          });
        }
      } catch (e) {
        if (kDebugMode) {
          AppLogger.debug('Error loading approval status for ${po.docno}: $e');
        }
      }
    }
  }
  ...
  void _clearAllFilters() {
    setState(() {
      ...
      _poList = []; // ล้าง list เพื่อโหลดใหม่
    });
    _loadData(); // โหลดข้อมูลใหม่จาก backend
  }
  ...
  void refreshListAfterDelete(String deletedGuid) {
    setState(() {
      _poList.removeWhere((po) => po.guidfixed == deletedGuid);
  ...
  Widget build(BuildContext context) {
        ...
        if (state is TransLoadSuccess) {
          setState(() {
            ...
            if (_currentOffset == 0) {
              ...
              _poList.addAll(state.trans);
            }
            _totalRecords = state.totalRecord;
          });
          _loadApprovalStatuses();

          // Auto-load ต่อถ้ายังมีข้อมูลเหลือและ user อยู่ใกล้ bottom
          WidgetsBinding.instance.addPostFrameCallback((_) {
            if (_scrollController.hasClients &&
                _scrollController.position.pixels >= _scrollController.position.maxScrollExtent - 200 &&
                _poList.length < _totalRecords) {
              _loadData(loadMore: true);
            }
          });
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(global.language('purchase_order')),
      ),
      body: content,
    );
  }
  ...
  Widget _buildHeader() {
    // Backend ทำการกรองข้อมูลแล้ว _totalRecords คือจำนวนที่กรองได้
    String headerText;
  ...
  Widget _buildList() {
    if (_isLoading && _poList.isEmpty) {
      return const Center(child: CircularProgressIndicator());
    }

    // Backend กรองข้อมูลแล้ว ใช้ _poList โดยตรง
    if (_poList.isEmpty) {
      return Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
    ...
    // แสดง load more spinner เมื่อกำลังโหลดและยังมีข้อมูลเหลือ
    final hasMoreData = _poList.length < _totalRecords;
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        ...
        itemCount: _poList.length + (showLoadMoreSpinner ? 1 : 0),
        itemBuilder: (context, index) {
          if (index >= _poList.length) {
  ...
  Widget _buildPOCard(TransactionModel po) {
    return POListCard(
      po: po,
      isSelected: _selectedGuid == po.guidfixed,
      fontSize: _fontSize,
      approvalStatus: _approvalStatusMap[po.docno],
      onTap: () => _onPOSelected(po),
    );
  }

  /// โหลด document เต็มจาก MongoDB แล้วส่งไป edit screen
  Future<void> _onPOSelected(TransactionModel po) async {
    setState(() => _selectedGuid = po.guidfixed ?? '');

    // ถ้าไม่มี guidfixed ให้ใช้ข้อมูลเดิม (header only)
    if (po.guidfixed == null || po.guidfixed!.isEmpty) {
      _passDocumentToCallback(po);
      return;
    }

    // โหลด document เต็มจาก MongoDB
    try {
      ...
      AppLogger.info('[PO List] Loading full document from MongoDB: ${po.docno}');
      ...
      // แปลง data.result เป็น TransactionModel
      TransactionModel fullDocument = TransactionModel.fromJson(data.result);

      // Parse JSON data ของ transaction (เช่น details, custnames)
      global.parseTransactionJsonData(fullDocument);

      // รักษาค่า flags จาก PostgreSQL (MongoDB ไม่มีค่าเหล่านี้)
      fullDocument.isref = po.isref;
      fullDocument.islocked = po.islocked;
      fullDocument.isclosed = po.isclosed;
      fullDocument.iscancel = po.iscancel;
      fullDocument.isdelete = po.isdelete;
      fullDocument.iscomparedsuccess = po.iscomparedsuccess;
      fullDocument.isclosedmanual = po.isclosedmanual;
      fullDocument.closedmanualByCode = po.closedmanualByCode;
      fullDocument.closedmanualByName = po.closedmanualByName;
      fullDocument.closedmanualAt = po.closedmanualAt;
      fullDocument.closedmanualReason = po.closedmanualReason;

      AppLogger.info('[PO List] Loaded full document with ${fullDocument.details?.length ?? 0} details (isref: ${fullDocument.isref}, isdelete: ${fullDocument.isdelete}, iscancel: ${fullDocument.iscancel}, isclosed: ${fullDocument.isclosed})');
      _passDocumentToCallback(fullDocument);
    } catch (e) {
      AppLogger.error('[PO List] Error loading full document: $e');
      // ถ้าโหลดไม่สำเร็จ ใช้ข้อมูล header จาก PostgreSQL
      _passDocumentToCallback(po);
    }
  }

  /// ส่ง document ไปยัง callback หรือ pop กลับ
