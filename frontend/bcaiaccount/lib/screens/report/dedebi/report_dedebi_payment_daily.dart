...
class ReportPaymentDailyScreen extends StatefulWidget {
  ...
  @override
  State<ReportPaymentDailyScreen> createState() =>
...
class _ReportPaymentDailyScreenState extends State<ReportPaymentDailyScreen> {
  ...
  Widget _buildContent(BiReportState state) {
    return switch (state) {
      ...
      BiReportGenerateSuccess() => _buildSuccessContent(
        state.data.whereType<PaymentDailyModel>().toList(),
        state.jobId,
        state.meta,
      ),
  ...
  void _refreshReportData() {
    ...
    if (kDebugMode) {
      ...
      AppLogger.debug('   - reportType: ${BiReportType.paymentDaily}');
  ...
  void _showTransactionDialog(PaymentDailyModel data) {
    showDialog(
      context: context,
      builder: (BuildContext context) {
