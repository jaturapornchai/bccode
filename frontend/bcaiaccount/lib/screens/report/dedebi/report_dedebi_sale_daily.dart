...
class ReportDedebiSaleDailyScreen extends StatefulWidget {
  ...
  @override
  State<ReportDedebiSaleDailyScreen> createState() =>
      _ReportDedebiSaleDailyScreenState();
}

class _ReportDedebiSaleDailyScreenState
    extends State<ReportDedebiSaleDailyScreen> {
  ...
  Widget build(BuildContext context) {
    return PopScope(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          title: Text(
            global.language('report_dedebi_sale_daily_title'),
      ...
      case BiReportSummarySuccess():
        setState(() {
          ...
          if (state.dailySummaryData != null) {
            ...
            if (kDebugMode) {
              AppLogger.debug(
                '⚠️ Warning: dailySummaryData is null in BiReportSummarySuccess',
              );
            }
          }
  ...
  Widget _buildContent(BiReportState state) {
    return switch (state) {
      ...
      BiReportGenerateSuccess() => _buildSuccessContent(
        state.data.whereType<SaleDailyReportData>().toList(),
        state.jobId,
        state.meta,
      ),
  ...
  void _refreshReportData() {
    ...
    if (kDebugMode) {
      ...
      AppLogger.debug('   - reportType: ${BiReportType.saleDaily}');
