...
import 'package:smlaicloud/screens/report/dedebi/components/gross_profit_by_product_table_view.dart';
...
/// รายงานกำไรขั้นต้นตามสินค้า (Gross Profit By Product Report)
/// แสดงข้อมูลกำไรแยกตามสินค้า พร้อมวิเคราะห์เปอร์เซ็นต์กำไร
class ReportGrossProfitByProductScreen extends StatefulWidget {
  const ReportGrossProfitByProductScreen({super.key});

  @override
  State<ReportGrossProfitByProductScreen> createState() =>
      _ReportGrossProfitByProductScreenState();
}

class _ReportGrossProfitByProductScreenState
    extends State<ReportGrossProfitByProductScreen>
  ...
  List<GrossProfitByProductModel> _currentData = [];
  String _currentJobId = '';

  // Total summary data
  GrossProfitByProductSummary? _currentTotalSummary;
  ...
  void initState() {
    if (kDebugMode) {
      AppLogger.debug('🚀 ReportGrossProfitByProductScreen initState called');
    }
    super.initState();
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: Text(
        global.language('gross_profit_by_product_report'),
  ...
  void _handleStateChanges(BuildContext context, BiReportState state) {
    switch (state) {
      ...
      case BiReportSummarySuccess():
        setState(() {
          _currentTotalSummary = state.grossProfitByProductSummaryData;
    ...
    if (state is BiReportGenerateSuccess || state is BiReportDetailSuccess) {
      setState(() {
        _currentData = state.data
            .whereType<GrossProfitByProductModel>()
            .toList();
  ...
  void _requestTotalSummary(String jobId) {
    context.read<BiReportBloc>().add(
      GetBiReportSummaryRequested(
        reportType: BiReportType.grossProfitByProduct,
        jobId: jobId,
  ...
  Widget _buildContent(BiReportState state) {
    return switch (state) {
      ...
      BiReportGenerateSuccess() => _buildSuccessContent(
        state.data.whereType<GrossProfitByProductModel>().toList(),
        state.jobId,
        state.meta,
      ),
  ...
  Widget _buildSuccessContent(
    List<GrossProfitByProductModel> data,
    String jobId,
    BiReportMeta meta,
  ) {
  ...
  Widget _buildSidebar(List<GrossProfitByProductModel> data) {
    return Container(
      width: 260,
  ...
  Widget _buildDataGrid(List<GrossProfitByProductModel> data) {
    return Container(
      ...
      child: GrossProfitByProductTableView(data: data),
    );
  }
  ...
  void _refreshReportData() {
    ...
    AppLogger.debug('🔍 Gross Profit By Product Report - Creating conditions:');
    AppLogger.debug('   fromDate: $_fromDate');
    AppLogger.debug('   toDate: $_toDate');
    AppLogger.debug(
      '   selectedBarcodes: ${_selectedBarcodes.selectedEntities.map((e) => e.code).join(", ")}',
    );

    // Extract barcode from selected entities (use first selected or empty string)
    final barcode = _selectedBarcodes.selectedEntities.isNotEmpty
        ? _selectedBarcodes.selectedEntities.first.code
        : '';
    ...
    AppLogger.debug('   barcode: ${conditions.barcode}');
