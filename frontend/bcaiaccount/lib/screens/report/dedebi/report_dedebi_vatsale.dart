...
import 'package:smlaicloud/screens/report/dedebi/components/vat_sale_table_view.dart';
...
/// รายงานภาษีขาย (VAT Sale Report)
/// แสดงรายงานภาษีมูลค่าเพิ่มจากการขาย พร้อมข้อมูลใบกำกับภาษี
class ReportVatSaleScreen extends StatefulWidget {
  const ReportVatSaleScreen({super.key});

  @override
  State<ReportVatSaleScreen> createState() => _ReportVatSaleScreenState();
}

class _ReportVatSaleScreenState extends State<ReportVatSaleScreen> with global.ThemeRefreshMixin {
  ...
  List<VatSaleModel> _currentData = [];
  String _currentJobId = '';

  // Total summary data
  VatSaleSummary? _currentTotalSummary;
  ...
  void initState() {
    if (kDebugMode) {
      AppLogger.debug('🚀 ReportVatSaleScreen initState called');
    }
    super.initState();
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: Text(
        global.language('report_vat_sale'),
  ...
  void _handleStateChanges(BuildContext context, BiReportState state) {
    switch (state) {
      ...
      case BiReportSummarySuccess():
        setState(() {
          _currentTotalSummary = state.vatSaleSummaryData;
  ...
  void _updateCurrentData(dynamic state) {
    if (state is BiReportGenerateSuccess || state is BiReportDetailSuccess) {
      setState(() {
        _currentData = state.data.whereType<VatSaleModel>().toList();
  ...
  void _requestTotalSummary(String jobId) {
    context.read<BiReportBloc>().add(
      GetBiReportSummaryRequested(
        reportType: BiReportType.vatSale,
        jobId: jobId,
  ...
  Widget _buildContent(BiReportState state) {
    return switch (state) {
      ...
      BiReportGenerateSuccess() => _buildSuccessContent(
        state.data.whereType<VatSaleModel>().toList(),
        state.jobId,
        state.meta,
      ),
  ...
  Widget _buildSidebar(List<VatSaleModel> data) {
    return Container(
      width: 260,
  ...
  Widget _buildDataGrid(List<VatSaleModel> data) {
    return Container(
      ...
      child: VatSaleTableView(data: data),
    );
  }
  ...
  void _refreshReportData() {
    ...
    AppLogger.debug('🔍 VAT Sale Report - Creating conditions:');
