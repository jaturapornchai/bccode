...
class ReportDedebiSalesScreen extends StatefulWidget {
  ...
  @override
  State<ReportDedebiSalesScreen> createState() =>
...
class _ReportDedebiSalesScreenState extends State<ReportDedebiSalesScreen> {
  ...
  String _showCancelledDocuments = global.language('report_dedebi_hide_cancelled_docs');
  String _saleType = global.language('report_dedebi_all'); // ทั้งหมด (default)
  String _posType = global.language('report_dedebi_all'); // ทั้งหมด (default)
  ...
  EntitySelectionModel _selectedSalespersons = const EntitySelectionModel(
    selectedEntities: [],
    isCancel: false,
  ); // Employee selection
  ...
  void initState() {
    if (kDebugMode) {
      AppLogger.debug('🚀 ReportDedebiSalesScreen initState called');
    }
    super.initState();
  ...
  Widget _buildContent(BiReportState state) {
    return switch (state) {
      ...
      BiReportGenerateSuccess() => _buildSuccessContent(
        state.data.whereType<SaleReportData>().toList(),
        state.jobId,
        state.meta,
      ),
    ...
    await showDialog(
      ...
      builder: (BuildContext context) {
        return ReportConditionDialog(
          ...
          initialSelectedCreditors:
              _selectedDebtors, // เปลี่ยนเป็น _selectedDebtors
          ...
          onConditionsSet: (conditions) {
            ...
            setState(() {
              ...
              _selectedDebtors =
                  conditions.selectedCreditors!; // เปลี่ยนเป็น _selectedDebtors
  ...
  void _refreshReportData() {
    ...
    final conditions = ReportConditionsModel(
      ...
      iscancel:
          (_showCancelledDocuments == global.language('report_dedebi_hide_cancelled_docs') ||
              _showCancelledDocuments == '')
          ? 'false'
          : '',
      inquirytype: (_saleType == global.language('report_dedebi_all') || _saleType == '')
          ? ''
          : (_saleType == global.language('report_dedebi_sale_type') ? '1' : '2'),
    ...
    if (kDebugMode) {
      ...
      AppLogger.debug('   - reportType: ${BiReportType.sale}');
    ...
    try {
      ...
      global.showErrorSnackBar(context, '${global.language('report_dedebi_error_sending_request')}: $e');
    }
  }
