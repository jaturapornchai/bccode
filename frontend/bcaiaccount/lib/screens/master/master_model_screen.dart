...
class MasterModelScreen extends StatefulWidget {
  const MasterModelScreen({super.key});

  @override
  State<MasterModelScreen> createState() => MasterModelScreenState();
}

class MasterModelScreenState extends State<MasterModelScreen>
  ...
  Widget listScreen({bool mobileScreen = false}) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: <Widget>[
          const ManualButton(path: 'product-model'),
          Padding(
                      ...
                      actions: <Widget>[
                        ...
                        ElevatedButton(
                          ...
                          onPressed: () {
                            Navigator.pop(context);
                            context.read<MasterModelBloc>().add(
                              MasterModelDeleteMany(guid: modelGuidListChecked),
                            );
                          },
    ...
    if (screenEvent == global.ScreenEventEnum.edit ||
      ...
      if (verifyData(model)) {
        ...
        if (selectGuid.trim().isEmpty) {
          context.read<MasterModelBloc>().add(
            MasterModelSave(modelModel: model),
          );
        } else {
          context.read<MasterModelBloc>().add(
            MasterModelUpdate(guid: selectGuid, modelModel: model),
          );
        }
      }
    }
                      ...
                      actions: <Widget>[
                        ...
                        ElevatedButton(
                          ...
                          onPressed: () {
                            Navigator.pop(context);
                            context.read<MasterModelBloc>().add(
                              MasterModelDelete(guid: selectGuid),
                            );
                          },
                          child: Text(global.language('confirm')),
                        ),
                  ...
                  Padding(
                    ...
                    child: TextFormField(
                      ...
                      decoration: InputDecoration(
                        ...
                        border: const OutlineInputBorder(),
                        labelText:
                            "${global.language("model_name")} (${languageList[i].name})",
