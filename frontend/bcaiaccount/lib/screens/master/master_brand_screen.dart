...
class MasterBrandScreen extends StatefulWidget {
  const MasterBrandScreen({super.key});
  ...
  Widget listScreen({bool mobileScreen = false}) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: <Widget>[
          const ManualButton(path: 'product-brand'),
          Padding(
                      ...
                      actions: <Widget>[
                        ...
                        ElevatedButton(
                          ...
                          onPressed: () {
                            Navigator.pop(context);
                            context.read<MasterBrandBloc>().add(
                              MasterBrandDeleteMany(guid: brandGuidListChecked),
                            );
                          },
    ...
    if (screenEvent == global.ScreenEventEnum.edit ||
      ...
      if (verifyData(brand)) {
        ...
        if (selectGuid.trim().isEmpty) {
          context.read<MasterBrandBloc>().add(
            MasterBrandSave(brandModel: brand),
          );
        } else {
          context.read<MasterBrandBloc>().add(
            MasterBrandUpdate(guid: selectGuid, brandModel: brand),
          );
        }
      }
    }
