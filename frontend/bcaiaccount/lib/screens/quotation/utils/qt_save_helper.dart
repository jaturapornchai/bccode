...
/// Helper class สำหรับเตรียมข้อมูลก่อนบันทึกใบเสนอราคา
///
/// รับผิดชอบ:
/// - แปลงวันที่เป็น UTC
...
class QTSaveHelper {
  ...
  static void prepareDataForSave(
    ...
    // 3. ใบเสนอราคาไม่มีเงินมัดจำ — บังคับ sumdeposit = 0
    ...
    if (screenData.creatorcode == null || screenData.creatorcode!.isEmpty) {
      ...
      AppLogger.debug('[QTSaveHelper] ตั้งค่าผู้สร้างเอกสาร: $creatorCode / $creatorName');
    }

    AppLogger.debug('[QTSaveHelper] เตรียมข้อมูลก่อนบันทึกเรียบร้อย');
  }
  ...
  static void _ensureTimezoneFields(TransactionModel screenData) {
    ...
    AppLogger.info('[QTSaveHelper] เติม timezone fields: docdatelocal=${screenData.docdatelocal}, timezone=${screenData.timezone}');
  }
  ...
  static DateTime _safeParseDatetime(String? dateString) {
    ...
    try {
      ...
      AppLogger.warning('[QTSaveHelper] แปลงวันที่ไม่สำเร็จ: "$dateString" → ใช้ DateTime.now() แทน');
      return DateTime.now();
    }
  }
}
