...
/// หน้าจอตั้งค่าการอนุมัติใบสั่งซื้อ (PO Approval Setting Screen)
/// กำหนดกฎการอนุมัติตามวงเงินและประเภทการจัดซื้อ
class POApprovalSettingScreen extends StatefulWidget {
  const POApprovalSettingScreen({super.key});

  @override
  State<POApprovalSettingScreen> createState() =>
      _POApprovalSettingScreenState();
}

class _POApprovalSettingScreenState extends State<POApprovalSettingScreen>
  ...
  List<ApproverInfoModel> _allUsers = []; // รายชื่อผู้ใช้ทั้งหมดสำหรับเลือก
  ...
  void initState() {
    super.initState();
    _loadData();
    _loadAllUsers(); // โหลดรายชื่อผู้ใช้สำหรับเลือกผู้อนุมัติ
  }
  ...
  Future<void> _loadAllUsers() async {
    ...
    debugPrint('[POApprovalSetting] Loading users for approver selection...');
    final result = await ApprovalApiServiceUsers.getApproversUserList();

    if (result.isSuccess) {
      debugPrint('[POApprovalSetting] Loaded ${result.items.length} users');
      setState(() {
        _allUsers = result.items;
        _isLoadingUsers = false;
      });
    } else {
      debugPrint('[POApprovalSetting] Failed to load users: ${result.errorMessage}');
  ...
  Future<void> _loadData() async {
    ...
    if (typesResult.isSuccess && settingsResult.isSuccess) {
      // ถ้าไม่มีประเภทการจัดซื้อ ให้สร้าง "ทั่วไป" อัตโนมัติ
      if (typesResult.items.isEmpty) {
        await _createDefaultPurchaseType();
        return; // _loadData จะถูกเรียกอีกครั้งหลังสร้าง
      }
  ...
  /// สร้างประเภทการจัดซื้อ "ทั่วไป" เป็นค่าเริ่มต้น
  Future<void> _createDefaultPurchaseType() async {
    final defaultType = PurchaseTypeModel(
      ...
      descriptions: [LanguageDataModel(code: 'th', name: 'การสั่งซื้อทั่วไป')],
      isActive: true,
    );

    final result = await ApprovalApiService.savePurchaseType(defaultType);
    if (result.isSuccess) {
      await _loadData(); // โหลดใหม่หลังสร้าง
    } else {
      setState(() => _isLoading = false);
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text(global.language('po_approval_setting')),
        leading: IconButton(
  ...
  /// แสดง dialog เพิ่มประเภทการจัดซื้อใหม่
  Future<void> _showAddPurchaseTypeDialog() async {
    ...
    final result = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        title: Text(global.language('add_purchase_type')),
        content: Column(
          ...
          children: [
            TextField(
              ...
              decoration: InputDecoration(
                ...
                hintText: 'เช่น OFFICE, EQUIPMENT',
            ...
            TextField(
              ...
              decoration: InputDecoration(
                ...
                hintText: 'เช่น ซื้อเพื่อใช้ในสำนักงาน',
                border: OutlineInputBorder(),
              ),
            ),
            SizedBox(height: 12),
    ...
    if (result == true) {
      ...
      if (saveResult.isSuccess) {
        _showSuccess('เพิ่มประเภทการจัดซื้อสำเร็จ');
        await _loadData();
      } else {
        setState(() => _isLoading = false);
  ...
  Future<void> _deletePurchaseType(PurchaseTypeModel type) async {
    ...
    if (confirm == true) {
      ...
      // ลบประเภทการจัดซื้อ (ใช้ guid สำหรับ mainapi)
    ...
    return Column(
      ...
      children: [
        Container(
          ...
          child: Row(
            children: [
              Icon(Icons.category, color: global.theme.infoHighlightTextColor),
              SizedBox(width: 8),
              Expanded(
                child: Text(
          ...
          child: _purchaseTypes.isEmpty
              ? Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text('ยังไม่มีประเภทการจัดซื้อ'),
                      const SizedBox(height: 8),
                      TextButton(
                        onPressed: () {
                          Navigator.pushNamed(context, '/purchase_type_screen');
                        },
                        child: Text('ไปเพิ่มประเภทการจัดซื้อ'),
                      ),
                    ],
                  ),
                )
              : ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return ListTile(
                      selected: isSelected,
                      selectedTileColor: global.theme.columnHeaderColor,
                      leading: Stack(
                        children: [
                          CircleAvatar(
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            Text(
              'เลือกประเภทการจัดซื้อจากด้านซ้าย',
  ...
  Future<void> _showApproverSelectionDialog(int ruleIndex, ApprovalRuleModel rule) async {
    ...
    final result = await showDialog<List<ApproverInfoModel>>(
      context: context,
      builder: (dialogContext) => _ApproverSelectionDialog(
        allUsers: _allUsers,
...
class _ApproverSelectionDialog extends StatefulWidget {
  final List<ApproverInfoModel> allUsers;
  final Set<String> selectedUserCodes;
  final bool isLoading;

  const _ApproverSelectionDialog({
  ...
  @override
  State<_ApproverSelectionDialog> createState() => _ApproverSelectionDialogState();
}

class _ApproverSelectionDialogState extends State<_ApproverSelectionDialog>
    with global.ThemeRefreshMixin {
