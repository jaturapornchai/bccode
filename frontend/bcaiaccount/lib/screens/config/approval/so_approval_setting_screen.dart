...
/// หน้าจอตั้งค่าการอนุมัติใบสั่งขาย (SO Approval Setting Screen)
/// กำหนดกฎการอนุมัติตามวงเงินและประเภทใบสั่งขาย
class SOApprovalSettingScreen extends StatefulWidget {
  const SOApprovalSettingScreen({super.key});

  @override
  State<SOApprovalSettingScreen> createState() =>
      _SOApprovalSettingScreenState();
}

class _SOApprovalSettingScreenState extends State<SOApprovalSettingScreen>
  ...
  Future<void> _loadAllUsers() async {
    ...
    debugPrint('[SOApprovalSetting] Loading users for approver selection...');
    final result = await ApprovalApiServiceUsers.getApproversUserList();

    if (result.isSuccess) {
      debugPrint('[SOApprovalSetting] Loaded ${result.items.length} users');
      setState(() {
        _allUsers = result.items;
        _isLoadingUsers = false;
      });
    } else {
      debugPrint('[SOApprovalSetting] Failed to load users: ${result.errorMessage}');
  ...
  Future<void> _loadData() async {
    ...
    if (typesResult.isSuccess && settingsResult.isSuccess) {
      // ถ้าไม่มีประเภทใบสั่งขาย ให้สร้าง "ทั่วไป" อัตโนมัติ
  ...
  /// สร้างประเภทใบสั่งขาย "ทั่วไป" เป็นค่าเริ่มต้น
  Future<void> _createDefaultPurchaseType() async {
    final defaultType = PurchaseTypeModel(
      ...
      descriptions: [LanguageDataModel(code: 'th', name: 'ใบสั่งขายทั่วไป')],
      isActive: true,
    );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text(global.language('approve_sale_order')),
        leading: IconButton(
  ...
  /// แสดง dialog เพิ่มประเภทใบสั่งขายใหม่
  Future<void> _showAddPurchaseTypeDialog() async {
    ...
    final result = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        title: Text(global.language('add_sale_order_type')),
        content: Column(
          ...
          children: [
            ...
            TextField(
              ...
              decoration: InputDecoration(
                ...
                hintText: global.language('eg_general_special_sale_order'),
                border: OutlineInputBorder(),
              ),
            ),
            SizedBox(height: 12),
    ...
    if (result == true) {
      ...
      if (saveResult.isSuccess) {
        _showSuccess('เพิ่มประเภทใบสั่งขายสำเร็จ');
        await _loadData();
      } else {
        setState(() => _isLoading = false);
  ...
  Future<void> _deletePurchaseType(PurchaseTypeModel type) async {
    ...
    if (confirm == true) {
      ...
      // ลบประเภทใบสั่งขาย (ใช้ guid สำหรับ mainapi)
    ...
    return Column(
      ...
      children: [
        Container(
          ...
          child: Row(
            children: [
              Icon(Icons.shopping_cart, color: global.theme.infoHighlightTextColor),
              SizedBox(width: 8),
              Expanded(
                child: Text(
                  global.language('sale_order_type'),
                  style: TextStyle(
                    fontSize: 16,
              ...
              ? Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text('ยังไม่มีประเภทใบสั่งขาย'),
                      const SizedBox(height: 8),
                      TextButton(
                        onPressed: () {
                          Navigator.pushNamed(context, '/sale_order_type_screen');
                        },
                        child: Text('ไปเพิ่มประเภทใบสั่งขาย'),
                      ),
                    ],
                  ),
                )
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            Text(
              'เลือกประเภทใบสั่งขายจากด้านซ้าย',
  ...
  Future<void> _showApproverSelectionDialog(int ruleIndex, ApprovalRuleModel rule) async {
    ...
    final result = await showDialog<List<ApproverInfoModel>>(
      context: context,
      builder: (dialogContext) => _SOApproverSelectionDialog(
        allUsers: _allUsers,
...
class _SOApproverSelectionDialog extends StatefulWidget {
  final List<ApproverInfoModel> allUsers;
  final Set<String> selectedUserCodes;
  final bool isLoading;

  const _SOApproverSelectionDialog({
  ...
  @override
  State<_SOApproverSelectionDialog> createState() => _SOApproverSelectionDialogState();
}

class _SOApproverSelectionDialogState extends State<_SOApproverSelectionDialog>
    with global.ThemeRefreshMixin {
