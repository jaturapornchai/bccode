...
class SaleChannelScreenState extends State<SaleChannelScreen>
  ...
  Future<void> getTemplate() async {
    const githubRawUrl =
        'https://raw.githubusercontent.com/smlsoft/dedepos_template/main/delivery.json';
    try {
      final fileContent = await global.readFileFromGithub(githubRawUrl);
      final salechannel = (json.decode(fileContent) as List)
            ...
            child: IconButton(
              ...
              onPressed: () async {
                ...
                if (selectedData.isNotEmpty) {
                  ...
                  // ignore: use_build_context_synchronously
                  context.read<SaleChannelBloc>().add(
                    SaleChannelSaveBulk(salechannels: saleChannelTempSeleted),
                  );
                }
              },
                      ...
                      actions: <Widget>[
                        ...
                        ElevatedButton(
                          ...
                          onPressed: () {
                            Navigator.pop(context);
                            context.read<SaleChannelBloc>().add(
                              SaleChannelDeleteMany(guid: guidListChecked),
                            );
                          },
  ...
  void saveOrUpdateData() {
    ...
    if (selectGuid.trim().isEmpty) {
      if (imageFile.path.isNotEmpty) {
        ...
        context.read<SaleChannelBloc>().add(
          SaleChannelSave(salechannelmodel: screenData),
        );
      }
    } else {
  ...
  void updateData(String guid) {
    ...
    if (imageWeb != null) {
      ...
      context.read<SaleChannelBloc>().add(
        SaleChannelUpdate(guid: guid, salechannelmodel: screenData),
      );
    }
  }
          ...
          children: [
            Expanded(
              child: priceList.isEmpty
                  ? Text(global.language("no_delivery_price_in_pos"))
                  : DropdownButton<PriceModel>(
                      value:
                          priceList.firstWhereOrNull(
                            (element) => element.keyNumber == screenData.price,
                          ) ??
                      ...
                      actions: <Widget>[
                        ...
                        ElevatedButton(
                          ...
                          onPressed: () {
                            Navigator.pop(context);
                            context.read<SaleChannelBloc>().add(
                              SaleChannelDelete(guid: selectGuid),
                            );
                          },
