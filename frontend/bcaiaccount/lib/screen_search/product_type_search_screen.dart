...
class ProductTypeSearchScreen extends StatefulWidget {
  const ProductTypeSearchScreen({super.key, required this.word});
  final String word;

  @override
  State<ProductTypeSearchScreen> createState() =>
      ProductTypeSearchScreenState();
}

class ProductTypeSearchScreenState extends State<ProductTypeSearchScreen>
  ...
  void loadDataList(String search) {
    context.read<ProductTypeBloc>().add(
      ProductTypeLoadList(
        offset: (productTypetListData.isEmpty)
            ? 0
            : productTypetListData.length,
      ...
      appBar: AppBar(
        ...
        leading: IconButton(
          ...
          onPressed: () {
            Navigator.pop(
              context,
              ProductTypeModel(code: '', names: [], guidfixed: ''),
            );
          },
        ),
      ),
          ...
          if (kIsWeb ||
            ...
            if (event is RawKeyDownEvent) {
              ...
              if (event.logicalKey == LogicalKeyboardKey.arrowUp) {
                isKeyDown = false;
                int index = productTypetListData.indexOf(
                  productTypetListData.firstWhere(
  ...
  Widget listObject(ProductTypeModel value, int index) {
