...
class ProductWarehouseSearchScreen extends StatefulWidget {
  const ProductWarehouseSearchScreen({super.key, required this.word});
  final String word;

  @override
  State<ProductWarehouseSearchScreen> createState() =>
      ProductWarehouseSearchScreenState();
}

class ProductWarehouseSearchScreenState
    extends State<ProductWarehouseSearchScreen>
  ...
  void loadDataList(String search) {
    context.read<WarehouseBloc>().add(
      WarehouseLoadList(
        offset: (warehouseListData.isEmpty) ? 0 : warehouseListData.length,
          ...
          if (kIsWeb ||
            ...
            if (event is RawKeyDownEvent) {
              ...
              if (event.logicalKey == LogicalKeyboardKey.tab) {
                if (selectGuid != "") {
                  Navigator.pop(
                    context,
                    SearchGuidCodeNameModel(
                      guid: warehouseListData[currentListIndex].guidfixed,
                      code: warehouseListData[currentListIndex].code,
                      names: warehouseListData[currentListIndex].names,
                      isCancel: false,
                    ),
                  );
                }
              ...
              if (event.logicalKey == LogicalKeyboardKey.arrowUp) {
                isKeyDown = false;
                int index = warehouseListData.indexOf(
                  warehouseListData.firstWhere(
  ...
  Widget listObject(WarehouseModel value, int index) {
