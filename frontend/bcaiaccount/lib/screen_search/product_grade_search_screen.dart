...
class ProductGradeSearchScreen extends StatefulWidget {
  const ProductGradeSearchScreen({super.key, required this.word});
  final String word;

  @override
  State<ProductGradeSearchScreen> createState() =>
      ProductGradeSearchScreenState();
}

class ProductGradeSearchScreenState extends State<ProductGradeSearchScreen>
    with SingleTickerProviderStateMixin, global.ThemeRefreshMixin {
  TextEditingController searchController = TextEditingController();
  FocusNode searchFocusNode = FocusNode(skipTraversal: true);
  ScrollController listScrollController = ScrollController();
  String searchText = "";
  List<MasterGradeModel> masterGradeListData = [];
  bool isKeyUp = false;
  ...
  void loadDataList(String search) {
    ...
    context.read<MasterGradeBloc>().add(
      MasterGradeLoadList(
        offset: (masterGradeListData.isEmpty) ? 0 : masterGradeListData.length,
          ...
          if (kIsWeb ||
            ...
            if (event is RawKeyDownEvent) {
              ...
              if (event.logicalKey == LogicalKeyboardKey.tab) {
                if (selectGuid != "") {
                  Navigator.pop(
                    context,
                    global.SearchCodeNameModel(
                      code: masterGradeListData[currentListIndex].code,
                      names: masterGradeListData[currentListIndex].names,
                      isCancel: false,
                      guidfixed: '',
                    ),
                  );
              ...
              if (event.logicalKey == LogicalKeyboardKey.arrowUp) {
                isKeyDown = false;
                int index = masterGradeListData.indexOf(
                  masterGradeListData.firstWhere(
                    (element) => element.guidfixed == selectGuid,
                  ),
                );
                if (index > 0) {
                  selectGuid = masterGradeListData[index - 1].guidfixed;
                  isKeyUp = true;
                }
                setState(() {});
              }
              if (event.logicalKey == LogicalKeyboardKey.arrowDown) {
                ...
                if (index < masterGradeListData.length - 1) {
                  selectGuid = masterGradeListData[index + 1].guidfixed;
                }
                isKeyDown = true;
                setState(() {});
  ...
  Widget listObject(MasterGradeModel value, int index) {
  ...
  Widget build(BuildContext context) {
    for (int i = 0; i < masterGradeListData.length; i++) {
      if (masterGradeListData[i].guidfixed == selectGuid) {
        currentListIndex = i;
        break;
      }
    }
              ...
              if (state is MasterGradeLoadSuccess) {
                setState(() {
                  ...
                  if (state.grades.isNotEmpty) {
                    masterGradeListData.addAll(state.grades);
                    if (masterGradeListData.isNotEmpty) {
                      selectGuid = masterGradeListData[0].guidfixed;
                    } else {
                      selectGuid = "";
                    }
                  }
