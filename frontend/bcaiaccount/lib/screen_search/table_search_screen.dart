...
class TableSearchScreen extends StatefulWidget {
  const TableSearchScreen({super.key, required this.word});
  final String word;
...
class OrderSettingSearchScreenState extends State<TableSearchScreen>
  ...
  void loadDataList(String search) {
    context.read<TableBloc>().add(
      TableLoadList(
        offset: (tableListData.isEmpty) ? 0 : tableListData.length,
          ...
          if (kIsWeb ||
            ...
            if (event is RawKeyDownEvent) {
              ...
              if (event.logicalKey == LogicalKeyboardKey.tab) {
                if (selectGuid != "") {
                  Navigator.pop(
                    context,
                    SearchGuidCodeNameModel(
                      guid: tableListData[currentListIndex].guidfixed,
                      code: tableListData[currentListIndex].number,
                      names: tableListData[currentListIndex].names,
                      isCancel: false,
                    ),
                  );
                }
              ...
              if (event.logicalKey == LogicalKeyboardKey.arrowUp) {
                isKeyDown = false;
                int index = tableListData.indexOf(
                  tableListData.firstWhere(
