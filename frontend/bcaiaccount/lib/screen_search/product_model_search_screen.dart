...
class ProductModelSearchScreen extends StatefulWidget {
  const ProductModelSearchScreen({super.key, required this.word});
  final String word;

  @override
  State<ProductModelSearchScreen> createState() =>
      ProductModelSearchScreenState();
}

class ProductModelSearchScreenState extends State<ProductModelSearchScreen>
    with SingleTickerProviderStateMixin, global.ThemeRefreshMixin {
  TextEditingController searchController = TextEditingController();
  FocusNode searchFocusNode = FocusNode(skipTraversal: true);
  ScrollController listScrollController = ScrollController();
  String searchText = "";
  List<MasterModelModel> masterModelListData = [];
  bool isKeyUp = false;
  ...
  void loadDataList(String search) {
    ...
    context.read<MasterModelBloc>().add(
      MasterModelLoadList(
        offset: (masterModelListData.isEmpty) ? 0 : masterModelListData.length,
          ...
          if (kIsWeb ||
            ...
            if (event is RawKeyDownEvent) {
              ...
              if (event.logicalKey == LogicalKeyboardKey.tab) {
                if (selectGuid != "") {
                  Navigator.pop(
                    context,
                    global.SearchCodeNameModel(
                      code: masterModelListData[currentListIndex].code,
                      names: masterModelListData[currentListIndex].names,
                      isCancel: false,
                      guidfixed: '',
                    ),
                  );
              ...
              if (event.logicalKey == LogicalKeyboardKey.arrowUp) {
                isKeyDown = false;
                int index = masterModelListData.indexOf(
                  masterModelListData.firstWhere(
                    (element) => element.guidfixed == selectGuid,
                  ),
                );
                if (index > 0) {
                  selectGuid = masterModelListData[index - 1].guidfixed;
                  isKeyUp = true;
                }
                setState(() {});
              }
              if (event.logicalKey == LogicalKeyboardKey.arrowDown) {
                ...
                if (index < masterModelListData.length - 1) {
                  selectGuid = masterModelListData[index + 1].guidfixed;
                }
                isKeyDown = true;
                setState(() {});
  ...
  Widget listObject(MasterModelModel value, int index) {
  ...
  Widget build(BuildContext context) {
    for (int i = 0; i < masterModelListData.length; i++) {
      if (masterModelListData[i].guidfixed == selectGuid) {
        currentListIndex = i;
        break;
      }
    }
              ...
              if (state is MasterModelLoadSuccess) {
                setState(() {
                  ...
                  if (state.models.isNotEmpty) {
                    masterModelListData.addAll(state.models);
                    if (masterModelListData.isNotEmpty) {
                      selectGuid = masterModelListData[0].guidfixed;
                    } else {
                      selectGuid = "";
                    }
                  }
