...
class ProductPatternSearchScreen extends StatefulWidget {
  const ProductPatternSearchScreen({super.key, required this.word});
  final String word;

  @override
  State<ProductPatternSearchScreen> createState() =>
      ProductPatternSearchScreenState();
}
...
class ProductPatternSearchScreenState extends State<ProductPatternSearchScreen>
  ...
  String searchText = "";
  List<MasterPatternModel> masterPatternListData = [];
  bool isKeyUp = false;
  ...
  void loadDataList(String search) {
    ...
    context.read<MasterPatternBloc>().add(
      MasterPatternLoadList(
        offset: (masterPatternListData.isEmpty)
            ? 0
            : masterPatternListData.length,
          ...
          if (kIsWeb ||
            ...
            if (event is RawKeyDownEvent) {
              ...
              if (event.logicalKey == LogicalKeyboardKey.tab) {
                if (selectGuid != "") {
                  Navigator.pop(
                    context,
                    global.SearchCodeNameModel(
                      code: masterPatternListData[currentListIndex].code,
                      names: masterPatternListData[currentListIndex].names,
                      isCancel: false,
                      guidfixed: '',
                    ),
                  );
              ...
              if (event.logicalKey == LogicalKeyboardKey.arrowUp) {
                isKeyDown = false;
                int index = masterPatternListData.indexOf(
                  masterPatternListData.firstWhere(
                    (element) => element.guidfixed == selectGuid,
                  ),
                );
                if (index > 0) {
                  selectGuid = masterPatternListData[index - 1].guidfixed;
                  isKeyUp = true;
                }
                setState(() {});
              }
              if (event.logicalKey == LogicalKeyboardKey.arrowDown) {
                ...
                if (index < masterPatternListData.length - 1) {
                  selectGuid = masterPatternListData[index + 1].guidfixed;
                }
                isKeyDown = true;
                setState(() {});
  ...
  Widget build(BuildContext context) {
    for (int i = 0; i < masterPatternListData.length; i++) {
      if (masterPatternListData[i].guidfixed == selectGuid) {
        currentListIndex = i;
        break;
      }
    }
              ...
              if (state is MasterPatternLoadSuccess) {
                setState(() {
                  ...
                  if (state.patterns.isNotEmpty) {
                    masterPatternListData.addAll(state.patterns);
                    if (masterPatternListData.isNotEmpty) {
                      selectGuid = masterPatternListData[0].guidfixed;
                    } else {
                      selectGuid = "";
                    }
                  }
