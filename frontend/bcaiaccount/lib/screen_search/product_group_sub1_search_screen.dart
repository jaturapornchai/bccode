...
class ProductGroupSub1SearchScreen extends StatefulWidget {
  const ProductGroupSub1SearchScreen({super.key, required this.word});
  final String word;

  @override
  State<ProductGroupSub1SearchScreen> createState() =>
      ProductGroupSub1SearchScreenState();
}

class ProductGroupSub1SearchScreenState
    extends State<ProductGroupSub1SearchScreen>
    with SingleTickerProviderStateMixin, global.ThemeRefreshMixin {
  TextEditingController searchController = TextEditingController();
  FocusNode searchFocusNode = FocusNode(skipTraversal: true);
  ScrollController listScrollController = ScrollController();
  String searchText = "";
  List<MasterGroupSub1Model> masterGroupSub1ListData = [];
  bool isKeyUp = false;
  ...
  void loadDataList(String search) {
    ...
    context.read<MasterGroupSub1Bloc>().add(
      MasterGroupSub1LoadList(
        offset: (masterGroupSub1ListData.isEmpty)
            ? 0
            : masterGroupSub1ListData.length,
          ...
          if (kIsWeb ||
            ...
            if (event is RawKeyDownEvent) {
              ...
              if (event.logicalKey == LogicalKeyboardKey.tab) {
                if (selectGuid != "") {
                  Navigator.pop(
                    context,
                    global.SearchCodeNameModel(
                      code: masterGroupSub1ListData[currentListIndex].code,
                      names: masterGroupSub1ListData[currentListIndex].names,
                      isCancel: false,
                      guidfixed: '',
                    ),
                  );
              ...
              if (event.logicalKey == LogicalKeyboardKey.arrowUp) {
                isKeyDown = false;
                int index = masterGroupSub1ListData.indexOf(
                  masterGroupSub1ListData.firstWhere(
                    (element) => element.guidfixed == selectGuid,
                  ),
                );
                if (index > 0) {
                  selectGuid = masterGroupSub1ListData[index - 1].guidfixed;
                  isKeyUp = true;
                }
                setState(() {});
              }
              if (event.logicalKey == LogicalKeyboardKey.arrowDown) {
                ...
                if (index < masterGroupSub1ListData.length - 1) {
                  selectGuid = masterGroupSub1ListData[index + 1].guidfixed;
                }
                isKeyDown = true;
                setState(() {});
          ...
          children: [
            ...
            Expanded(
              child: SingleChildScrollView(
                ...
                child: Column(
                  children: masterGroupSub1ListData
  ...
  Widget listObject(MasterGroupSub1Model value, int index) {
  ...
  Widget build(BuildContext context) {
    for (int i = 0; i < masterGroupSub1ListData.length; i++) {
      if (masterGroupSub1ListData[i].guidfixed == selectGuid) {
        currentListIndex = i;
        break;
      }
    }
              ...
              if (state is MasterGroupSub1LoadSuccess) {
                setState(() {
                  ...
                  if (state.groupSub1s.isNotEmpty) {
                    masterGroupSub1ListData.addAll(state.groupSub1s);
                    if (masterGroupSub1ListData.isNotEmpty) {
                      selectGuid = masterGroupSub1ListData[0].guidfixed;
                    } else {
                      selectGuid = "";
                    }
                  }
