...
3. **goapi consumer** เริ่มที่ `handlers.StartConsumers()` (backend/internal/goapi/bootstrap.go:157-160, gate ด้วย env `ENABLE_KAFKA=="true"`; env ตัวนี้ถูก set จาก key `enablekafka` ใน bootstrap.json → backend/internal/goapi/setupconfig/loader.go:77,292; local container มี `/app/bootstrap.json` ค่า `"enablekafka": "true"` — docker exec mainapi read-only) → เขียนตาราง PG per-holding `doc / docdetail / docref / docpayment` (backend/internal/goapi/mypg/insert_doc.go:77-83,171-173,239-242,323-329)
