...
> อัปเดต 2026-09-25: ตรวจ §2–§8, §11–§12 และช่องว่างใหม่หลังถอด MongoDB/Kafka/Redis/ClickHouse ([ADR](decisions/2026-09-23-remove-mongo-kafka-redis-clickhouse.md)) — BFF ที่ปลายทาง backend ถูกลบแล้วติดสถานะ **BACKEND-REMOVED** (จอขึ้น "รอพัฒนา" ผ่าน `isMenuBackendRetired`, `frontend/src/lib/menu-screen-status.ts:40-55`); เลขบรรทัดใน §9–§10, §13 ยังเป็นของรอบ 2026-09-07/09-16
...
- `next.config.ts` rewrite `/backend/:path*` → `${BCAI_LOCAL_BACKEND_URL}/:path*` (afterFiles); ไม่ตั้ง env จะใช้ค่า fallback `http://mainapi:8888` (production) / `http://localhost:8888` (dev) ไม่ throw แล้ว (`frontend/next.config.ts:19,71`) — ส่วน route handler ฝั่ง server ยัง throw ถ้าไม่ตั้ง (`frontend/src/lib/backend-url.ts:117-120`)
- บล็อก (beforeFiles → `/_blocked-auth-route`) กลุ่ม login/register ที่ browser ไม่ควรเรียกตรง เช่น `/backend/login`, `/backend/googlelogin`, `/backend/dev-login`, `/backend/demo-login`, `/backend/register-username` (`frontend/next.config.ts:26-38`) และกลุ่มอันตราย `/backend/goapi/get|exec|getdoc`, `/backend/reportm/*`, `/backend/goapi/api/setup/*`, `/backend/goapi/api/mcp/*`, `/backend/goapi/mcp/*`, `/backend/reload-config`, `/backend/profile/link-line*` (`frontend/next.config.ts:51-64`)
...
| JWT verify ฝั่ง Next | HS256 ด้วย `JWT_SECRET_KEY` + ตรวจ `exp` — ใช้เฉพาะ system-settings เมื่อคำขอระบุ `holdingcode` (ข้ามถ้าไม่ตั้ง env); **ไม่เทียบ claim `holdingcode` กับค่าที่ขอ และไม่ยิง `/select-holding` แล้ว** (kind `atlas` ถูกลบ 2026-09-23) — การตรวจสิทธิ์จริงอยู่ที่ backend (`backend/pkg/microservice/live_authorization.go`) | `frontend/src/lib/server-jwt.ts:7-34`, `frontend/src/app/api/system-settings/[[...settingPath]]/route.ts:170-186` |
| Login methods | username/password → `/login`; Google → ตรวจ `oauth2.googleapis.com/tokeninfo` แล้วค่อย `/googlelogin`; Demo → `/demo-login` (backend ตัดสินด้วย `BCAI_DEMO_LOGIN_ENABLED`); Dev → BFF `/api/auth/dev-login` → `/dev-login` ต้อง `BCAI_DEV_LOGIN_ENABLED=true` และ secret ≥ 32 ตัวอักษร (**หน้า login ไม่มีปุ่ม Dev Login แล้ว** — ปุ่มถูกแทนด้วย Demo, `frontend/src/app/login-screen.tsx:317-323`; route BFF ยังอยู่) | `frontend/src/app/api/auth/login/route.ts:63`, `frontend/src/app/api/auth/google/verify/route.ts:54-55,82`, `frontend/src/app/api/auth/demo-login/route.ts:9,27`, `frontend/src/app/api/auth/dev-login/route.ts:9,22-27,34` |
...
|---|---|---|---|---|
| `/` | Login (username, Google, Demo — ไม่มีปุ่ม Dev Login; ปุ่มที่ใช้ class `dev-login-button` คือปุ่ม Demo) | `login-wrapper.tsx` → `login-screen.tsx` (841 บรรทัด) | LIVE | `frontend/src/app/page.tsx:2,9-10`, `frontend/src/app/login-wrapper.tsx:4,73`, `frontend/src/app/login-screen.tsx:317-323,657-662` |
| `/holding` | เลือก/สร้างกลุ่มกิจการ + แสดงผู้ดูแลแบบอ่านอย่างเดียว (เพิ่ม/ถอดที่ ตั้งค่าระบบและการเข้าถึง › บัญชีเข้าระบบ) | `holding/holding-screen.tsx` (1474) | LIVE | `frontend/src/app/holding/page.tsx:3,11`, `frontend/src/app/holding/holding-screen.tsx:1108-1110` |
| `/workspace` | เลือกบริษัท/สาขา, สร้าง holding/branch, หน่วยนับเริ่มต้น, ผูก LINE | `workspace/workspace-screen.tsx` (2280) | LIVE (หน่วยนับ = BACKEND-REMOVED) | `frontend/src/app/workspace/page.tsx:3,11` |
| `/menu` | เมนูหลัก + dashboard + ทางลัด; จอสินค้า/บาร์โค้ด/ชุดสินค้า/marketplace (tabs) ยังอยู่ในโค้ดแต่ route เมนูอยู่ใน `RETIRED_BACKEND_ROUTES` | `menu/main-menu-screen.tsx` (2965) และ `menu/*-screen.tsx` | LIVE (สินค้า = BACKEND-REMOVED) | `frontend/src/app/menu/page.tsx:3,11`, `frontend/src/lib/menu-screen-status.ts:40-43` |
| `/gl/[...glPath]` | จอบัญชีแยกประเภท (ผังบัญชี, สมุดรายวัน, ผ่านรายการ, ปิดงวด/สิ้นปี, รายงาน GL) | `gl/general-ledger-screen.tsx` | LIVE | `frontend/src/app/gl/[...glPath]/page.tsx`, `frontend/src/app/gl/general-ledger-screen.tsx:14-35` |
| `/mcp-tokens` | จัดการ API / MCP token (OWNER/ADMIN) | `mcp-tokens/token-screen.tsx` | LIVE | `frontend/src/app/mcp-tokens/page.tsx` |
| `/[systemSetting]` | config slug (`SYSTEM_SETTING_CONFIGS` + `productMasterConfigs()`; นับ 31 slug เมื่อ 2026-09-07) — redirect 4 ตัว: `permissionlink`→`/user`, `approvalsetting`→`/workspace`, `productcategorylist`→`/productcategorygroupselectscreen`, `bank`→`/bookbankscreen`; slug ไม่รู้จัก → 404; slug ที่ `basePath` ไม่ใช่ API บน PostgreSQL = BACKEND-REMOVED | `system-settings/system-settings-screen.tsx` (**16107 บรรทัด**) | LIVE บางจอ | `frontend/src/app/[systemSetting]/page.tsx:19-24`, `frontend/src/lib/system-setting-screens.ts:294,1114,1358-1372`, `frontend/src/lib/menu-screen-status.ts:45-55` |
| `/currency` | สกุลเงิน CRUD | `currency/currency-screen.tsx` (1033) | BACKEND-REMOVED (backend ไม่มี `/currency` แล้ว) | `frontend/src/app/currency/page.tsx:3,11` |
| `/price_history` | ประวัติราคาสินค้า | `menu/product-price-history-screen.tsx` | BACKEND-REMOVED | `frontend/src/app/price_history/page.tsx:3,11` |
| `/product_barcode_shelf` | พิมพ์ป้ายสินค้า | `menu/product-barcode-shelf-screen.tsx` | BACKEND-REMOVED | `frontend/src/app/product_barcode_shelf/page.tsx:3,11` |
| `/line-oa` | redirect ไป `/workspace` (backend LINE OA เดิมถูกถอด 2026-09-23) | — | REDIRECT | `frontend/src/app/line-oa/page.tsx:3-8` |
...
| `/api/workspace/[...path]` | GET/POST | M: `holdings`→`/list-holding?limit=100`+`/holding/{code}`, `holding-info`, `branches`→`/organization/branch/list`, `product-units*`→`/unit/list`,`/unit`,`/unit/bulk`; POST `select-holding`→`/select-holding`, `branch`→`/organization/branch`, `create-holding`→`/create-holding`, `update-holding`, `product-units/defaults` | Bearer | LIVE (900 บรรทัด; `product-units*` = BACKEND-REMOVED — backend ไม่มี `/unit*` แล้ว) | `workspace/[...workspacePath]/route.ts:44-101,126-178,213,415,471,496` |
| `/api/system-settings/[[...path]]` | GET/POST/PUT/DELETE | ตาม `kind` ของ slug — เหลือ 6 แบบ (`SystemSettingKind`, `atlas`/`ai-provider`/`copy-uat` ถูกลบออกพร้อมกับ MongoDB 2026-09-23): `main-crud` (ค่าเริ่มต้น)→`basePath` — มี API บน PostgreSQL เฉพาะ `/organization/business-type`, `/organization/branch`, `/holding/employee`, `/holding/permission`, `/organization/role-permission` (`menu-screen-status.ts:45-49`) ส่วน basePath อื่น (เช่น `/debtaccount/debtor`, `/unit`, `/product/group`, `/warehouse`) = BACKEND-REMOVED; `company`→GET `/shop/{id}`, POST/PUT `/holding/{code}`; `restaurant-setting` (จอ `workdayscreen`, `holidayscreen`)→`/restaurant/settings*` ซึ่ง backend ไม่มีแล้ว = BACKEND-REMOVED; `goapi-crud`→ใช้ G พร้อม query `holdingcode` (2026-09-25 ไม่มี config ใดใช้ kind นี้แล้ว); `permission-catalog`→ตอบจาก `flattenMenuItems()` ในหน่วยความจำ ไม่พร็อกซีไป backend; `report`→ปฏิเสธทุก method (405 "รายงานนี้ไม่มี CRUD API") | Bearer + JWT holdingcode (ถ้าตั้ง `JWT_SECRET_KEY`) ทุก kind ที่ส่ง holdingcode มา (ไม่มี `/select-holding` แยกแล้ว) | LIVE (537 บรรทัด) | `system-settings/[[...settingPath]]/route.ts:24-116,138-163,155-167,197-231,266-297`, `frontend/src/lib/system-setting-screens.ts:77-89` |
| `/api/product/[[...path]]` | GET/POST/PUT/DELETE | M `/product?…`, `/product/{id}`, `/product/resync` | Bearer | BACKEND-REMOVED | `product/[[...productPath]]/route.ts:15-24,43-51,133,152-164` |
| `/api/product-barcode/[[...path]]` | GET/POST/PUT/DELETE | M `/product/barcode*` | Bearer | BACKEND-REMOVED | `product-barcode/[[...barcodePath]]/route.ts:36,79,92,102-114` |
| `/api/product-barcode/list` | POST | G `/api/product/barcode/list` | Bearer | BACKEND-REMOVED | `product-barcode/list/route.ts:16,39` |
| `/api/product-barcode/master/[master]` | GET | M ตาม `MASTER_PATHS` 9 คีย์ — ใช้งานจริงแค่ `businesstype` (`/organization/business-type`) + `branch` (`/list-holding`); ที่เหลือ (`/product/group`, `/unit`, `/product/order-type`, `/debtaccount/*`, `/product`) เป็นท่อของจอ "รอพัฒนา" หลังถอด MongoDB ตาม ADR [ถอด MongoDB/Kafka/Redis/ClickHouse](decisions/2026-09-23-remove-mongo-kafka-redis-clickhouse.md) (2026-09-23 ลบ 6 คีย์ที่ไม่มีผู้เรียก) | Bearer (ผ่าน proxyMainApiJson) | LIVE (บางคีย์) | `product-barcode/master/[master]/route.ts:18-32` |
| `/api/product-barcode/bom/[barcode]` | GET | M `/product/barcode/bom/{code}?itemcode=` | Bearer | BACKEND-REMOVED | `product-barcode/bom/[barcode]/route.ts:10-42` |
| `/api/product-barcode/price-history/[barcode]` | GET | M `/product/barcode/price-history/{code}` | Bearer | BACKEND-REMOVED | `product-barcode/price-history/[barcode]/route.ts:6-30` |
| `/api/product-price-history/[[...path]]` | GET/POST | M `/product/barcode/price-history/*` | Bearer | BACKEND-REMOVED | `product-price-history/[[...historyPath]]/route.ts:14,24,35,47` |
...
| `/api/integration/gl/[...path]` | GET/POST | M `/integration/gl/v2/*` (API token ไม่ใช่ session) | API token | LIVE | `integration/gl/[...path]/route.ts:18,22`, `frontend/src/lib/integration-proxy.ts` |
| `/api/mcp-tokens/[[...path]]` | GET/POST | M `/mcp-tokens`, `/mcp-tokens/companies`, `/mcp-tokens/{id}/revoke` | Bearer (backend ตรวจ OWNER/ADMIN) | LIVE | `mcp-tokens/[[...tokenPath]]/route.ts:17-34` |
| `/mcp/gl` (นอก `api/`) | GET/POST/DELETE | M `/mcp/gl` (MCP token) | MCP token | LIVE | `frontend/src/app/mcp/gl/route.ts` |
...
| `system-setting-screens.ts` | config slug (slug/route/kind/basePath/ฟิลด์; `SystemSettingKind` เหลือ 6 แบบ: `company`, `goapi-crud`, `main-crud`, `permission-catalog`, `report`, `restaurant-setting`) = source ของ `/[systemSetting]` | `frontend/src/lib/system-setting-screens.ts:77-83,294,1114,1358-1372` |
| `menu-data.ts`, `menu-icons.ts`, `menu-usage.ts`, `user-shortcuts.ts` | โครงเมนู `MENU_SECTIONS` (transactions/reports …), icon ต่อ route, นับการใช้เมนู, ทางลัดผู้ใช้ใน storage; **เพิ่ม/ลบ 1 เมนู = แก้ 4 จุดพร้อมกันเสมอ** `menu-data.ts` → `backend/assets/language/languages.tsv` (13 คอลัมน์ ครบทุกภาษา) → `ROUTE_ICON_KEYS` ใน `menu-icons.ts` → จำนวนใน `menu-icons.test.ts` (2026-09-25: `toHaveLength(195)`, `frontend/src/lib/menu-icons.test.ts:10`) ไม่งั้น `frontend/src/lib/menu-data.test.ts` + `frontend/src/lib/menu-icons.test.ts` fail — ดู `docs/kms/19-menu-coverage-market-standard.md:64-65` และ `.agents/skills/ui-scale-polish/SKILL.md` §8 | `frontend/src/lib/menu-data.ts:169` (`MENU_SECTIONS`) |
...
- `components/system-settings/*` — field editors ของจอตั้งค่า (image upload, permission matrix/sets, role-screen-matrix, thailand-address, holding-scope, branch/company selectors, language) + `work-day-panel.tsx`
...
- ใน `src/app/`: `theme-toggle.tsx`, `font-picker.tsx`, `language-dialog.tsx`, `zoom-control.tsx`, `app-header-controls.tsx`, `motion-config.tsx`, `shared/motion-variants.ts`, `shared/skeleton-card.tsx`, `system-settings/*-tree-view.tsx` (4 ไฟล์), `menu/tab-product-*.tsx` (12 ไฟล์ — นับจาก `ls frontend/src/app/menu`)
...
|---|---|---|---|---|
| Unit (vitest, env node) | `frontend/vitest.config.ts:3-12` include `src/**/*.test.ts` | 107 ไฟล์ (นับ 2026-09-25; lib, api route, security tests เช่น `login-screen.security.test.ts`, `holding-screen.security.test.ts`, `main-menu-password.security.test.ts`, `menu/main-menu-sidebar-resize.test.ts`) | — | `frontend/src/app/login-screen.security.test.ts:7-21` |
| e2e ใน frontend | `frontend/playwright.config.ts` testDir `./e2e`, baseURL `E2E_BASE_URL` (default `localhost:3000`), ไม่ start server เอง | 10 spec (นับ 2026-09-25): `branch-thai-address-cascade`, `dev-login`, `gl-chart-of-accounts-error`, `gl-journal-details`, `gl-journal-review`, `gl-report-drill`, `mcp-tokens`, `menu-champ-upgrade`, `menu-consistency`, `menu-tree-master` — spec CRUD สินค้า/บาร์โค้ด/คลังเดิมถูกลบพร้อม backend MongoDB 2026-09-23 | spec ส่วนใหญ่คลิกปุ่ม "ทดลองใช้ระบบ (Demo)"; เฉพาะ `dev-login.spec.ts` ยังหาปุ่ม "เข้าทดสอบระบบ (Dev Login)" ที่ไม่มีใน `login-screen.tsx` แล้ว (ดูช่องว่าง) | `frontend/playwright.config.ts:6-17`, `frontend/e2e/dev-login.spec.ts:6-21` |
| UAT ที่ root | `playwright.config.ts` testDir `./tests`, baseURL `PW_BASE_URL` (default `127.0.0.1:3000`), project `setup` สร้าง `.auth/user.json` แล้ว chromium reuse | 12 spec + `auth.setup.ts` (นับ 2026-09-25) | คลิกปุ่ม /Dev Login/ → `/holding` เลือก "บ้านเชียง" → `/workspace` เลือกสาขา TST03 / 00001 (ปุ่ม Dev Login ไม่มีในหน้า login แล้ว — ดูช่องว่าง) | `playwright.config.ts:17,32,42-54`, `tests/auth.setup.ts:14-25` |
...
- **ชื่อ key ที่ผู้ช่วย AI เสนอต้องอ่านความหมายซ้ำ** (ตั้งแต่ 2026-09-23 ห้ามใช้ผู้ช่วย AI ภายนอกตาม AGENTS.md — บทเรียนนี้มาจากรอบ 2026-09-16) — "ยังไม่ผ่าน" (ยังไม่ผ่านรายการเข้า GL) ถูกตั้งเป็น `fa_not_yet_approved` + แปลว่า "Not Yet Approved / まだ承認されていません" ทั้ง 10 ภาษา ต้องแก้เป็น `fa_not_posted` (Not posted / 未転記) — คำว่า "ผ่าน/ผ่านรายการ" ในบริบทบัญชีคือ post ไม่ใช่ approve
...
- (ตรวจแล้ว 2026-09-25) เทียบ BFF กับ route ที่ backend ลงทะเบียนจริง (grep `.GET("/`/`.POST("/` ใน `backend/`): `/sessions/active-count`, `/holding-member/list`, `/organization/*`, `/gl/v2/*`, `/fa/v2/*`, `/mcp-tokens`, `/mcp/gl`, `/integration/gl/v2/*`, `/image/upload`, `/video/upload` มีอยู่; `/product*`, `/unit*`, `/currency*`, `/transaction/*`, `/warehouse`, `/debtaccount/*`, `/restaurant/settings*`, `/api/user/lineoa/*` ไม่มีแล้ว → แถวใน §5 ติด BACKEND-REMOVED
...
- ขัดกันจริงในโค้ด: `frontend/src/app/login-screen.tsx:317-323,657-662` มีเฉพาะปุ่ม "ทดลองใช้ระบบ (Demo)" (comment บอกว่าแทนปุ่ม Dev Login เดิม) และ `frontend/src/app/login-screen.security.test.ts:8-12` บังคับว่าห้ามมี `/api/auth/dev-login` ในหน้า login — แต่ `frontend/e2e/dev-login.spec.ts:8` และ `tests/auth.setup.ts:16` ยังหาปุ่ม "Dev Login" อยู่ → e2e/UAT ชุดนี้น่าจะไม่ผ่านตั้งแต่ขั้น login (**ยังไม่รันยืนยัน**); ถามลุงจืดว่าจะย้าย e2e ไป Demo หรือคืนปุ่ม Dev
- (ปิดแล้ว) skill อยู่ที่ `.agents/skills/ui-scale-polish/SKILL.md` (track ใน git; 1,528 บรรทัด ตรวจ 2026-09-25) ตามกฎ "กฎและ skill ของ AI ทุกตัวอยู่ที่เดียว" ใน `AGENTS.md` — Claude Code เข้าถึงผ่าน junction `.claude/skills` จาก `npm run ai:link`
...
- `system-settings-screen.tsx` 16,107 บรรทัด (2026-09-25)ในไฟล์เดียว — ไม่ได้อ่านทั้งไฟล์ (ใช้ grep) จึง**ยังไม่ตรวจ** พฤติกรรมรายจอทุก slug
- ไม่ได้รัน `npm test`/`typecheck`/playwright ในรอบนี้ (อ่านโค้ดอย่างเดียว) — สถานะผ่าน/ตกล่าสุด**ยังไม่ตรวจ**
- คำถาม: ไฟล์ `frontend/src/app/.dialog-header` ที่ยังถูก track ลบได้ไหม (R2 แต่รอคำสั่ง) — `.next-dev*.log` เลิก track แล้ว 2026-09-09 (§12)
