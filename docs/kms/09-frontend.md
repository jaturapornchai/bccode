...
| `menu-data.ts`, `menu-icons.ts`, `menu-usage.ts`, `user-shortcuts.ts` | โครงเมนู `MENU_SECTIONS` (transactions/reports …), icon ต่อ route, นับการใช้เมนู, ทางลัดผู้ใช้ใน storage; **เพิ่ม/ลบ 1 เมนู = แก้ 4 จุดพร้อมกันเสมอ** `menu-data.ts` → `backend/assets/language/languages.tsv` (13 คอลัมน์ ครบทุกภาษา) → `ROUTE_ICON_KEYS` ใน `menu-icons.ts` → จำนวนใน `menu-icons.test.ts` (ปัจจุบัน `toHaveLength(224)`) ไม่งั้น `frontend/src/lib/menu-data.test.ts` + `frontend/src/lib/menu-icons.test.ts` fail — ดู `docs/kms/19-menu-coverage-market-standard.md:61-62` และ `docs/skills/ui-scale-polish/SKILL.md` §8 | `frontend/src/lib/menu-data.ts:48-155` |
...
- **ขั้นตอนกวาดไทยฝังโค้ดทีละจอ** (ใช้กับ FA 2026-09-16, 109 จุด): `scratch/lit-scan.js` ดูภาพรวม → `fa-scan.js` แยกเป็น 3 ทรง (string literal / JSX text / template literal ที่มี `${}`) → `fa-plan.js` จับคู่แถวเดิมด้วยไทย **แล้วอ่าน en ของผู้สมัครทุกตัวด้วยตาก่อนตัดสิน** → `fa-batches.js` ให้ DeepSeek แปล 10 ภาษา *พร้อมตั้งชื่อ key จากอังกฤษของตัวเอง* (ได้ `fa_asset_code` แทน `fa_27`) → `fa-validate.js` → `fa-apply.js` แทนที่อัตโนมัติ → template literal แก้มือด้วย `{0}`. กับดักที่เจอจริง: (1) แก้จำนวนบรรทัดก่อน apply แล้วเลขบรรทัดที่สแกนไว้เลื่อนทั้งไฟล์ — สแกนใหม่ก่อน apply เสมอ (2) `placeholder="ไทย"` ใน JSX ต้องใส่ปีกกา `{tr(...)}` ไม่งั้น TS1145 (3) key ที่ DeepSeek ตั้งอาจชนกับแถวที่เพิ่ง append ไปในรอบเดียวกัน → uniquify ใหม่แล้ว rename ในโค้ดด้วย
