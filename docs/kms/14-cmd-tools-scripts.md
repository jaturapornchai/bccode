...
| `tools/install-hooks.mjs` | คัดลอก `.githooks/*` เข้า `.git/hooks/` (ไม่ใช้ `core.hooksPath` เพราะมันทับ hook เดิมทั้งโฟลเดอร์) — ต้องรันซ้ำทุกครั้งที่ `.githooks/` เปลี่ยน | `npm run hooks:install` | LIVE | `tools/install-hooks.mjs:1-11` |
