...
- ชุดสิทธิ์ (`role_permission.rolecode`) เป็นโปรไฟล์สำเร็จรูปเพื่อความสะดวก: `USER`/`ADMIN`/`OWNER` คือชุดมาตรฐานที่ทุก Membership ระดับนั้นได้อัตโนมัติ และตั้งชุดเพิ่มได้ไม่จำกัดด้วยรหัส A-Z 0-9 _ - ยาว 2-30 ตัว (เช่น `ACCOUNTING`, `SALES`) Membership เลือกชุดเพิ่มเติมได้หลายชุดใน `shopusers.permissionsets` (รหัสถูก normalize เป็นตัวพิมพ์ใหญ่ ไม่ซ้ำ) สิทธิ์จริงของ Membership = Union ของชุดมาตรฐานตาม Role กับทุกชุดที่เลือก (`/organization/role-permission/me` คืน `permissions` ที่รวมแล้วพร้อม `permissionsets`) ชุดใดมี `*` ผลรวมเป็น `*` และ ADMIN/OWNER ที่ยังไม่มีชุดมาตรฐานของตนยังได้ `*` เสมอไม่ว่าจะเลือกชุดเพิ่มหรือไม่ Union เป็นการเพิ่มสิทธิ์อย่างเดียว ไม่มีชุดแบบ "ห้าม" (ตั้งโดยลุงจืด 2026-09-02)
