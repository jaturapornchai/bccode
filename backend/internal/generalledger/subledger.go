...
func (m *subledgerMutation) apply(d *JournalDetails, reconcile bool) error {
	if len(d.Partners)+len(d.BankAccounts)+len(d.Documents)+len(d.Allocations)+len(d.Settlements)+len(d.BankLines)+len(d.StatementLines)+len(d.Matches)+len(d.Withdrawals)+len(d.Withholdings)+len(d.Vats) > 2000 {
