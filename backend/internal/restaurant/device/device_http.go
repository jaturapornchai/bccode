...
func NewDeviceHttp(ms *microservice.Microservice, cfg config.IConfig) DeviceHttp {
	...
	svc := services.NewDeviceHttpService(repo, masterSyncCacheRepo)
...
func (h DeviceHttp) RegisterHttp() {
	...
	h.ms.GET("/restaurant/device", h.SearchDevicePage)
	h.ms.GET("/restaurant/device/list", h.SearchDeviceLimit)
	h.ms.POST("/restaurant/device", h.CreateDevice)
	h.ms.GET("/restaurant/device/:id", h.InfoDevice)
	h.ms.PUT("/restaurant/device/:id", h.UpdateDevice)
	h.ms.DELETE("/restaurant/device/:id", h.DeleteDevice)
	h.ms.DELETE("/restaurant/device", h.DeleteDeviceByGUIDs)
}
...
func (h DeviceHttp) SearchDevicePage(ctx microservice.IContext) error {
	...
	docList, pagination, err := h.svc.SearchDevice(holdingCode, pageable)
...
func (h DeviceHttp) SearchDeviceLimit(ctx microservice.IContext) error {
	...
	docList, total, err := h.svc.SearchDeviceStep(holdingCode, lang, pageableStep)
