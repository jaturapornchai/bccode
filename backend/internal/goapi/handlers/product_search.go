...
func UnifiedProductSearchHandler(c echo.Context) error {
	...
	logger.Info("[UnifiedSearch] shopid=%s, keyword=%s, limit=%d, include_balance=%v",
		req.ShopID, req.Keyword, req.Limit, req.IncludeBalance)
...
// calculateBalancesForSearch - คำนวณยอดคงเหลือ (เวอร์ชันเดิม - deprecated)
func calculateBalancesForSearch(db *sql.DB, itemCodes []string, whcode, locationcode string) (map[string]float64, error) {
	if len(itemCodes) == 0 {
		return nil, nil
	}
	...
	// Build WHERE clause with optional whcode/locationcode filter
	whereExtra := ""
	if whcode != "" {
		whcode = strings.ReplaceAll(whcode, "'", "''")
		whereExtra += fmt.Sprintf(" AND whcode = '%s'", whcode)
	}
	if locationcode != "" {
		locationcode = strings.ReplaceAll(locationcode, "'", "''")
		whereExtra += fmt.Sprintf(" AND locationcode = '%s'", locationcode)
	}
	...
	`, strings.Join(quoted, ","), whereExtra)
	...
	for rows.Next() {
		...
		if err := rows.Scan(&itemCode, &balance); err != nil {
			continue
		}

		result[itemCode] = balance
	}
...
// formatPackingString - แปลงยอดคงเหลือเป็น format "5 โหล x 3 ชิ้น"
// Algorithm: Greedy - หารด้วยหน่วยใหญ่สุดก่อน
func formatPackingString(balance float64, units []SearchProductUnit) string {
	if balance <= 0 {
		return "0"
	}
	...
	if baseUnitName == "" {
		if len(units) > 0 {
			baseUnitName = units[len(units)-1].UnitName
			if baseUnitName == "" {
				baseUnitName = units[len(units)-1].UnitCode
			}
		}
		if baseUnitName == "" {
			baseUnitName = "หน่วย"
	...
	// กรองและเรียง packing units (ratio > 1, มากไปน้อย)
	type packingUnit struct {
		UnitName string
		Ratio    float64
	}
	var packingUnits []packingUnit
	for _, u := range units {
		ratio := u.UnitStand / u.UnitDivide
		if ratio > 1.0 {
			name := u.UnitName
			...
			packingUnits = append(packingUnits, packingUnit{
				UnitName: name,
				Ratio:    ratio,
			})
		}
	...
	sort.Slice(packingUnits, func(i, j int) bool {
		return packingUnits[i].Ratio > packingUnits[j].Ratio
	})

	// ถ้าไม่มี packing units ให้แสดงแบบปกติ
	if len(packingUnits) == 0 {
		return fmt.Sprintf("%.0f %s", balance, baseUnitName)
	}
	...
	for _, pu := range packingUnits {
		count := int(remaining / pu.Ratio)
		if count > 0 {
			displayName := pu.UnitName
			// Handle duplicate names
			if usedNames[displayName] || displayName == baseUnitName {
				displayName = "แพ็ค"
			}
			usedNames[displayName] = true

			parts = append(parts, fmt.Sprintf("%d %s", count, displayName))
			remaining -= float64(count) * pu.Ratio
		}
	}

	// Remainder
	if remaining > 0.001 {
		remainderUnitName := baseUnitName
		if usedNames[baseUnitName] {
			remainderUnitName = "อัน-ชิ้น"
		}

		// Format: ตัดทศนิยมถ้าเป็นจำนวนเต็ม
		if remaining == float64(int(remaining)) {
			parts = append(parts, fmt.Sprintf("%d %s", int(remaining), remainderUnitName))
		} else {
			parts = append(parts, fmt.Sprintf("%.2f %s", remaining, remainderUnitName))
		}
	}
