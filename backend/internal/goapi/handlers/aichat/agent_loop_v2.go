...
type SSEEvent struct {
	Type string `json:"type"` // "status", "thinking", "tool_start", "tool_done", "answer", "done", "error"
	Data any    `json:"data"`
}
...
- For external info (laws, recipes, market prices, news) → call ` + "`web_search`" + ` first
...
- Unsure how? Call ` + "`execute_js`" + ` and try multiple approaches in one script
...
(products/sales/customers) or by general world knowledge — call ` + "`query_knowledge_base`" + `
before answering.
...
If query_knowledge_base returns nothing relevant, say so plainly and do not invent
...
you MUST call ` + "`execute_js`" + ` (or another query tool) before writing the final answer:
...
you should ALSO call ` + "`web_search`" + ` in parallel with the database queries — even when shop
...
Skip web_search ONLY for purely internal-only questions: own sales totals, customer balances,
...
# PRIMARY TOOL: execute_python (preferred) / execute_js (fallback)
...
which means fewer retry iterations and faster answers. Only fall back to execute_js if a specific
...
# Other tools (when execute_js is overkill)
- query_postgresql / query_mongodb / query_clickhouse — direct queries
- aggregate_mongodb — pipeline
- list_mongodb_collections / list_clickhouse_tables
...
surface (e.g. pre-fetched KB + execute_js on PG), you MUST have at least that many sub-headings.
...
- The user wants Plan + Data + Analysis only. Follow-up questions belong in the separate ` + "`suggested_questions`" + ` field at the end (not in the answer body).
...
- **CITATION RULE (MANDATORY)**: Every fact from web_search or Knowledge Base MUST have a clickable Markdown link.
  - Web search: ` + "`[title](URL)`" + ` (URL comes from web_search result)
  - KB passage: ` + "`[doc_name](VIEW_URL)`" + ` (VIEW_URL comes from the passage metadata in KB context)
...
- **CITATION RULE (MANDATORY)**: Every fact from web_search or Knowledge Base MUST have a clickable source link.
  - Web search: ` + "`<a href=\"URL\" target=\"_blank\">title</a>`" + ` (URL comes from web_search result)
  - KB passage: ` + "`<a href=\"VIEW_URL\" target=\"_blank\">doc_name</a>`" + ` (VIEW_URL comes from the passage metadata in KB context)
...
func RunAgentLoopV2(ctx context.Context, req AgentV2Request, emitSSE func(SSEEvent)) (*AgentChatResponse, error) {
	...
	// เหตุผล: model บางตัว (devstral) ชอบวน execute_python เพิ่ม fallback แบบ append code
	...
	for i := 0; i < maxIterationsV2; i++ {
		...
		//
		// Phase A (sequential): validate + parse args + loop detection + emit tool_start
		// Phase B (parallel):   ยิง mcpServer.ExecuteToolDirect พร้อมกันทุก tool ด้วย goroutine
		// Phase C (sequential): append tool results ตามลำดับเดิม + emit tool_done
		//
		// เหตุผล: AI อาจเรียก query_pg + query_mongo ใน iteration เดียวกัน
		...
		if loopDetectedThisBatch {
			...
			for _, t := range tasks {
				...
				t.resolvedMsg = fmt.Sprintf(
					`{"error":"loop_detected","message":"You called tool '%s' with the same arguments %d times and got the same result. Change approach: try a different tool, or change collection/query/filter, or summarize from data already gathered. Then reply in Thai."}`,
					t.toolName, lastToolSigCount,
				)
			}
		}

		// Emit tool_start สำหรับทุก task ที่จะ execute จริง
		for _, t := range tasks {
			...
			if t.loopDetected {
				emitSSE(SSEEvent{Type: "tool_done", Data: map[string]any{
					"tool": t.toolName, "duration_ms": 0, "success": false, "loop": true,
				}})
				continue
			}
		...
		// === Phase C: collect results ตามลำดับเดิม + append messages + emit tool_done ===
		for _, t := range tasks {
			if t.preResolved {
				...
				// loop-detected case ไม่ต้อง emit tool_done ซ้ำ — emit ไปแล้วใน prep phase
				continue
			}
			...
			// emit tool_done พร้อมข้อมูลครบ — frontend เอาไป render real-time ได้
