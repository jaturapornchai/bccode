...
func OnConsumeMessageStockReceiveProductDelete(msg string) error {
	...
	if docData.HoldingCode == "" || docData.DocNo == "" {
		logger.Error("OnConsumeMessageStockReceiveProductDelete: Invalid data - missing HoldingCode or DocNo")
...
func ProcessStockReceiveProductDocument(docData models.StockReceiveProductStruct) error {
	...
	logger.Info("ProcessStockReceiveProductDocument: Step 2 - Connecting to PostgreSQL for holdingCode=%s", docData.HoldingCode)
...
func MapStockReceiveProductToDocDetailStructs(processData models.ProcessMongoTransModel, holdingCode string) []models.DocDetailStruct {
