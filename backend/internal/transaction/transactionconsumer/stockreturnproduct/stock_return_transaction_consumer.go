...
import (
	...
	stockreturnproductconfig "smlcloudplatform/internal/transaction/stockreturnproduct/config"
...
type StockReturnTransactionConsumer struct {
	ms                   *microservice.Microservice
	cfg                  pkgConfig.IConfig
	svc                  IStockReturnProductConsumerService
	transactionPhaser    usecases.ITransactionPhaser[models.StockReturnProductTransactionPG]
	stockPhaser          usecases.IStockTransactionPhaser[models.StockReturnProductTransactionPG]
	stockConsumerService stocktransaction.IStockTransactionConsumerService
}

func InitStockReturnTransactionConsumer(ms *microservice.Microservice, cfg pkgConfig.IConfig) services.ITransactionDocConsumer {
	persister := ms.Persister(cfg.PersisterConfig())
	producer := ms.Producer(cfg.MQConfig())

	repo := NewStockReturnTransactionPGRepository(persister)
	stockReceiveProductConsumerService := NewStockReturnProductConsumerService(repo)
	stockService := stocktransaction.NewStockTransactionConsumerService(persister, producer)

	stockReceiveTransactionConsumer := NewStockReturnTransactionConsumer(
		ms,
		cfg,
...
func NewStockReturnTransactionConsumer(
	ms *microservice.Microservice,
	cfg pkgConfig.IConfig,
	svc IStockReturnProductConsumerService,
	stockConsumerService stocktransaction.IStockTransactionConsumerService,
	...
	stockReceiveTransactionPhaser := StockReturnTransactionPhaser{}
	stockReceiveStockPhaser := StockReturnStockPhaser{}

	return &StockReturnTransactionConsumer{
		ms:                   ms,
		cfg:                  cfg,
		svc:                  svc,
...
func (c *StockReturnTransactionConsumer) ConsumeOnCreateOrUpdate(ctx microservice.IContext) error {
...
func (c *StockReturnTransactionConsumer) ConsumeOnBulkCreateOrUpdate(ctx microservice.IContext) error {
...
func (c *StockReturnTransactionConsumer) ConsumeOnDelete(ctx microservice.IContext) error {
...
func (c *StockReturnTransactionConsumer) ConsumeOnBulkDelete(ctx microservice.IContext) error {
...
func (c *StockReturnTransactionConsumer) RegisterConsumer(ms *microservice.Microservice) {
	...
	stockProductReceiveKafkaConfig := stockreturnproductconfig.StockReturnProductMessageQueueConfig{}
...
func MigrationDatabase(ms *microservice.Microservice, cfg pkgConfig.IConfig) error {
	...
	pst.AutoMigrate(
		models.StockReturnProductTransactionPG{},
		models.StockReturnProductTransactionDetailPG{},
	)
	return nil
}
