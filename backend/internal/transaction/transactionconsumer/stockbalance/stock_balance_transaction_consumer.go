...
import (
	...
	stockbalanceConfig "smlcloudplatform/internal/transaction/stockbalance/config"
...
func InitStockReceiveTransactionConsumer(ms *microservice.Microservice, cfg pkgConfig.IConfig) services.ITransactionDocConsumer {
	...
	stockBalanceDetailMongoRepository := repositories.NewStockBalanceDetailRepository(mongoPersister)

	StockBalanceConsumerService := NewStockReceiveTransactionConsumerService(repo, stockBalanceDetailMongoRepository)
