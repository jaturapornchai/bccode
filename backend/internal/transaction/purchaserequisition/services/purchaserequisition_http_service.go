...
type PurchaseRequisitionHttpService struct {
	repoMq           repositories.IPurchaseRequisitionMessageQueueRepository
	repo             repositories.IPurchaseRequisitionRepository
	repoCache        trancache.ICacheRepository
	cacheExpireDocNo time.Duration
	syncCacheRepo    mastersync.IMasterSyncCacheRepository
	services.ActivityService[models.PurchaseRequisitionActivity, models.PurchaseRequisitionDeleteActivity]
	contextTimeout time.Duration
}

func NewPurchaseRequisitionHttpService(
	repo repositories.IPurchaseRequisitionRepository,
	repoCache trancache.ICacheRepository,
	repoMq repositories.IPurchaseRequisitionMessageQueueRepository,
	syncCacheRepo mastersync.IMasterSyncCacheRepository,
) *PurchaseRequisitionHttpService {
	contextTimeout := time.Duration(15) * time.Second
	insSvc := &PurchaseRequisitionHttpService{
		repo:             repo,
		repoMq:           repoMq,
		repoCache:        repoCache,
	...
	insSvc.ActivityService = services.NewActivityService[models.PurchaseRequisitionActivity, models.PurchaseRequisitionDeleteActivity](repo)
	return insSvc
}

func (svc PurchaseRequisitionHttpService) getContextTimeout() (context.Context, context.CancelFunc) {
	return context.WithTimeout(context.Background(), svc.contextTimeout)
}

func sanitizeExchangeRate(doc *models.PurchaseRequisition) {
...
func (svc PurchaseRequisitionHttpService) getDocNoPrefix(docDateLocalStr string) string {
...
func (svc PurchaseRequisitionHttpService) CreatePurchaseRequisition(shopID string, authUsername string, doc models.PurchaseRequisition) (string, string, *validators.ValidationResult, error) {
	...
	docData.PurchaseRequisition.CreatorCode = authUsername
	if docData.PurchaseRequisition.CreatorName == "" {
		docData.PurchaseRequisition.CreatorName = authUsername
	}
	docData.PurchaseRequisition.CreatedAt = time.Now()

	// Default urgency
	if docData.PurchaseRequisition.Urgency == 0 {
		docData.PurchaseRequisition.Urgency = 1
	}
...
func (svc PurchaseRequisitionHttpService) UpdatePurchaseRequisition(shopID string, guid string, authUsername string, doc models.PurchaseRequisition) (*validators.ValidationResult, error) {
	...
	originalCreatorCode := findDoc.PurchaseRequisition.CreatorCode
	originalCreatorName := findDoc.PurchaseRequisition.CreatorName
	originalCreatedAt := findDoc.PurchaseRequisition.CreatedAt
	...
	docData.PurchaseRequisition.CreatorCode = originalCreatorCode
	docData.PurchaseRequisition.CreatorName = originalCreatorName
	docData.PurchaseRequisition.CreatedAt = originalCreatedAt
	docData.PurchaseRequisition.UpdaterCode = authUsername
	docData.PurchaseRequisition.UpdaterName = authUsername
	docData.PurchaseRequisition.UpdatedAt = time.Now()
...
func (svc PurchaseRequisitionHttpService) InfoPurchaseRequisition(shopID string, guid string) (models.PurchaseRequisitionInfo, error) {
	...
	if len(findDoc.GuidFixed) < 1 {
		return models.PurchaseRequisitionInfo{}, errors.New("document not found")
	}
...
func (svc PurchaseRequisitionHttpService) SaveInBatch(shopID string, authUsername string, dataList []models.PurchaseRequisition) (common.BulkImport, error) {
	...
	payloadList, payloadDuplicateList := importdata.FilterDuplicate[models.PurchaseRequisition](dataList, svc.getDocIDKey)
...
func (svc PurchaseRequisitionHttpService) getDocIDKey(doc models.PurchaseRequisition) string {
	return doc.DocNo
}
...
func (svc PurchaseRequisitionHttpService) GetModuleName() string {
