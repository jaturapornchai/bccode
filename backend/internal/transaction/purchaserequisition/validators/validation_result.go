...
func (r *ValidationResult) ToErrorResponse(lang string) ValidationErrorResponse {
	summary := BilingualMessage{
		TH: "ข้อมูลใบขอซื้อไม่ถูกต้อง กรุณาตรวจสอบและแก้ไข",
		EN: "Purchase requisition validation failed. Please review and correct the errors.",
	}
...
func (r *ValidationResult) ToUpdateErrorResponse(lang string) ValidationErrorResponse {
	...
	resp.ErrorCode = "PR_UPDATE_VALIDATION_FAILED"
	updateSummary := BilingualMessage{
		TH: "ไม่สามารถแก้ไขใบขอซื้อได้ กรุณาตรวจสอบและแก้ไข",
		EN: "Cannot update purchase requisition. Please review and correct the errors.",
	}
	resp.Message = updateSummary.Get(lang)
	return resp
}

type PRSuccessResponse struct {
	Success bool   `json:"success"`
...
func NewCreateSuccessResponse(id, docNo, lang string) PRSuccessResponse {
	msg := BilingualMessage{
		TH: "บันทึกใบขอซื้อสำเร็จ",
		EN: "Purchase requisition saved successfully.",
	}
	return PRSuccessResponse{
		Success: true,
		ID:      id,
		DocNo:   docNo,
...
func NewUpdateSuccessResponse(id, lang string) PRSuccessResponse {
	msg := BilingualMessage{
		TH: "แก้ไขใบขอซื้อสำเร็จ",
		EN: "Purchase requisition updated successfully.",
	}
	return PRSuccessResponse{
		Success: true,
		ID:      id,
