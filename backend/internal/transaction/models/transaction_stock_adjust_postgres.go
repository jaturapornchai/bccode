...
type StockAdjustmentTransactionPG struct {
	...
	Items         *[]StockAdjustmentTransactionDetailPG `json:"items" gorm:"items;foreignKey:holdingcode,docno"`
}
...
func (j *StockAdjustmentTransactionPG) BeforeUpdate(tx *gorm.DB) (err error) {
	...
	tx.Model(&StockAdjustmentTransactionDetailPG{}).Where(" holdingcode=? AND docno=?", j.HoldingCode, j.DocNo).Find(&details)
