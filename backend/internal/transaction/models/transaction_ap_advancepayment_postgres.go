...
type APAdvancePaymentTransactionPG struct {
	...
	Items                *[]APAdvancePaymentTransactionDetailPG `json:"items" gorm:"items;foreignKey:shopid,docno"`
}
...
func (m *APAdvancePaymentTransactionPG) BeforeUpdate(tx *gorm.DB) (err error) {
	...
	tx.Model(&APAdvancePaymentTransactionDetailPG{}).Where(" shopid=? AND docno=?", m.ShopID, m.DocNo).Find(&details)
