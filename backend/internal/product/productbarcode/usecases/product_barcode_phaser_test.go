...
func TestPhaseProductBarcodeUsesOwnUnitSnapshot(t *testing.T) {
	...
	doc.DivideValue = 1
	doc.StandValue = 12
	refBarcodes := []models.RefProductBarcode{{
		Barcode:     "8850000000099",
		DivideValue: 1,
		StandValue:  99,
	}}
	doc.RefBarcodes = &refBarcodes

	got, err := (usecases.ProductBarcodePhaser{}).PhaseProductBarcodeDoc(&doc)
	assert.NoError(t, err)
	assert.Equal(t, int64(1), got.DivideValue)
	assert.Equal(t, int64(12), got.StandValue)
}
...
func TestProductBarcodePhaser(t *testing.T) {
	...
	"guidfixed": "335YJBgJXUok8crFh7u1t8E4mME",
	"itemcode": "ITEM99",
	"barcode": "955004",
	"groupguid": "",
	"groupcode": "",
	...
	"businesstypes": [
		{
			"guidfixed": "30LbS14GyApu6OX70r23D2suc35",
			"code": "003",
			"names": [
				{
					"code": "th",
	...
	t.Run("Assert All ProductBarcodePg Fields", func(t *testing.T) {
		...
		assert.Equal(t, int64(1), got.StandValue, "StandValue should be 1 (default)")
		assert.Equal(t, int64(1), got.DivideValue, "DivideValue should be 1 (default)")
