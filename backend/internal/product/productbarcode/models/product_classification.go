...
func ValidateProductClassification(itemType int8, materialType int8) error {
	if !IsValidItemType(itemType) {
		return fmt.Errorf("invalid item_type %d: allowed values are 0=Stock, 1=Service, 2=Set, 3=Not Stock", itemType)
	}
	...
	if itemType == ItemTypeSet && materialType != MaterialTypeSet {
		return fmt.Errorf("invalid materialtype %d for item_type 2 (Set): expected materialtype 3 (Set)", materialType)
	}
	return nil
}
