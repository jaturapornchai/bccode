...
func (p *Producer) getProducer() *kafka.Producer {
	if p.prod == nil {
		prod, _ := p.newKafkaProducer(p.servers, p.protocol, p.sslca, p.sslkey, p.sslcert)
		p.prod = prod
	}
	return p.prod
}
...
func (p *Producer) TestConnect() error {
	if p.prod == nil {
		prod, err := p.newKafkaProducer(p.servers, p.protocol, p.sslca, p.sslkey, p.sslcert)
		if err != nil {
			return err
		}
		p.prod = prod
...
func (p *Producer) SendMessage(topic string, key string, message interface{}) error {
	...
	prod := p.getProducer()
	if prod == nil {
		p.logger.Error("[KAFKA] Producer is nil - not connected")
	...
	if err != nil {
		p.logger.Error("[KAFKA] Failed to produce message to topic %s: %v", topic, err)
		return err
	}
	...
	if m.TopicPartition.Error != nil {
		p.logger.Error("[KAFKA] Delivery failed for topic %s: %v", topic, m.TopicPartition.Error)
		return fmt.Errorf("delivery failed: %v", m.TopicPartition.Error)
	}

	p.logger.Debug("[KAFKA] Message delivered successfully to topic %s, partition %d, offset %d",
...
func (p *Producer) newKafkaProducer(servers string, protocal string, sslca string, sslkey string, sslcert string) (*kafka.Producer, error) {
	...
	if sslca != "" {
		...
		configMap.SetKey("ssl.ca.location", sslca)
		configMap.SetKey("ssl.key.location", sslkey)
		configMap.SetKey("ssl.certificate.location", sslcert)
	}
