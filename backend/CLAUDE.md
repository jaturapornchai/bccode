...
- Do not remove or rename existing `holdingcode` / `holdingcode` fields until all callers, migrations, indexes, tests, reports, Kafka consumers, and object paths are verified and there is a real functional benefit. **DEV exception:** under the *No-Migration / Disposable DEV Database Rule* (`.agents/rules/bc-account-core-rules.md`), DEV database data is disposable — in DEV you may change/remove/rename fields and collections freely and let the DB be recreated; this verification ritual applies to PRODUCTION only.
