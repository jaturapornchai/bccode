...
- Forward: backfill `shopUsers.user_uid` from `shopUsers.username -> users.username -> users.uid`, then create index `ix_shopUsers_holdingcode_user_uid`.
- Rollback index only:
  ```js
  db.shopUsers.dropIndex("ix_shopUsers_holdingcode_user_uid")
  ```
- If an older deployment expects the legacy index name, recreate the same keys with the old name:
  ```js
  db.shopUsers.createIndex(
    { holdingcode: 1, user_uid: 1 },
    { name: "ix_shop_users_holdingcode_user_uid" }
  )
  ```
