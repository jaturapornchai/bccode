-- ============================================================================
-- Realistic Thai SME Accounting Data Clean-up (PostgreSQL)
-- Wrapped in a strict transaction (BEGIN ... COMMIT) for ACID data integrity.
-- Removes all "ข้อมูลตัวอย่าง", "demo", "สมมติ" keywords and replaces with
-- professional Thai SME accounting names and realistic transaction narrations.
-- ============================================================================

BEGIN;

-- 1. Holdings: Real Business Names
UPDATE holdings 
...
-- 3. Users: Professional Display Names
UPDATE users 
...
-- 4. gl_records: Accounts (Remove "ข้อมูลตัวอย่าง - ")
UPDATE gl_records 
...
-- Specific clean-up for BM69-110102 (Fix accidental 'x')
UPDATE gl_records 
SET payload = jsonb_set(
...
-- 7. gl_records: Journals Narrations & Descriptions
UPDATE gl_records 
SET payload = jsonb_set(
  payload, 
  '{description}', 
...
-- 8. gl_lines: Clean account_name and description
UPDATE gl_lines 
...
-- 9. chartofaccounts: Clean legacy table
UPDATE chartofaccounts 
SET accountname = replace(accountname, 'ข้อมูลตัวอย่าง - ', '') 
WHERE accountname LIKE 'ข้อมูลตัวอย่าง - %';

UPDATE chartofaccounts 
SET accountname = 'เงินฝากกระแสรายวัน - ธ.กสิกรไทย' 
WHERE accountcode = 'BM69-110102';
