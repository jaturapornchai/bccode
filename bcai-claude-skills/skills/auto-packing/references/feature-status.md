...
## Legend
- **done** = เสร็จแล้ว ใช้งานได้
- **backend-done** = backend พร้อม, frontend ยังไม่ได้ทำ
- **in-progress** = กำลังทำ
...
| 5 | Incremental stock calculation | **done** | checksum-based skip unchanged items |
...
| 11 | Print tracking (isPrint, printAt, printBy) | **done** | `updateprint/:docno` |
| 12 | Confirm tracking (isConfirm, confirmAt, confirmBy) | **done** | `confirmpickandpack/:docno` |
| 13 | Cancel support | **done** | `cancel/:docno` + `cancel-by-saleinvoice/:id` |
| 14 | Close job | **done** | `closejob/:id` |
| 15 | Available sale invoices (not yet packed) | **done** | `available-saleinvoice` endpoint |
| 16 | Group by warehouse | **done** | `by-warehouse` endpoint |
| 17 | Packing progress per SI | **done** | `saleinvoice-status` endpoint |
| 18 | Dashboard (overall stats) | **done** | `dashboard` endpoint |
...
| 24 | Pick & Pack list screen (bclms) | **pending** | Route exists, placeholder page |
...
| 32 | BOM tree graph view (bcaiaccount) | **done** | `product_bom_widget.dart` + GraphView |
| 33 | BOM unit conversion (standvalue/dividevalue) | **done** | Multi-unit support |
| 34 | Conditional BOM | **done** | `condition` flag |
| 35 | Sale Invoice BOM pricing | **done** | `transactionSaleinvoiceBOMPrices` |
| 36 | Combo/Gift set via BOM | **done** | Use BOM structure for bundling |
...
| 45 | Shipping label template | **planned** | Need label design + print API |
...
| # | Feature | Status | Notes |
|---|---------|--------|-------|
| 49 | Box/carton master data | **planned** | Need dimension fields |
...
| Category | Done | Backend-Done | In-Progress | Pending | Planned | Future |
|----------|------|-------------|-------------|---------|---------|--------|
| Auto Packing | 5 | 0 | 0 | 2 | 0 | 0 |
| Pick & Pack | 16 | 0 | 0 | 4 | 0 | 0 |
| BOM | 10 | 0 | 0 | 0 | 0 | 0 |
| Warehouse | 3 | 3 | 0 | 0 | 0 | 0 |
| Packing List | 0 | 0 | 0 | 1 | 2 | 1 |
| Bin Packing | 0 | 0 | 0 | 0 | 1 | 3 |
| **Total** | **34** | **3** | **0** | **7** | **3** | **4** |
