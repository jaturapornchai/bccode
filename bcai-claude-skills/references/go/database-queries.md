# Database Queries — SQL Patterns และ Common Queries

## PostgreSQL Connection

```go
// Per-shop connection (ใช้บ่อยสุด)
db, err := mypg.PgSqlFastConnect(shopID) // ชื่อ DB = shopID
defer db.Close()

// ห้ามใช้ string concat — ใช้ parameterized เสมอ
// db.Query(`SELECT * FROM product WHERE shopid=$1`, shopID)
// ห้าม: db.Query(`SELECT * FROM product WHERE shopid='` + shopID + `'`)
```
...
## ClickHouse Queries (Analytics เท่านั้น)
...
// สำหรับ field ที่อาจเป็น NULL
var nullableField sql.NullString
rows.Scan(&nullableField)
value := nullableField.String // ถ้า null จะได้ ""

var nullableFloat sql.NullFloat64
rows.Scan(&nullableFloat)
amount := nullableFloat.Float64 // ถ้า null จะได้ 0
```
