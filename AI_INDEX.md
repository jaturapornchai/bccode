...
- GUID/business-key contract: create immutable `guidfixed` exactly once for every MongoDB record, but never use GUID for business relations or cross-store joins. Use normalized `code`/`itemcode`/`unitcode`/document/composite keys. Product `code` is unique per Holding; barcode text may repeat but active `(holdingcode + itemcode + barcode)` is unique.
