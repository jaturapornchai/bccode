...
- **Row Identity/Highlight Key**: Use immutable `guid_fixed` / `guid` for React keys, selection state, editing state, and delete/update identity when present. Use a composite business-code fallback only for legacy rows that truly do not have an immutable GUID, and migrate those screens toward GUID identity.
...
- **Holding Access Scope Fields**: User, screen permission, permission group, user permission assignment, and approval CRUD screens are Holding-owned. Use explicit `access_scopes[]`, `scope_rules[]`, or `approval_rules[]` with `scope_type`, `business_code`, `branch_code`, and `all_branches` for company/branch applicability. Render pickers and checkboxes; do not expose raw JSON scope editors.
- **Company Access Fields**: General master-data `business_codes` and equivalent company access editors are company-level only. Do not render branch trees or save branch selections in these fields; branch-specific permissions belong to the Holding access scope model above or explicit branch-owned screens. Read legacy `company_guids` only as a transition alias.
...
- **Product Category Group Screens**: For `product_category_group_select_screen` and `productcategorylist`, `group_number` means a usage/device/channel group, not a menu category. The UI must make the selected group understandable (e.g. ordering device, cashier/POS, kitchen/KDS, delivery) and manage a complete tree with `parent_guid` / `parentguidall`; `codelist` belongs to sellable leaf categories or explicitly sellable nodes.
