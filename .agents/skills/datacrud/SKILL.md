...
- Every CRUD datalist stays in one shared CRUD/workbench system and chooses an explicit read provider per screen: `mongodb-direct` for list/detail reads from MongoDB-backed operational APIs, or `pgsql-read-mongo-crud` for list reads from PostgreSQL projections while every create/edit/delete/save still uses MongoDB-backed CRUD. Do not infer the mode when Jead has not specified it; inspect config/source first and ask or mark unconfirmed when unclear. `productunit` uses `mongodb-direct`.
- The right-side read-only detail panel and edit form must hydrate from the MongoDB-backed detail API only. Left list rows are for selection/highlight/search only and must not be used as right-pane truth, especially when the list mode is `pgsql-read-mongo-crud`. If MongoDB detail fetch fails or the immutable id is blank, show a clear error instead of keeping stale/fallback data.
...
- **Compact Read-Only Details**: In fill-height right panes, grid content must use `content-start items-start` so short summary cards keep their natural height instead of stretching into empty space. For paired summary/detail sections, use pane-width-aware `auto-fit/minmax` columns that collapse to one column when the splitter or viewport makes the pane narrow.
- **Actions**: Save/cancel buttons must stay visible/reachable. In a right-side inline add/edit pane, place the primary Save/Cancel actions in the fixed top header/action bar and never make a bottom-only footer the sole way to finish the form; users must not scroll to the end just to save. A pinned footer remains acceptable for dialogs/wizards when it is always visible and also owns step navigation.
...
- **Company-Owned CRUD Context**: Operational master screens use one server-validated active `businesscode` from workspace context. Do not render/save `businesscodes[]` or `companyguids[]` availability fields. Missing company context blocks CRUD visibly. Company/branch pickers remain only for Holding permission scopes, explicit branch-owned workflows, and read-only Holding aggregation filters.
