...
- **Company Access Fields**: `business_codes` and equivalent company access editors are company-level only. Do not render branch trees or save branch selections in these fields; branch access is inherited from the selected company/workspace context unless the screen is explicitly branch-owned. Read legacy `company_guids` only as a transition alias.
