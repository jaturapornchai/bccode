...
- **Product Option Set Templates**: Product add/edit screens should expose a clear "use template" control for repeated option structures stored in `product_variant_matrix`. Preserve the template's user-defined option order (for example color > size or size > color) and let users adjust product-specific choices after applying it.
