...
- **Holding Access Scope UI**: User, screen permission, permission group, user permission assignment, and approval screens are Holding-level screens. Show the active Holding in the header, then use the shared Holding scope widget for applicability: a whole-Holding checkbox, searchable company add-to-list, selected-company branch panel, branch search add-to-list, and per-company `all_branches` checkbox. The widget stores `scope_type`, `businesscode`, `branchcode`, and `all_branches`. Do not render full company/branch checkbox trees and do not expose raw JSON scope editing to normal users.
