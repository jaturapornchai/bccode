...
- **Add Copy Buttons**: Editable CRUD/list screens should show `เพิ่ม (Copy)` beside `เพิ่ม` when a selected row can be duplicated. Style both buttons with the same primary treatment, open create mode instead of edit mode, and prefill the form from the selected row as completely as possible.
