...
- **Product Option Set Templates**: `product_variant_matrix` create/edit screens and product add/edit screens should expose a clear `แม่แบบ` control for repeated option structures stored in `product_variant_matrix`. Preserve the template's user-defined option order (for example color > size or size > color), load `sku_combinations` as selectable SKU-level templates, and let users adjust product-specific choices after applying one.
