...
- **Tenancy Scope**: Authorize `holdingcode` plus normalized `businesscode` from JWT/session/standard workspace context for every company-owned operational route; verify the company and access scope server-side, never trust a mutation-body company value, and require `branchcode` for branch-owned data. Holding access routes may remain holding-scoped; Holding aggregate routes are read-only and preserve `businesscode`.
