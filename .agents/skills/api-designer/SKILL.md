...
- **Scope Parameters**: Holding-owned access/permission APIs use validated `holdingcode`. Company-owned operational APIs require validated `holdingcode + businesscode` from route/context header/JWT, never a mutation body; branch-owned data also requires `branchcode`. Prefer `/api/v1/holdings/:holdingcode/companies/:businesscode/resources` for new company resources. Holding aggregate APIs are read-only and preserve `businesscode` on every row/group.
