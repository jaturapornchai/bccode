...
- **Product Variant Copy Workflow**: Repeated option patterns such as color > size, size > color, storage > color, flavor > pack, or package > network should be duplicated through `คัดลอก` from an existing `product_variant_matrix` row when possible. Copy opens create mode and carries option tiers, SKU rows, media, specs, import mappings, and marketplace-related setup from the selected row so the user only adjusts what differs.
