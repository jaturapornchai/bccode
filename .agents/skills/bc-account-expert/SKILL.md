...
- **Product Variant Templates**: Repeated option patterns such as color > size, size > color, storage > color, flavor > pack, or package > network should be modeled as reusable `product_variant_matrix` templates. The template owns the option order and `sku_combinations` can be selected as SKU-level templates; each product can apply the template and then customize choices, SKU/barcode details, prices, media, and marketplace mappings.
