...
- **อ้างอิง**: `frontend/src/app/tax/tax-filing-workbench.tsx` (`money()`, `totals`), `frontend/src/lib/thai-tax.ts` (`toMoney`, `fetchWhtReport`), `backend/internal/goapi/handlers/tax_money.go` (`moneySQL`, `moneyText`, `thaiBahtText`, `loadCompanyHeader`)
