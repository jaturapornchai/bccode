...
- **New Standard Pattern:**
  - **กฎเหล็กการสร้างตัวอย่างผังบัญชี (Seed / Sample / Initial Data Rule):** หากมีการสร้างตัวอย่างผังบัญชีในระบบ ไม่ว่าจะอยู่ใน SQL seed, JSON fixtures, หรือโค้ดตั้งค่าเริ่มต้น **ต้องมีหลายระดับ (Multi-level Hierarchy) แบบมาตรฐานวิชาชีพบัญชีประเทศไทย (TFRS / DBD) เสมอ ห้ามแบนราบเป็นระดับ 1 อย่างเดียวเด็ดขาด**:
    * **Level 1 (หมวดบัญชีหลัก)**: เช่น `10000` สินทรัพย์, `20000` หนี้สิน, `30000` ส่วนของเจ้าของ, `40000` รายได้, `50000` ค่าใช้จ่าย (`allowposting: false`, `"level": 1`)
    ...
    * **Level 4 / 5 (บัญชีย่อยลงรายการ / Posting Accounts)**: เช่น `11110` เงินสดในมือ, `11120` เงินฝากกระแสรายวัน, `11130` เงินฝากออมทรัพย์, `11210` ลูกหนี้การค้า, `11310` สินค้าคงเหลือ, `11410` ภาษีซื้อ, `12110` อาคารและอุปกรณ์, `12120` ค่าเสื่อมราคาสะสม-อาคารและอุปกรณ์ (`allowposting: true`, `"level": 4`)
    * **ต้องระบุฟิลด์ `"level"` และ `"parentaccountcode"` ใน JSON Payload เสมอ**
...
- **Anti-pattern / Deprecated:**
  - ห้ามสร้างตัวอย่างผังบัญชีแบบ Flat (ระดับ 1 ระดับเดียว) โดยไม่มีบัญชีคุมและบัญชีย่อย
  - ห้ามลืมใส่ฟิลด์ `"level"` ใน Payload JSON ของผังบัญชี
...
## 8.29 จอรายงานภาษี/การเงินแสดงยอดจาก backend เท่านั้น — string ทศนิยม + `formatAmount` (2026-09-23)

- **แบบแผน**: ยอดเงินทุกช่อง (แถว, ยอดรวมท้ายตาราง, KPI, แบบ ภ.พ.30, ภ.ง.ด., 50 ทวิ) มาจาก backend เป็น string ทศนิยม 2 ตำแหน่ง แสดงด้วย `formatAmount(value, 2)` จาก `frontend/src/lib/general-ledger.ts`; ยอดรวมท้ายตารางใช้ `summary` ทั้งงวดที่ backend รวมด้วย decimal (ไม่ใช่ผลบวกของแถวที่โหลดมา) และติดป้าย "รวมทั้งงวด (N รายการ)" ด้วยจำนวนจาก backend; ข้อมูลผู้ประกอบการมาจาก `company` ที่ backend ส่ง ถ้าไม่มีให้แสดง "ยังไม่ระบุ"/"ยังไม่ระบุในทะเบียนบริษัท"; ช่องกรอกยอดเงินใช้ `type="text" inputMode="decimal"` + regex จำกัด 2 ตำแหน่ง แล้วส่ง string ให้ backend คำนวณ
- **ห้ามทำซ้ำ**: `Number(x).toLocaleString(...)`, `reduce((a, r) => a + r.amount)` บน browser, สูตรภาษี/ตัวอักษรบาทฝั่ง frontend, ค่า default ปลอม (เลขผู้เสียภาษี/ชื่อบริษัท/ที่อยู่สมมติ), `<input type="number">` กับเงิน, ปุ่ม "ส่งเข้าระบบ" ที่แค่เปลี่ยน state โดยไม่เรียก API
- **เหตุผล**: float ทำให้ยอดเพี้ยนระดับสตางค์ (กฎ Accounting Number) และยอดรวมจากแถวที่โหลดหน้าเดียวผิดเมื่อข้อมูลเกิน limit; ค่า default ปลอมพิมพ์ลงแบบยื่นภาษีจริงได้ = ความเสี่ยงทางกฎหมาย; กฎ Backend-First ห้ามคำนวณบัญชีบน browser
- **วิธีตรวจ**: `grep -n "toLocaleString\|parseFloat\|Number(" <ไฟล์จอ>` ต้องไม่เจอกับยอดเงิน; unit test ของ fetcher ต้องมีเคส JSON number/null → แสดง `0.00` (ไม่เดา); เปิดจอจริงดูว่ายอดท้ายตาราง = `summary` จาก network response
- **อ้างอิง**: `frontend/src/app/tax/tax-filing-workbench.tsx` (`money()`, `totals`), `frontend/src/lib/thai-tax.ts` (`toMoney`, `fetchWhtReport`), `backend/internal/goapi/handlers/tax_money.go` (`moneySQL`, `moneyText`, `loadCompanyHeader`), `backend/internal/whtcert/bahttext.go` (`BahtText`)

## 8.30 แบบฟอร์มราชการ = backend สร้าง PDF, จอแค่แสดง + ทางลัด/เมนูต้องใช้ได้กับผู้ใช้ GL อย่างเดียว (2026-09-23)

- **แบบแผน**: เอกสารที่ต้องตรงแบบราชการ (50 ทวิ) → backend คืน `application/pdf` (`POST /api/report/tax/wht/certificate`), BFF ส่ง byte ผ่าน (`PDF_PATHS` + `proxyPdf` ใน `frontend/src/app/api/goapi/[...goPath]/route.ts`), จอสร้าง object URL แล้วแสดงใน `<iframe>` ครึ่งขวา (ฟอร์มซ้าย/ตัวอย่างขวา) ให้พิมพ์/ดาวน์โหลดจาก viewer ของเบราว์เซอร์; ยอดเงินในฟอร์มเป็น read-only จากแถวรายงาน; error จาก backend เป็น language key (`wht_cert_*`) → แสดงด้วย `tr(code, message)` ตามภาษาบนจอ. ทางลัดเริ่มต้น (ทั้ง "ใช้บ่อย" และ fallback) กรอง `isMenuScreenPending` ออก; ป้าย "รอเชื่อมข้อมูล" ของแบบภาษีตาม `LIVE_TAX_FORMS` ต้องสะท้อนว่าข้อมูลมาจาก GL จริงหรือไม่
- **ห้ามทำซ้ำ**: ใบราชการแบบ HTML + `window.print()` (ตำแหน่งเพี้ยนตามเบราว์เซอร์/เครื่องพิมพ์), โชว์ `message` ที่ backend แปลตาม `Accept-Language` ของเบราว์เซอร์ (ได้อังกฤษบนจอไทย), ทางลัดเริ่มต้นที่ชี้จอ "รอพัฒนา", ปุ่มที่ frontend บังคับเงื่อนไขเกิน backend (เช่น ต้องมีอีเมลก่อนเพิ่มบริษัท) จนบัญชี Demo ใช้ไม่ได้
- **เหตุผล**: ตำแหน่งตัวเลขต้องตรงแบบ 100% ทำได้เฉพาะ PDF ที่ backend วางพิกัดเอง; ภาษา error ต้องตามภาษาที่ผู้ใช้เลือก ไม่ใช่ของเบราว์เซอร์; ลุงจืดกำหนด (2026-09-23) ว่าผู้ใช้ GL ตัวเดียว (สำนักงานบัญชี) ต้องใช้ได้เลย
- **วิธีตรวจ**: Demo → เมนู บัญชีแยกประเภท › รายงานภาษีและแบบยื่น › 50 ทวิ → สร้าง PDF ต้องได้ network 200 `application/pdf` และ viewer 2 หน้า; ใส่เลขผู้เสียภาษีผิด checksum ต้องได้ข้อความไทยใน `role="alert"`; holding ใหม่เปิดหน้าแรกต้องไม่มีทางลัด "รอพัฒนา"; ตรวจ light+dark
- **อ้างอิง**: `frontend/src/app/tax/wht-certificate-panel.tsx`, `frontend/src/lib/thai-tax.ts` (`requestWhtCertificatePdf`), `frontend/src/lib/menu-screen-status.ts` (`LIVE_TAX_FORMS`), `frontend/src/app/menu/dashboard-home.tsx` (`defaultShortcutIds`), `frontend/src/app/system-settings/company-branch-tree-view.tsx` (`canCreateOrganization`)


## 8.31 ฐานภาษีต้องแก้ได้เสมอ — เก็บเป็นข้อมูลในใบสำคัญ ไม่ใช่ค่าประมาณ read-only (2026-09-23)

- **แบบแผน**: ภาษีหัก ณ ที่จ่ายเก็บเป็นรายละเอียดใบสำคัญ `details.withholdings[]` (ทิศทาง/แบบ ภ.ง.ด./คู่ค้า/วันที่จ่าย/ประเภทเงินได้/เงื่อนไข/ฐาน/อัตรา/ภาษี/เลขหนังสือรับรอง) ในจอรายวัน หมวดแรกของ "หลักฐาน ลูกหนี้–เจ้าหนี้ และธนาคาร"; ช่อง "ภาษีที่หัก" เว้นว่างได้ (placeholder `gl_wht_tax_auto`) → backend คำนวณ ฐาน × อัตรา ปัดตาม scale ปีบัญชี; แก้ฐาน/อัตราแล้วล้างภาษีกลับเป็นอัตโนมัติ; แก้ได้ทั้งฉบับร่างและหลังผ่านบัญชี (ผ่านบัญชีแล้วใช้ "เพิ่มผลกระทบยอด" + เหตุผลบังคับ → backend แทนทั้งชุดและเก็บค่าเดิม+เหตุผลใน `gl_subledger_audit` action `withholding_replace`); หลังบันทึกต้องโหลดใบสำคัญจาก backend มาแสดง (ค่าที่ backend คำนวณต้องเห็นทันที). รายงาน/50 ทวิ ใช้ฐานที่บันทึก (`taxbasesource: "recorded"`) และแสดงคำใบ้ `data-field="taxbasesource"` ว่าเป็นฐานที่บันทึกหรือค่าประมาณจากบรรทัดบัญชี (`inferred`) พร้อมบอกว่าแก้ได้ที่ไหน; ตัวเลือกประเภทเงินได้ใช้ `WHT_INCOME_OPTIONS` ชุดเดียวทั้ง GL และ 50 ทวิ; error ข้อความยาวเกินช่องแบบฟอร์มต้องบอกชื่อช่อง (`field` → `WHT_FIELD_LABELS`)
- **ห้ามทำซ้ำ**: ฐานภาษีที่คำนวณย้อนจากบรรทัดบัญชีแล้วล็อก read-only (ยอดจ่ายรวม VAT ≠ ฐานภาษีหัก); ถือค่าที่ส่งไปเป็นค่าที่บันทึกหลัง save (ช่องภาษีว่างทั้งที่ PG มีค่า); แก้ข้อมูลภาษีหลังผ่านบัญชีโดยไม่เก็บเหตุผล; ตัวเลือกประเภทเงินได้ซ้ำสองชุด; error "ข้อความยาวเกินช่อง" ที่ไม่บอกว่าช่องไหน
- **เหตุผล**: ลุงจืดสั่ง 2026-09-23 "ต้องระวังเรื่องฐานภาษีด้วย ต้องเปลี่ยนแปลงได้เสมอ" + ตรวจใน PostgreSQL ทุกครั้ง; Champ `BCAPWTaxList.BaseOfTax` แก้ได้เสมอ; ยอด GL ไม่เปลี่ยนเมื่อแก้ฐาน (เป็นหลักฐานประกอบ) จึงแก้หลังผ่านบัญชีได้โดยมี audit
- **วิธีตรวจ**: Demo → rungrueng/01/00000 → สมุดรายวัน เพิ่มรายการ (ใส่รหัสสาขา) → เพิ่มคู่ค้า + ภาษีหัก ณ ที่จ่าย ฐาน 100,000 อัตรา 1 ภาษีว่าง → บันทึก → PG `payload->details->withholdings` ต้องได้ tax 1000 และจอแสดง 1,000.00 ทันที → แก้ฐาน/พิมพ์ภาษีเอง → PG เปลี่ยนในแถวเดิม → ผ่านรายการ → เพิ่มผลกระทบยอดแก้ฐาน + เหตุผล → PG version เพิ่ม, status posted, `gl_subledger_audit` มี before/after/reason, `gl_lines` ไม่เปลี่ยน → 50 ทวิ แถวนั้น `taxbasesource=recorded` และ prefill ตรง; ตรวจ light+dark
- **อ้างอิง**: `frontend/src/app/gl/gl-journal-details.tsx` (section `withholdings`), `frontend/src/app/gl/gl-journals.tsx` (`save()` โหลดใบสำคัญหลังบันทึก), `frontend/src/lib/gl-journal-details.ts` (`reconciliationChanges`), `frontend/src/lib/thai-tax.ts` (`WHT_INCOME_OPTIONS`), `frontend/src/app/tax/wht-certificate-panel.tsx` (`WHT_FIELD_LABELS`, taxbasesource hint), `backend/internal/generalledger/subledger_withholding.go`, `backend/internal/generalledger/subledger_reconcile.go`, `backend/internal/goapi/handlers/tax_report.go` (`recordedWithholdings`)

## 8.32 รายละเอียดภาษีมูลค่าเพิ่มในใบสำคัญ — ฟอร์มตามประเภท, ค่าเริ่มต้นจากบริบท, ฐานภาษีแก้ได้เสมอ (2026-09-23)

- **แบบแผน**: ภาษีซื้อ/ขายเก็บเป็น `details.vats[]` ของใบสำคัญ GL (1 แถว = 1 ใบกำกับ ชื่อฟิลด์ตาม `vat.sql`) และแสดงเป็น section ถัดจากภาษีหัก ณ ที่จ่ายด้วย component เดียวกัน — ฟิลด์ที่ไม่เกี่ยวกับแถวนั้นซ่อนด้วย `show:(row)=>boolean` (ใบกำกับเดิมเฉพาะใบเพิ่ม/ลดหนี้, สถานะใช้สิทธิ + เหตุผลเฉพาะภาษีซื้อ) แทนการ disable; งวดภาษีเป็น `<select>` "เดือน ปี" (ไทยแสดง พ.ศ. ผ่าน key `gl_vat_period_option` = `{0} {2}`, en `{0} {1}`) ช่วง −12..+6 เดือนจากวันที่ใบสำคัญ + ค่าที่บันทึกไว้เดิมเสมอ; ค่าเริ่มต้นแถวใหม่มาจากบริบท: ประเภทภาษีจากสมุด (UV = ขาย, อื่น = ซื้อ), ใบกำกับภาษี, วันที่/งวด = เดือนของใบสำคัญ, อัตรา "7.00", ใช้สิทธิในงวดนี้ (ซื้อ); เลือกคู่ค้าแล้วเติมชื่อ/เลขภาษี/สาขาให้แต่แก้ต่อได้; ช่องภาษีว่าง = backend คำนวณ ฐาน × อัตรา (placeholder บอก) และแก้ฐาน/อัตราแล้วล้างภาษีที่พิมพ์ไว้ให้คำนวณใหม่
- **ห้ามทำซ้ำ**: คำนวณ VAT/ฐานภาษีจากบรรทัดเดบิต-เครดิตแล้วล็อก read-only; เก็บใบลดหนี้เป็นยอดติดลบ (เก็บบวกเสมอ รายงานเป็นผู้กลับเครื่องหมาย); ใช้ `<input type="month">` หรือช่องปี/เดือนแยก (คน 40+ พิมพ์ ค.ศ./พ.ศ. สลับ); แสดงช่องใช้สิทธิกับภาษีขาย; ปล่อย section ภาษีเป็น read-only หลังผ่านบัญชี (ต้องอยู่ใน `taxKeys` ที่แก้ได้เสมอและส่งผ่าน reconcile พร้อมเหตุผล)
- **เหตุผล**: ลุงจืดสั่ง 2026-09-23 ฐานภาษีต้องเปลี่ยนแปลงได้เสมอ + ผู้ใช้ GL อย่างเดียวต้องยื่น ภ.พ.30 ได้; รายงานภาษีซื้อ/ขาย/ภ.พ.30 อ่านเฉพาะค่าที่บันทึก (`VatRecordsForPeriod`) จึงต้องให้ผู้ใช้กรอกงวดที่ถูกต้องได้ง่ายที่สุด; แก้หลังผ่านบัญชีไม่เปลี่ยนยอด GL แต่เก็บค่าเดิมใน `gl_subledger_audit` action `vat_replace`
- **วิธีตรวจ**: Demo → rungrueng/01/00000 → สมุดรายวัน เพิ่มรายการ → เปิด "ภาษีมูลค่าเพิ่ม (ใบกำกับภาษี)" → เพิ่ม: ต้องได้ ภาษีซื้อ/ใบกำกับภาษี/วันที่วันนี้/งวดเดือนนี้ (พ.ศ.)/7.00/ใช้สิทธิในงวดนี้ → เปลี่ยนเป็นใบลดหนี้ ต้องมีช่องใบกำกับเดิม 2 ช่อง → เปลี่ยนเป็นภาษีขาย ช่องใช้สิทธิต้องหาย; หลังบันทึกตรวจ PG `payload->'details'->'vats'`; ทดสอบ backend `go test -tags integration ./internal/generalledger -run Vat` + `./internal/goapi/handlers -run Vat`
- **อ้างอิง**: `frontend/src/app/gl/gl-journal-details.tsx:34` (fields `vats` + `show`), `:61` (`taxKeys`), `:65` (`newRow` ค่าเริ่มต้น), `:136` (period select); `frontend/src/lib/gl-journal-details.ts:44-56` (`defaultVatTaxType`, `vatPeriodValue`, `vatPeriodPatch`, `vatPeriodChoices`); `backend/internal/generalledger/subledger_vat.go` (`vat()` ตรวจตาม CHECK ของ vat.sql, `VatRecordsForPeriod`); `backend/internal/generalledger/subledger_reconcile.go:78` (`vat_replace`); `backend/internal/goapi/handlers/tax_report.go:171` (`buildVatRegister`), `:217` (`sumPP30`)

## 8.33 จอแบบยื่นกรมสรรพากร — จอเดียวสร้างจาก schema ของ backend, เงินเก็บไม่มีคอมมาแต่แสดงมีคอมมา, ตัวอย่าง PDF ต้องเห็นทันที (2026-09-23)

- **แบบแผน**: ทุกแบบ (ภ.ง.ด.2/2ก/3/53/50/51/93/94, ภ.พ.30/36, ภ.ธ.40) ใช้ `TaxFormEditor` ตัวเดียว — ช่องสร้างจาก `POST /api/report/tax/form/schema` จัดเป็นหมวด `<details>` ตาม `group` (ป้ายหมวด `tax_form_group_<group>`), ใบแนบ `rows` = ตารางหัวคอลัมน์ sticky + เพิ่ม/ลบแถว, `sheets` = การ์ดทีละแผ่น; ลำดับงาน บน→ล่าง: เลือกแบบ/ปี/เดือน → ฉบับที่บันทึก → แถบปุ่ม (บันทึก = CTA หลัก, พิมพ์ PDF, คำนวณยอดรวม, ดึงยอดจากบัญชี, ยื่นเพิ่มเติม, ลบ) → ข้อความสถานะ + หมายเหตุจาก backend → ค้นหาช่อง → หมวดช่อง → ใบแนบ. **ช่องเงิน** เก็บในเอกสารเป็น `"125000.00"` (ไม่มีคอมมา — ค่าเดียวกับที่ PG เก็บ) แต่แสดง `125,000.00` เมื่อไม่ได้โฟกัสช่อง (`displayMoney` + state `editing`), ออกจากช่องแล้ว normalize ด้วย `formatAmount(...).replace(/,/g, "")`; ชนิดช่องกันพิมพ์ผิดด้วย `acceptsTyping` (เงินทศนิยม ≤ 2, เลข 13 หลัก, int); save/PDF เรียก compute ก่อนเสมอ; **ตัวอย่าง PDF**: จอ xl = คอลัมน์ขวา sticky ดูคู่ช่องกรอก, จอเล็กกว่า (iPad) = overlay กลางจอ `fixed inset-0` + ปุ่ม "ปิดตัวอย่าง" autoFocus + Esc; **ข้อความสถานะเก็บเป็น `{ key, fallback, version? }` แล้วแปลตอน render** (สลับภาษาแล้วเปลี่ยนตาม); ปีแสดง พ.ศ. เมื่อภาษาไทย ค.ศ. เมื่ออังกฤษ, ชื่อเดือนจาก key `month_*`; เปลี่ยนงวด/แบบ/ดึงยอดใหม่ขณะมีค่าที่ยังไม่บันทึก → confirm; ลบ → confirm danger; `beforeunload` guard
- **ห้ามทำซ้ำ**: แบบราชการ HTML + `window.print()` (ถูกลบจาก `tax-filing-workbench.tsx` แล้ว); เก็บ/ส่งยอดเงินที่มีคอมมาหรือเป็น JS number; `setStatus(tr(...))` เก็บข้อความที่แปลแล้วใน state (ค้างเป็นภาษาเดิมหลังสลับภาษา — เจอจริงตอน UAT); วางตัวอย่าง PDF ต่อท้ายหน้าบนจอแคบ (ผู้ใช้กดพิมพ์แล้วไม่เห็นอะไร — iframe ไปอยู่ y≈2700px); คำนวณยอดรวม/ภาษีบน browser; แตกเมนูใหม่ต่อแบบ (ใช้เมนูเดิม 5 รายการเปิดแบบนั้นตรง ๆ + "แบบยื่นภาษีทั้งหมด" 1 รายการ); fallback ไทยแบบ array ในโค้ด (`MONTH_TH`) แทน key
- **เหตุผล**: ลุงจืดสั่ง 2026-09-23 ทำระบบให้สัมพันธ์กับแบบใน `mydocs/sample` ครบทุกแบบ + ฐานภาษีแก้ได้เสมอ + ตรวจ PG ทุกครั้ง; ตำแหน่งบนแบบต้องตรง 100% จึงให้ backend วาด PDF (§8.30); คน 40+ อ่านยอดเงินไม่มีคอมมายาก แต่ถ้าเก็บคอมมาไว้ใน state ค่าที่ส่ง/เก็บจะไม่ canonical; จอ iPad (768–1279) เป็นเป้าหมายหลัก (ดู [[viewport-target-ipad-up]]) คอลัมน์ข้างไม่พอ จึงต้องเป็น overlay
- **วิธีตรวจ**: Demo → rungrueng/01/00000 → เมนู "แบบยื่น ภ.ง.ด.53" → เลือกเดือนที่มีภาษีหัก → prefill ต้องได้แถวใบแนบ + หมายเหตุ (ฐานประมาณ/ไม่มีเลขผู้เสียภาษี) → แก้ฐาน/ภาษีแถวหนึ่ง (พิมพ์จริงด้วยคีย์บอร์ด ไม่ใช้ form_input) → คำนวณยอดรวม → บันทึก → PG `select version, document->'values'->>'total_income' from tax_filings` ต้องได้ค่าไม่มีคอมมาตรงจอ → พิมพ์ PDF ต้อง 200 `application/pdf` และเห็นตัวอย่างทันทีทั้งจอ 800px (overlay) และ ≥1280 (คอลัมน์ขวา) → ลบฉบับ → PG 0 แถว (ลบ `tax_filing_history` ของ id ทดสอบด้วย) → สลับธีมมืดด้วยปุ่มจริง + สลับ English (ข้อความจอ/สถานะ/หมายเหตุต้องเปลี่ยน, ป้ายช่องเป็นไทยตามแบบทางการ); `vitest src/app/tax/tax-form-language-keys.test.ts` ตรวจ key ทุกตัว (รวม `tax_form_title_*`, `tax_form_group_*`, `month_*`, code/note ของ backend) และห้ามไทยฝังนอก `tr()`/`{ key, fallback }`
- **หมายเหตุที่ตรวจใหม่หลังคำนวณ (2026-09-23)**: หมายเหตุจาก prefill ค้างอยู่จนกว่าจะดึงยอดใหม่ ยกเว้น key ที่ backend ส่งมาใน `rechecked` ของ `POST .../form/compute` — จอต้อง**แทน**เฉพาะ key เหล่านั้นด้วย `notes` ชุดใหม่ (`setNotes(prev => [...prev.filter(n => !rechecked.includes(n.key)), ...notes])` ใน `runCompute`) ห้ามล้างหมายเหตุทั้งหมด (หมายเหตุฐานประมาณ/ที่มาเครดิตยังจริงอยู่) และห้ามตรวจเงื่อนไขซ้ำบน browser (backend-first); ตอนนี้ตรวจใหม่เฉพาะ `tax_form_note_missing_taxid` (แบบกลุ่ม wht) — วิธีตรวจ: ภ.ง.ด.53 เดือนที่มีแถวไม่มีเลข 13 หลัก → กรอกเลข → กด "คำนวณยอดรวม" → หมายเหตุนั้นหาย หมายเหตุอื่นอยู่ครบ, network ตอบ `notes: []` + `rechecked`
- **อ้างอิง**: `frontend/src/app/tax/tax-form-editor.tsx` (`TaxFormEditor`, `type Status`, overlay ตัวอย่าง PDF), `frontend/src/app/tax/tax-form-fields.tsx` (`TaxFormFieldInput` + `displayMoney`, `TaxFormFieldGroups`, `TaxFormRowsTable`, `TaxFormSheets`), `frontend/src/lib/tax-forms.ts` (API client, `acceptsTyping`, `cleanDocument`, `TAX_FORM_ROUTES`), `frontend/src/app/tax/tax-form-language-keys.test.ts`, `frontend/src/app/menu/main-menu-screen.tsx` (`taxFormRouteCode` ก่อน `isThaiTaxRoute`), `backend/internal/goapi/handlers/tax_form*.go`, `backend/internal/rdform/`, ADR `docs/kms/decisions/2026-09-23-rd-tax-forms-engine.md`

## 8.34 ปุ่มที่ไม่มี API จริงรองรับ = ถอดออก ให้จอเป็นอ่านอย่างเดียว + บอกทางไปจอเจ้าของงาน (2026-09-23)

- **แบบแผน**: ถ้างานหนึ่งมีจอเจ้าของอยู่แล้ว (เช่น เพิ่ม/ถอดผู้ดูแลกลุ่มกิจการ → ตั้งค่าระบบและการเข้าถึง › บัญชีเข้าระบบ, `PUT/DELETE /holding/permission`) จออื่นแสดง **รายการอ่านอย่างเดียว** + ข้อความไทยบอกทางไปจอเจ้าของงาน (key ใน `languages.tsv`) — ไม่ทำฟอร์มซ้ำอีกชุด; ป้ายปุ่มเปิดต้องตรงกับสิ่งที่ทำได้จริง ("ดูรายชื่อผู้ดูแล" ไม่ใช่ "จัดการผู้ดูแล")
- **ห้ามทำซ้ำ**: ปุ่ม/ฟอร์มที่ยิง route ที่ backend ไม่ได้ register (ผู้ใช้กดแล้วได้ 404 เป็นข้อความผิดพลาดงง ๆ) และ BFF ที่ยังมี POST/DELETE ไปหา route ที่ตายแล้ว — ลบทั้งปุ่ม, BFF method, handler/service ที่ไม่มีใครเรียก, และแถว `languages.tsv` ของปุ่มนั้นใน change เดียวกัน (เดินหน้าอย่างเดียว)
...
## 8.35 จอตั้งค่าห้ามโชว์ข้อผิดพลาดดิบจาก backend — ผ่านตัวแปลงกลาง `user-facing-error.ts` เสมอ (2026-09-24)

- **แบบแผนใหม่ (New Standard Pattern)**:
  - **ฝั่งจอ**: ทุก fetch ที่ล้มเหลวให้ `throw settingsRequestError(response.status, payload)` (เก็บ HTTP status + `errorcode` + `message` + `message_th`) แทน `new Error(extractMessage(payload))`; ทุก catch แสดงผ่าน `userFacingErrorText(error, language, dictionary, fallback)` ห้ามอ่าน `error.message` ตรง ๆ
  - **ลำดับการเลือกข้อความ** (`requestErrorText`): (1) message เป็น key ใน `languages.tsv` → แปล (2) message/message_th ตรงกับข้อความในพจนานุกรมภาษาที่เลือก (backend render จาก key มาแล้ว) → ใช้เลย (3) **ภาษาไทยเท่านั้น และมาก่อน errorcode**: `message_th`/message ที่เป็นประโยคไทยเฉพาะเรื่อง ไม่มีคำเทคนิค และไม่ใช่ข้อความ default ของ apperr (`genericThaiMessages` เช่น "ไม่มีสิทธิ์", "ไม่พบข้อมูล") → ใช้เลย — เช่น FORBIDDEN + "กรุณาเลือกกลุ่มกิจการก่อน" ต้องขึ้นประโยคนี้ ไม่ใช่ "คุณไม่มีสิทธิ์ทำรายการนี้" (4) `errorcode` ที่รู้จัก → แถว `ss_err_*` (5) นอกนั้นตาม HTTP status: 400/422 `ss_err_invalid_data`, 401 `ss_err_session_expired`, 403 `ss_err_no_permission`, 404 `ss_err_not_found`, 409 `ss_err_conflict`, 5xx/เครือข่าย `ss_err_try_again` — ข้อความดิบที่ถูกซ่อน `console.warn` ให้นักพัฒนาเห็น
  - **ข้อความที่จอสร้างเอง** (`new Error(backendText(...))`) แสดงได้ตามเดิม — ฟังก์ชันระดับ module ที่ไม่มี dictionary ให้รับข้อความจากผู้เรียก (`resizeLogoFile(file, backendText(dictionary, "ss_only_png_supported", ...))`) ห้ามฝังไทยใน `throw new Error("...")`; `TypeError`/`SyntaxError`/`DOMException` (fetch ล้ม, body ไม่ใช่ JSON) = เทคนิค → "ลองใหม่"
  - **คำเทคนิคที่ถูกซ่อนเสมอ** (`technicalPattern`): decoder/driver (`json:`, `pq:`, `sql:`, `cannot unmarshal`, `find failed`), เครือข่าย (`failed to fetch`, `dial tcp`, `econnrefused`), `context deadline`/`context canceled`, `EOF`, `invalid UUID`, `__v`, `runtime error`, `HTTP 500` — แม้ขึ้นต้นด้วยไทยก็ซ่อน
  - **อ่าน body ที่อาจไม่ใช่ JSON**: `await response.json().catch(() => null)` แล้ว `throw settingsRequestError(response.status, data)` (หน้า error HTML ของ proxy → "Unexpected token <" ห้ามถึงผู้ใช้) — ใช้กับจอนอกตั้งค่าด้วย เช่นรายชื่อผู้ดูแลในจอกลุ่มกิจการ
  - **ผลรายแถว (นำเข้าผู้ใช้)**: backend ส่ง key ของ `languages.tsv` ต่อแถว (`shopUserErrorRow`) จอแปลผ่าน `requestErrorText({ status, message }, ...)` ห้ามแสดง `r.message` ดิบ — *จอนำเข้าผู้ใช้ถูกลบแล้ว 2026-09-24 (§8.36 ข้อ 6) กติกานี้ใช้กับผลรายแถวของจอนำเข้าอื่นในอนาคต*
  - **BFF ตรวจข้อมูลแล้วตอบ key**: ขอบเขตการใช้งานที่ส่งมาผิดแม้กฎเดียว → 400 `{errorcode:"VALIDATION_FAILED", message:"ss_err_access_scope_invalid"}` (ห้ามทิ้งกฎเสียเงียบ ๆ — ADMIN/OWNER ที่เหลือ `[]` = ทั้งกลุ่มกิจการ)
  - **ช่องติ๊ก "ใช้ได้ทั้งกลุ่มกิจการ"**: ติ๊กแล้วเอาออกต้องได้บริษัท/สาขาที่เลือกไว้คืน (`toggleHoldingScopeRules` + ref ที่ผูกกับค่าที่ commit จริง — record อื่นไม่ได้ค่าข้าม)
  - **JSON ในช่องกรอก**: `buildPayload` จับ parse error แล้ว `throw new JsonFieldError(fieldLabel(...))` → จอแสดง `jsonInvalid (ชื่อช่อง)` ผ่าน `payloadErrorText`
  - **Toast สำเร็จ**: ต่อท้ายได้เฉพาะ message ที่เป็น key (`dictionaryMessage`) ห้ามต่อข้อความดิบจาก backend
  - **ฝั่ง backend**: ตอบผ่าน `apperr.Respond` ด้วย code มาตรฐาน + `message` = `language.Text(key, lang)` + `message_th` = ไทยของ key เดียวกัน (`localizedAppError`); error ที่ไม่รู้จัก = `INTERNAL_ERROR` ห่อ cause ไว้ log เท่านั้น
- **กับดัก/ห้ามทำซ้ำ (Anti-pattern)**: `catch (e) { setError(e.message) }` (เคยพบใน `handleSave` ของผังบริษัท/สาขา และ `bulk-user-import.tsx`); `throw new Error(json?.message)`; `(await response.json())` โดยไม่ `.catch`; ให้ errorcode ชนะประโยคไทยเฉพาะเรื่อง; `ctx.ResponseError(400, err.Error())` / `"find failed"` / `backendText(dict, message, message)` กับข้อความที่ไม่ใช่ key (หลุดเป็น "บันทึกไม่สำเร็จ: json: cannot unmarshal object into Go struct field ..." บนจอบัญชีเข้าระบบ); `` throw new Error(`HTTP ${status}`) ``; `"Load shops failed"`/ข้อความอังกฤษฝังโค้ด; ต่อ `extractMessage(payload)` เข้า toast สำเร็จ; แสดง `SyntaxError` ของ `JSON.parse` ให้ผู้ใช้
- **เหตุผล (Root Cause & Rationale)**: ผู้ใช้หลักคนไทย 40+ ต้องได้ประโยคที่บอกว่าต้องทำอะไรต่อ (กฎ 40+ ข้อ 8); ข้อความ decoder/driver ของ Go เป็นอังกฤษเทคนิคและเผยโครงสร้างภายใน (ความปลอดภัย); ถ้าทิ้ง status/code ตอน throw จอจะเดาไม่ได้ว่าเป็นสิทธิ์/ไม่พบ/ข้อมูลผิด; BFF ส่ง `Accept-Language` ของเบราว์เซอร์ (ไม่ใช่ภาษาที่เลือกในแอป) จึงต้องเช็คข้อความกับพจนานุกรมภาษาที่เลือก และใช้ `message_th` เมื่อจอเป็นไทย
- **วิธีตรวจ**: `npx vitest run src/components/system-settings/user-facing-error.test.ts` (รวมเคส "ไทยเฉพาะเรื่องก่อน errorcode" และคำเทคนิค); `npx vitest run "src/app/api/system-settings"`; `go test ./internal/shop/ -run 'TestShopUserAppError|TestSaveFullProfileRejectsFixable'` (repo validation → 400 ไม่ใช่ 500); จริง: Demo → rungrueng → ตั้งค่าระบบและการเข้าถึง › บัญชีเข้าระบบ → ลบบัญชีตัวเอง/เปลี่ยนบทบาทตัวเอง ต้องได้ประโยคไทยจาก `ss_err_cannot_*` ไม่ใช่ข้อความอังกฤษ; สลับ English แล้วต้องไม่มีไทยค้าง
- **อ้างอิง (Reference Implementation)**: `frontend/src/components/system-settings/user-facing-error.ts` (`requestErrorText` :144, `userFacingErrorText` :175, `codeRows` :53, `statusRow` :67, `technicalPattern` :77, `genericThaiMessages` :83, `isSpecificThaiText` :104); `frontend/src/app/holding/holding-screen.tsx` (`listHoldingMembers` :1425); BFF `frontend/src/app/api/system-settings/[[...settingPath]]/route.ts` (`hasInvalidAccessScopeRules` :369); `frontend/src/components/system-settings/field-editors/holding-scope-editor.tsx` (`toggleHoldingScopeRules` :202); `frontend/src/app/system-settings/system-settings-screen.tsx` (`errorText` :942, `payloadErrorText` :966, `JsonFieldError` ใน `buildPayload` :15126); `frontend/src/app/system-settings/company-branch-tree-view.tsx` (`saveErrorMessage` :2152, catch ของ `handleSave` :1266); `frontend/src/components/system-settings/field-editors/permission-sets-editor.tsx`; `backend/internal/shop/shopuser_http.go` (`localizedAppError` :231, `shopUserAppError` :238, `shopUserErrorRow` :253) + `shopuser_http_test.go`; ต่างจาก §8.24 (GL ใช้ไทยจาก backend ก่อน) — จอตั้งค่าใช้ key/code ก่อน เพราะรองรับ 2 ภาษา

## 8.36 จอภาษี: ป้ายใบกำกับซ้ำ = ไอคอน+ข้อความ+เลขใบสำคัญ, คำแนะนำเดือนใช้สิทธิภาษีซื้อ, ยืนยันก่อนทับค่าด้วยยอดบัญชีพร้อมรายการช่อง (2026-09-24)

- **แบบแผนใหม่ (New Standard Pattern)**:
  1. **ป้ายใบกำกับซ้ำในทะเบียนภาษี** — backend ส่ง `duplicatedocnos: string[]` ต่อแถว (+ `summary.duplicatecount` ทั้งงวด); จอแสดง `DuplicateInvoiceBadge` ใต้เลขที่ใบกำกับ = ไอคอน `AlertTriangle` + ข้อความ "ใบกำกับซ้ำ" (`vat_ui_duplicate_badge`) + บรรทัด "บันทึกซ้ำในใบสำคัญ {0}" (`vat_ui_duplicate_docnos`) ด้วย token `border-destructive/40 bg-destructive/10 text-destructive`, ตัวอักษร ≥ 0.85rem, `[overflow-wrap:anywhere]`; และกล่องแจ้งเหนือตาราง (`vat_ui_duplicate_notice` แทน `{0}` ด้วย duplicatecount) `role="status"` ที่บอก**ทางแก้** (ลบรายละเอียดภาษีที่เกิน/แก้เลขที่ให้ตรงใบจริง) และบอกว่า**เลขที่เดียวกันจากผู้ออกใบกำกับต่างรายไม่ใช่ใบซ้ำ** (ม.86/4 ใบกำกับระบุผู้ออก + เลขที่ https://www.rd.go.th/5208.html) — backend ตัดสินว่าซ้ำ จอแค่แสดง
  2. **คำแนะนำเดือนใช้สิทธิภาษีซื้อในใบสำคัญ GL** — `vatClaimTiming(row)` (เลขปี/เดือนล้วน ไม่ใช้ `Date`) คืน `late` (1–6 เดือนหลังเดือนออกใบกำกับ เฉพาะใบกำกับภาษี) และ `before`/`exceeded` ทุกประเภทเอกสาร (ตรงกับ `checkPurchaseClaimWindow` ของ backend) → กล่อง `role="status"` ไอคอน `Info` บอกข้อความที่ต้องเขียนในใบกำกับ "ถือเป็นภาษีซื้อในเดือนภาษี {เดือน ปี พ.ศ.}" (`vat_ui_claim_late_hint`); `before`/`exceeded` → กล่อง `role="alert"` ไอคอน `AlertTriangle` ที่ใช้ **key เดียวกับ error ของ backend** (`vat_claim_before_invoice_month`, `vat_claim_window_exceeded`) ผู้ใช้จึงเห็นคำแนะนำเดียวกันก่อนและหลังกดบันทึก; ไม่บล็อกปุ่มบันทึก (backend เป็นผู้ตัดสิน) — แหล่งอ้างอิง ม.82/3 วรรคสอง https://www.rd.go.th/5206.html + ประกาศอธิบดีฯ VAT ฉบับที่ 4 ข้อ 2 (แก้โดยฉบับที่ 76) https://www.rd.go.th/3417.html
  3. **ยืนยันก่อนทับค่าด้วยยอดจากบัญชีเสมอ** — ปุ่ม "ดึงยอดจากบัญชี" (ทั้งแถบปุ่มและในกล่องเตือนยอดเปลี่ยน) ดึง prefill มาก่อน *ยังไม่แตะจอ* → `formChanges(schema, doc, fresh)` + `attachmentChanged` → มีช่องที่จะเปลี่ยน = `confirm()` tone warning ที่ description เป็นรายการ "{ช่อง}: {ค่าเดิม} → {ค่าใหม่}" (เงินมีคอมมา, ตัวเลือกเป็นป้าย, ว่าง = "(ว่าง)", แสดงไม่เกิน 6 แถว + "และอีก n ช่อง") และบอกว่าฉบับที่บันทึกยังไม่เปลี่ยนจนกว่าจะกดบันทึก; ไม่มีช่องเปลี่ยน = ไม่ถาม
  4. **เตือน "ยอดบัญชีเปลี่ยน" ต้องไม่เตือนยอดที่ผู้ใช้แก้เอง** — จำ prefill ที่ค่าในจอตั้งต้น (`basisRef`) แล้วเทียบ prefill ตอนนี้กับ prefill ตอนเตรียม (`mode: "basis"`); เปิดฉบับที่บันทึกซึ่งไม่มียอดตั้งต้น (backend ยังไม่เก็บ) → เทียบกับค่าที่บันทึกโดยคงค่าช่องที่แก้ในรอบนี้ (`ledgerBaseline(..., editedRef)`) และ**บอกผู้ใช้ตรง ๆ** (`tax_ui_drift_no_basis`) ว่าช่องที่เคยแก้เองครั้งก่อนอาจขึ้นว่าต่าง
  5. **ปุ่มในหน้าต่างรายละเอียดต้องทำสิ่งที่ป้ายบอกจริง** — "พิมพ์เอกสารรับรอง" ที่เรียก `window.print()` (พิมพ์หน้าทะเบียน ไม่ใช่หนังสือรับรอง) ถูกลบ: ภาษีที่เราหัก → ปุ่ม "ออกหนังสือรับรอง 50 ทวิ ของรายการนี้" (`tax_ui_open_50twi`) เปิดแท็บแผง PDF ของ backend ที่เลือกแถวนั้นไว้แล้ว; ภาษีถูกหัก/ภาษีมูลค่าเพิ่ม = ไม่มีปุ่ม; ชิปบริษัทใช้ `taxCompanyLabel` (หัวบริษัทจาก backend → บริษัทใน workspace ของ session ที่รหัสตรงกันเท่านั้น → รหัส) อ่าน localStorage ผ่าน `useSyncExternalStore` (SSR ได้ค่าว่าง ไม่ hydration mismatch); การ์ดยอดภาษีใช้ป้ายกลาง "ยอดภาษีมูลค่าเพิ่มรวม" (`tax_ui_vat_total`) เพราะยอดรวมมีอัตรา 0%/ยกเว้นด้วย
  6. **ปุ่มที่ยิง route ที่ไม่ได้ register = ลบทั้งสาย** (ต่อยอด §8.34) — ปุ่ม "นำเข้าผู้ใช้จาก Excel/CSV" + `bulk-user-import.tsx` + BFF `/api/holding-users-import` ถูกลบ เพราะ backend ตั้งใจไม่ register `POST /holding/users/import` (commit 07c9c21c: กันการข้ามขั้นตอนตอบรับและ audit)
- **กับดัก/ห้ามทำซ้ำ (Anti-pattern)**: สื่อ "ซ้ำ" ด้วยสีแดงของแถวอย่างเดียว; จอเดาเองว่าใบกำกับซ้ำจากเลขที่อย่างเดียว (ผู้ขายต่างรายใช้เลขเดียวกันได้); คิดเดือนต่างด้วย `new Date(...)` (เขตเวลาทำให้วันสิ้นเดือนเลื่อนเดือน); ข้อความ error ฝั่งจอที่คนละถ้อยคำกับ backend; `guard()` ที่ถามเฉพาะตอน dirty แล้วทับฉบับที่บันทึกเงียบ ๆ; เทียบค่าที่บันทึก (ที่ผู้ใช้แก้) กับยอดบัญชีแล้วเตือนว่า "บัญชีเปลี่ยน"; `window.print()` บนปุ่มที่ชื่อว่าเอกสารราชการ; ป้าย "ยังไม่ระบุ" ทั้งที่ session รู้บริษัท; ป้ายอัตราตายตัว "7%" บนยอดรวมที่รวมหลายอัตรา; regex คำเทคนิคที่ไม่มี word boundary (`load failed` จับ "upload failed" ของจอเองทิ้ง — แก้เป็น `\bload failed\b` ใน `user-facing-error.ts`)
- **เหตุผล (Root Cause & Rationale)**: ผู้ใช้ 40+ ต้องรู้ว่าต้องทำอะไรต่อ ไม่ใช่เห็นแค่สี (กฎ 40+ ข้อ 5, 8); ใบกำกับหนึ่งฉบับใช้สิทธิได้ครั้งเดียว (ม.82/5) และใบซ้ำ/สำเนาเป็นภาษีซื้อต้องห้าม (ประกาศฯ ฉบับที่ 42 https://www.rd.go.th/3398.html) ใช้สิทธิผิดงวด = ภาษีซื้อไม่ได้รับเครดิต; การทับค่าที่ผู้ใช้แก้โดยไม่ถาม = ข้อมูลหาย (กฎ 40+ ข้อ 7); คำเตือนผิดทำให้ผู้ใช้เลิกเชื่อคำเตือนจริง
- **วิธีตรวจ**: `npx vitest run src/lib/thai-tax.test.ts src/lib/tax-forms.test.ts src/lib/gl-journal-details.test.ts src/components/system-settings/user-facing-error.test.ts src/app/tax src/app/gl/gl-language-keys.test.ts`; Demo → rungrueng/01/00000 → รายงานภาษีซื้อ งวดที่มีใบกำกับซ้ำ ต้องเห็นกล่องแจ้ง + ป้ายซ้ำพร้อมเลขใบสำคัญ (ตรวจ network ว่า `duplicatedocnos`/`duplicatecount` มาจาก backend); ใบสำคัญ → ภาษีซื้อ ใบกำกับ ม.ค. เลือกงวด ก.พ. ต้องเห็นข้อความ "ถือเป็นภาษีซื้อในเดือนภาษี กุมภาพันธ์ …", เลือก ส.ค. ต้องเห็นกล่องเตือนนอกกำหนด; แบบ ภ.พ.30 ที่บันทึกแล้ว แก้ช่องเงิน → บันทึก → ต้องไม่ขึ้นเตือนยอดเปลี่ยน, กด "ดึงยอดจากบัญชี" ต้องได้ dialog รายการช่องก่อนทับ
- **อ้างอิง (Reference Implementation)**: `frontend/src/app/tax/tax-filing-workbench.tsx` (`DuplicateInvoiceBadge` :714, `showDuplicateNotice` :194, `taxCompanyLabel` :192, ปุ่ม 50 ทวิ :700); `frontend/src/lib/thai-tax.ts` (`toTaxRecord`/`toTextList`, `taxCompanyLabel` :185); `frontend/src/app/gl/gl-journal-details.tsx` (`VatClaimHint` :75, ใช้ :164); `frontend/src/lib/gl-journal-details.ts` (`vatClaimTiming` :63); `frontend/src/app/tax/tax-form-editor.tsx` (`checkDrift` :148, `applyPrefill` :169, `onPrefill` :326, `confirmPrefill` :339, `displayValue` :401); `frontend/src/lib/tax-forms.ts` (`ledgerBaseline` :315, `ledgerDrift` :339, `formChanges` :348, `attachmentChanged` :358)

## 8.37 ช่องยอดเงิน `AmountInput`, วางจาก Excel และตัวเลือกสมุดรายวัน — ไม่ปัด ไม่ตัด ไม่เดา บอกแถว-ช่องที่ผิด (2026-09-24)

- **แบบแผนใหม่ (New Standard Pattern)**:
  1. **ช่องยอดเงินทุกจอ GL ใช้ `AmountInput`** — ค่าเป็น decimal string เสมอ (ห้าม `Number()`/`parseFloat` กับยอดเงิน, AGENTS.md กฎ 20); กด Tab ผ่านช่องที่ไม่ได้แตะต้องคงค่าเดิม (`"0"` ไม่กลายเป็น `""` — `amountBlurValue(touched, …)`); แปลงเลขไทย ๐–๙ และตัดคอมมา/ตัวอักษรที่ไม่ใช่ตัวเลขพร้อมแจ้ง `gl_amount_characters_removed` (`normalizeAmountText`); **ไม่ปัดเศษ ไม่ตัดเครื่องหมายลบเอง** — แสดงปัญหาใต้ช่องด้วย `amountInputIssue` → `gl_amount_negative_not_allowed` / `gl_amount_too_many_decimals` แล้วให้ผู้ใช้แก้
  2. **ก่อนส่ง backend**: `normalizeJournalLines` เปลี่ยนเดบิต/เครดิตว่างเป็น `"0"`; ข้อความผิดพลาดของบรรทัดขึ้นต้นด้วย "บรรทัดที่ N: ช่อง…" (`gl_line_amount_invalid`, `gl_line_amount_negative`)
  3. **ช่องเลขผู้เสียภาษี/สาขาในรายละเอียดภาษี** ไม่ใส่ `maxLength=13` — รับค่าที่วางแบบมีขีดแล้ว `taxDigits` ตัดเหลือตัวเลข บอก "ตอนนี้ N หลัก" (`gl_detail_tax_id_hint`); สาขา `normalizeBranchNo` เติม 0 ซ้ายให้ครบ 5 หลัก (`0` → `00000` = สำนักงานใหญ่); ยอดภาษีว่าง = ไม่ส่งช่องนั้น (backend คำนวณ ฐาน × อัตรา); อัตราบังคับกรอก (`gl_detail_rate_required_inline`); ปัญหาบอกหมวด+แถว+ช่อง (`journalDetailsProblem`)
  4. **วางจาก Excel/Google Sheets** (ปุ่ม "วางจาก Excel" + Ctrl+V ในตาราง): `parseClipboardJournalLines(text)` คืน `{lines, issues}` — แถวที่มีแต่เดบิตเก็บไว้ ฝั่งว่าง = `"0"`; ยอดติดลบ/วงเล็บ `(1,500.00)`/ตัวอักษร = **ปฏิเสธทั้งชุด** พร้อมบอกแถวและช่อง (`gl_paste_rejected` + `gl_paste_issue_*`) ไม่แปลงเอง; เกิน 500 บรรทัดต่อใบ = ไม่วาง (`gl_paste_too_many_lines`); เบราว์เซอร์ไม่ให้อ่านคลิปบอร์ด → บอกให้กด Ctrl+V แทน (`gl_clipboard_denied` / `gl_clipboard_unsupported`); วางสำเร็จแจ้งจำนวนบรรทัดและเตือนให้ตรวจก่อนบันทึก
  5. **ตัวเลือกสมุดรายวัน** โหลดจาก master `journal-books` (เฉพาะสมุดที่เปิดใช้งานและมีประเภท — `activeJournalBooks`), แสดง "รหัส · ชื่อ" ตามภาษา; **ตัดสินใจจาก `booktype` เท่านั้น** (ค่า VAT เริ่มต้นขายเมื่อ `booktype` 4, ใบยอดยกมาเลือกประเภท 6) ห้ามเทียบรหัส `JV/SV/UV`; ใบที่บันทึกแล้ว **ล็อกช่องสมุด** (backend `journal_book_immutable`); สมุดที่ปิดใช้งานแต่ใบเดิมยังอ้างอยู่แสดงต่อท้ายพร้อม `gl_book_inactive_suffix`
  6. **จอกำหนดสมุดรายวัน**: ช่องประเภทสมุด (บังคับ, 1 ทั่วไป 2 จ่าย 3 รับ 4 ขาย 5 ซื้อ 6 ยอดยกมา) + ชื่ออังกฤษ (ไม่บังคับ) + เปิด/ปิดใช้งาน; ป้ายประเภทในรายการใช้สีจากตัวแปรธีมเท่านั้น และสมุดเก่าที่ไม่มีประเภทมีป้าย `gl_book_type_missing_badge`; คำอธิบายใต้ฟอร์มบอกว่าสมุดที่มีใบสำคัญลบ/เปลี่ยนรหัส-ประเภทไม่ได้ ให้ปิดใช้งานแทน (`gl_journal_book_in_use_hint`)
  7. **error จาก backend ชี้ช่องด้วย `field`** (`code`, `name`, `nameen`, `booktype`, `bookcode`, `branchcode`, `lines[i].description` …) — โฟกัส/ไฮไลต์ช่องนั้น ไม่โชว์ข้อความรวมลอย ๆ
- **กับดัก/ห้ามทำซ้ำ (Anti-pattern)**: `onBlur={() => onChange(text.trim() || "")}` ที่ทำให้ `"0"` กลายเป็น `""`; `toFixed(2)`/`Math.round` กับยอดเงิน; `maxLength={13}` บนช่องเลขภาษีที่ผู้ใช้วางแบบมีขีด (ตัดหลักท้ายทิ้งเงียบ ๆ); parser ที่ `Math.abs()` ยอดติดลบหรือทิ้งแถวที่ช่องเครดิตว่าง; รายการสมุด hard-code `["JV","PV","RV","SV","UV"]` หรือ `bookcode === "SV"` เป็นเงื่อนไข; regex path/code `[\p{L}\p{N}]` ที่ไม่มี `\p{M}` (ดู `docs/kms/17-dev-gotchas.md`)
- **เหตุผล (Root Cause & Rationale)**: นักบัญชี 40+ คีย์ด้วย Tab/Numpad ต่อเนื่อง — ช่อง 0.00 ที่หายเป็นว่างทำให้ทั้งใบบันทึกไม่ได้โดยไม่รู้สาเหตุ; การปัด/แปลงเครื่องหมายเองทำให้ยอดในบัญชีไม่ตรงเอกสาร (ต้องให้คนตัดสิน); สมุดรายวันเป็น master ที่ผู้ใช้ตั้งเอง (ADR `docs/kms/decisions/2026-09-24-user-defined-journal-books.md`) รหัสจึงไม่มีความหมายตายตัว
- **วิธีตรวจ**: `npx vitest run src/app/gl/amount-input.test.ts src/lib/clipboard-journal-parser.test.ts src/lib/gl-journal-details.test.ts src/lib/general-ledger.test.ts src/app/gl/gl-masters.test.ts src/app/gl/gl-journals.test.ts`; จอจริง: ปุ่ม Demo → `/gl/journal-books` สร้างสมุด `สมุดซื้อ` ประเภท 5 → `/gl/journals` เลือกสมุดนั้น, Tab ผ่านช่องยอด 0.00, วาง TSV จาก Excel ที่มีแถวเดบิตอย่างเดียวและแถว `(1,500.00)` แล้วดูข้อความ
- **อ้างอิง (Reference Implementation)**: `frontend/src/app/gl/gl-common.tsx` (`normalizeAmountText` :254, `amountInputIssue` :274, `amountBlurValue` :289, `AmountInput` :312); `frontend/src/lib/general-ledger.ts` (`validateJournalBook` :85, `defaultJournalBookCode` :107, `normalizeJournalLines` :292); `frontend/src/lib/clipboard-journal-parser.ts` (`parseClipboardJournalLines` :99); `frontend/src/lib/gl-journal-details.ts` (`taxDigits` :53, `normalizeBranchNo` :57, `normalizeJournalDetails` :79, `journalDetailsProblem` :91); `frontend/src/app/gl/gl-journals.tsx` (ตัวเลือกสมุด :812-822, วางจาก Excel :836, :889); `frontend/src/app/gl/gl-masters.tsx` (`JournalBookTypeBadge` :1158, ช่องประเภทสมุด :1186)

## 8.38 แผง "ไฟล์ยื่นกรมสรรพากร (.txt)" ในจอแบบยื่น — สร้างจากฉบับที่บันทึกแล้ว, จุดที่ต้องแก้กดแล้วพาไปช่อง (2026-09-24)

- **แบบแผนใหม่ (New Standard Pattern)**:
  1. **ปุ่มอยู่ในแถบปุ่มของจอแบบยื่นเดิม ไม่เพิ่มเมนู** และแสดงเฉพาะแบบที่ backend ติดธง `meta.rdfile` (ห้ามฝังรายการแบบไว้ที่ frontend); ป้ายไทยเต็ม "ไฟล์ยื่นกรมสรรพากร (.txt)" + ไอคอน, สูง `h-11`
  2. **ปุ่มปิดเมื่อยังไม่บันทึกหรือมีค่าที่แก้ค้าง** (`ready`) — เหตุผลอยู่ใน `title` + ข้อความ `sr-only` ที่ผูก `aria-describedby` และในแผงมี `role="status"` "บันทึกฉบับนี้ก่อน ไฟล์สร้างจากฉบับที่บันทึกแล้ว" (ไฟล์ต้องตรงกับฉบับที่ผู้ใช้ตรวจ/พิมพ์ PDF)
  3. **แผงตั้งค่าเฉพาะค่าที่แบบไม่มี**: ชื่อแผนก (สาขา 00000 เติม "สำนักงานใหญ่" ให้), ครั้งที่ส่ง (ค่าเริ่มต้น 00, `tabular-nums`), LTO และประเภทสาขาเป็น `ChoiceSelect` มีตัวเลือก "ไม่ระบุ" + คำอธิบายใต้ช่อง "ยื่นด้วยสื่อไม่บังคับ"; บอกในแผงว่าไฟล์ใช้กับโปรแกรม SWC-UI และ **อัปโหลดเข้า e-Filing ตรง ๆ ไม่ได้** (`tax_rdfile_help`)
  4. **สร้างไม่ผ่าน → กล่อง `role="alert"` รายการจุดที่ต้องแก้** หัวข้อ "สร้างไฟล์ไม่ได้ — พบ {n} จุดที่ต้องแก้", แสดงไม่เกิน 50 รายการ + "และอีก {n} รายการ" (`RDFILE_ISSUE_ROWS`), แต่ละรายการ = ข้อความจาก key + "แถวที่ n:" + ชื่อช่องไทย; **กดได้เฉพาะรายการที่พาไปช่องได้จริง** (ช่องในแผง → โฟกัสช่องในแผง, ช่องบนแบบ/ใบแนบ → `onIssue` ให้จอหลักไฮไลต์และเลื่อนไป) — ช่องที่แบบไม่มีคอลัมน์แสดงเป็นข้อความ ไม่ใช่ปุ่มที่กดแล้วไม่เกิดอะไร; ช่องในแผงที่ผิดขอบแดง + `ring-destructive/25`
  5. **ผลลัพธ์ผูกกับฉบับ `id:version`** — บันทึกรุ่นใหม่/เปิดฉบับอื่นแล้วผลเดิม (ชื่อไฟล์/รายการผิด) ซ่อนเอง; สำเร็จแสดง "สร้างไฟล์แล้ว: {filename}" (`role="status"`) และดาวน์โหลดด้วย blob ที่ BFF ส่งผ่านทุก byte (BOM + CRLF ต้องไม่ถูกแปลง)
- **กับดัก/ห้ามทำซ้ำ (Anti-pattern)**: สร้างไฟล์จากค่าบนจอที่ยังไม่บันทึก (ไฟล์ไม่ตรงกับ PDF/ฉบับที่ตรวจ); แทนที่อักขระต้องห้ามหรือปัดทศนิยมให้เงียบ ๆ ฝั่ง frontend; โชว์รายการปัญหาเป็นข้อความรวมก้อนเดียวหรือโชว์รหัสช่องดิบ (`l1_date`); ทำทุกรายการเป็นปุ่มแม้พาไปไหนไม่ได้; ใช้ `String.replace(template, value)` ตรง ๆ กับค่าที่มี `$&` (ใช้ callback `fill`); เขียนว่า "อัปโหลดเข้า e-Filing ได้"
- **เหตุผล (Root Cause & Rationale)**: ผู้ใช้ 40+ ต้องรู้ทันทีว่าผิดที่ไหนและแก้อย่างไร — ไฟล์ Format กลางมีหลายสิบช่องต่อบรรทัด ถ้าบอกแค่ "ไฟล์ไม่ถูกต้อง" จะหาไม่เจอ; ไฟล์ต้องมาจากฉบับที่บันทึก เพราะตัวเลขต้องตรงกับกระดาษที่ยื่น (backend อ่าน `tax_filings` ด้วย id+version); e-Filing ไม่รับ .txt (FAQ กรมสรรพากร) ถ้าไม่บอกผู้ใช้จะอัปโหลดผิดช่องทาง
- **วิธีตรวจ**: `npx vitest run src/app/tax/tax-rdfile-panel.test.ts src/app/tax/tax-form-language-keys.test.ts`; จอจริง: ปุ่ม Demo → rungrueng/01/00000 → แบบยื่น ภ.ง.ด.53 → ดึงยอด → บันทึก → ปุ่มไฟล์ยื่นเปิดได้ → ลบเลขอ้างอิงการลงทะเบียน → บันทึก → สร้างไฟล์ → รายการปัญหาไทย กดแล้วไปช่องนั้น → ใส่กลับ → ไฟล์ดาวน์โหลด (ตรวจ BOM/CRLF ด้วย node); ตรวจ light+dark
- **อ้างอิง (Reference Implementation)**: `frontend/src/app/tax/tax-rdfile-panel.tsx` (`TaxRdFileButton` :66, `TaxRdFileIssues` :102, `TaxRdFilePanel` :192, `panelField`/`PANEL_ISSUE_FIELDS` :30); `frontend/src/lib/tax-forms.ts` (`isHeadOfficeBranch`, `RD_HEAD_OFFICE_DEPT_NAME`); backend `backend/internal/goapi/handlers/tax_rdfile.go`; mapping ช่องไฟล์ `docs/kms/21-thai-tax-form-references.md` §11

## 8.39 `AmountInput` — คำเตือนใต้ช่องผูกกับค่าที่ทำให้เกิดเท่านั้น (2026-09-24, ต่อจาก 8.37)

- **แบบแผนใหม่ (New Standard Pattern)**: ปัญหาของช่องยอดเงินแสดงใต้ช่องเป็น `<span role="alert">` (`text-[0.9rem]`, สี destructive, `[overflow-wrap:anywhere]`) + `aria-invalid` + `aria-describedby` ผูกกัน; ลำดับความสำคัญ `amountFieldIssue`: ยอดติดลบ → ทศนิยมเกิน → "ตัดตัวอักษรที่ไม่ใช่ตัวเลขออกแล้ว"; คำเตือนตัดตัวอักษรจำค่าที่ช่องส่งออกไปตอนตัด (`droppedFor`) และแสดงเฉพาะตอนโฟกัสอยู่หรือค่าปัจจุบันยังเป็นค่านั้น (`droppedWarningShown`) — พิมพ์ต่อแล้ว commit ค่าใหม่ให้คำเตือนตามไป ค่าเปลี่ยนจากภายนอก (วางจาก Excel, โหลดใบใหม่) คำเตือนเก่าหาย
- **กับดัก/ห้ามทำซ้ำ (Anti-pattern)**: เก็บคำเตือนเป็น boolean ลอย ๆ (`dropped: true`) — ค่าเปลี่ยนจากที่อื่นแล้วคำเตือนเก่ายังค้างใต้ช่องที่ถูกต้องแล้ว (review 2026-09-24); แสดงคำเตือนด้วยสีขอบอย่างเดียว (ผิดกฎ 40+ ข้อ 5); ใช้ `title` แทนข้อความใต้ช่อง
- **เหตุผล (Root Cause & Rationale)**: คำเตือนที่ไม่ตรงกับตัวเลขที่เห็นทำให้ผู้ใช้สงสัยว่ายอดผิดทั้งที่ถูก และเลิกเชื่อคำเตือน; screen reader ต้องอ่านเหตุผลเมื่อโฟกัสช่อง
- **วิธีตรวจ**: `npx vitest run src/app/gl/amount-input.test.ts`; จอจริง: `/gl/journals` พิมพ์ `1,5a00` → คำเตือนตัดตัวอักษร → Tab → ค่า 1500 คำเตือนยังอยู่ → วางจาก Excel ทับ → คำเตือนหาย
- **อ้างอิง (Reference Implementation)**: `frontend/src/app/gl/gl-common.tsx` (`amountInputIssue` :278, `droppedWarningShown` :287, `amountFieldIssue` :292, `AmountInput` :326, ข้อความ :428-432, `role="alert"` :463)

## 8.40 หนังสือรับรอง 50 ทวิ ของรายการที่บันทึกแล้ว — ยอดอ่านอย่างเดียว, บันทึกก่อนพิมพ์, dirty guard ทั้งจอ (2026-09-24)

- **แบบแผนใหม่ (New Standard Pattern)**:
  1. เปิดจากรายการภาษีหักที่บันทึกในใบสำคัญ (`recordRef` = journalid + withholdingid) → **แบบยื่น ประเภทเงินได้ วันที่จ่าย เงื่อนไข และยอด เป็น `readOnly`** (`figuresLocked`), เลขที่หนังสือรับรองล็อกเมื่อบันทึกไว้แล้ว (`runNoLocked`); ส่ง PDF โดยอ้างรายการ ให้ backend พิมพ์ตามค่าที่บันทึก
  2. ข้อมูลผู้จ่าย/ผู้รับเงินแก้ได้ แล้วกด "บันทึกลงใบสำคัญ" (ร่าง = update, ผ่านบัญชีแล้ว = reconcile พร้อม dialog ยืนยัน); **ถ้ามีค่าที่แก้ค้างแล้วกดสร้าง PDF → ไม่สร้าง** และบอก "กรุณากด “บันทึกลงใบสำคัญ” ก่อน แล้วจึงกดสร้าง PDF" (`wht_cert_ui_save_before_pdf`)
  3. **Dirty guard**: แผงรายงาน `onDirtyChange` ให้จอรายงาน; เปลี่ยนแท็บ/เลือกแถวอื่น/ออกจากจอ → `leaveWhtCertificate` เปิด confirm ไทย "ละทิ้งข้อมูลที่ยังไม่บันทึก?" พร้อมบอกทางเลือก; ปิดแท็บเบราว์เซอร์ → `beforeunload`; แผงถูกถอดออก → รายงาน `false`
  4. สลับภาษาในแอปไม่ล้างค่าที่แก้ค้าง; ชื่อประเภทเงินได้ตามภาษาในแอป (ไม่ใช่ภาษาเบราว์เซอร์)
- **กับดัก/ห้ามทำซ้ำ (Anti-pattern)**: ให้แก้ยอด/วันที่จ่ายบน 50 ทวิ ของรายการที่บันทึก (หนังสือรับรองไม่ตรงกับ ภ.ง.ด. ที่ยื่น); สร้าง PDF จากค่าบนจอที่ยังไม่บันทึกขณะอ้างรายการ (backend พิมพ์ค่าที่บันทึก ผู้ใช้จะงงว่าที่แก้หายไปไหน); เก็บที่อยู่ผู้หักใน localStorage; ปิดแผงเงียบ ๆ ทิ้งค่าที่แก้
- **เหตุผล (Root Cause & Rationale)**: 50 ทวิ ต้องตรงกับรายการที่ยื่นใน ภ.ง.ด.3/53 ทุกตัวเลข — แหล่งความจริงคือรายการในใบสำคัญ; กฎ 40+ ข้อ 7 ห้ามทิ้งข้อมูลที่ผู้ใช้กรอกโดยไม่เตือน
- **วิธีตรวจ**: `npx vitest run src/app/tax/wht-certificate-panel.test.ts`; จอจริง: Demo → รายงานภาษีหัก ณ ที่จ่าย → แถวที่บันทึก → 50 ทวิ → ยอด/วันที่เป็นอ่านอย่างเดียว → แก้ที่อยู่ผู้รับ → กดสร้าง PDF → ข้อความให้บันทึกก่อน → เลือกแถวอื่น → dialog ยืนยัน → บันทึกลงใบสำคัญ → PG `payload->details->withholdings` เปลี่ยนในแถวเดิม → PDF ตรงค่าที่บันทึก
- **อ้างอิง (Reference Implementation)**: `frontend/src/app/tax/wht-certificate-panel.tsx` (`leaveWhtCertificate` :177, `figuresLocked` :224, `runNoLocked` :230, `generate` :263, dirty guard :304-314); `frontend/src/app/tax/tax-filing-workbench.tsx` (`leaveWhtEdits` :90)

## 8.41 แถบเตือนสมุดรายวันที่ยังไม่กำหนดประเภท — รายชื่อครบทุกเล่ม กดเปิดได้ทันที (2026-09-24)

- **แบบแผนใหม่ (New Standard Pattern)**: หน้า "กำหนดสมุดรายวัน" แสดงแถบ `role="status"` (ขอบ/พื้นจาก `destructive` ผ่าน opacity ของธีม, `text-[0.95rem] leading-relaxed`) เมื่อมีสมุดที่เปิดใช้งานแต่ไม่มีประเภท: หัวข้อ "สมุดรายวัน {0} เล่มยังไม่ได้กำหนดประเภท — เลือกประเภทก่อนบันทึกเอกสาร" + วิธีทำ + **ปุ่ม "รหัส · ชื่อ" ทุกเล่ม** (ไม่ใช่เฉพาะหน้าที่เปิดอยู่, `min-h-11`, ตัดบรรทัดได้) กดแล้วเปิดสมุดนั้น; ช่องประเภทในฟอร์มของสมุดเก่ามีคำอธิบาย `gl_book_type_untyped_hint`; เปลี่ยนชื่อหรือปิดใช้งานได้โดยไม่ต้องเลือกประเภท (backend `allowUntyped`); ผู้ใช้เลือกประเภท **ตามชื่อที่ตนตั้ง** — จอและ backend ไม่เดาจากรหัส
- **กับดัก/ห้ามทำซ้ำ (Anti-pattern)**: ให้ผู้ใช้ไล่เปิดสมุดทีละเล่มเพื่อหาเล่มที่ไม่มีประเภท; บังคับเลือกประเภทก่อนปิดใช้งานสมุดที่เลิกใช้; เดาประเภทจากรหัส `SV`/`UV` (ข้อมูลเดิมความหมายสลับกัน); ใช้สีแดงอย่างเดียวโดยไม่มีข้อความ
- **เหตุผล (Root Cause & Rationale)**: tenant เดิมมีสมุดไม่มีประเภทและบันทึกเอกสารใหม่ไม่ได้จนกว่าจะกำหนด — ผู้ใช้ต้องเห็นว่าติดเพราะอะไรและกดแก้ได้ในคลิกเดียว (ADR `docs/kms/decisions/2026-09-24-user-defined-journal-books.md` ข้อ 6)
- **วิธีตรวจ**: `npx vitest run src/app/gl/gl-masters.test.ts src/lib/general-ledger.test.ts`; จอจริง: สร้างสมุดไม่มีประเภทผ่าน API → `/gl/journal-books` แถบแสดงรายชื่อ → กดชื่อ → เลือกประเภท → บันทึก → แถบหาย; เปลี่ยนชื่อ/ปิดใช้งานเล่มอื่นโดยไม่เลือกประเภท → บันทึกผ่าน; ตรวจ light+dark
- **อ้างอิง (Reference Implementation)**: `frontend/src/app/gl/gl-masters.tsx` (`UntypedJournalBooksNotice` :1162, ช่องประเภท :1209); `frontend/src/lib/general-ledger.ts` (`untypedJournalBooks` :98); backend `backend/internal/generalledger/journal_books.go` (`validateJournalBookMaster` :110, `guardJournalBookUpdate` :140)

## 8.42 ค่าที่ backend พิมพ์ตามรายการที่บันทึก ต้องอ่านอย่างเดียวบนจอ + ไม่เดาค่าเริ่มต้นที่มีผลทางภาษี + dialog สัญญาเฉพาะสิ่งที่ backend ทำได้ (2026-09-24, ต่อจาก 8.40)

- **แบบแผนใหม่ (New Standard Pattern)**:
  1. ค่าที่ backend ใช้จากรายการที่บันทึกเสมอ (เช่น วันที่ออก 50 ทวิ `certificate_date`) → ช่องบนจอ `readOnly` แสดงค่านั้น + ข้อความใต้ช่อง `text-[0.9rem] text-muted-foreground` บอกว่าแก้ที่ไหน (`wht_cert_ui_issue_date_recorded`); helper บริสุทธิ์ `whtCertificateIssueDate` ทดสอบได้
  2. ไม่รู้ค่าที่มีผลทางภาษี (แบบยื่นของแถวฐานประมาณ) → `<select>` เริ่มว่าง + `<option value="" disabled>` "— เลือกแบบยื่น … —" (`wht_cert_ui_form_choose`) ให้ผู้ใช้เลือก; backend ตอบ `wht_cert_form_invalid` ช่อง `form` ถ้าส่งว่าง
  3. ข้อความ "ระบุ" ที่คู่กับประเภท ต้องส่งประเภทไปด้วยเสมอ (backend จับคู่ด้วยประเภท) และเติมค่าที่บันทึก (`income_description`) ให้เห็นก่อน
  4. 403 ที่มี `code` จาก backend (`wht_cert_scope_denied`, `user_access_expired`) แปลเป็นข้อความทางแก้ — ไม่กลืนเป็น "ลองใหม่"
  5. จอที่แก้ค่าได้นอกโมดูล GL (จอแบบยื่นภาษี) ใช้ `useDirtyGuard(route, dirty)` จาก `gl-common` และเมนูหลักรับ route นั้นผ่าน `guardsUnsavedRoute` → ปิดแท็บ/เปลี่ยนบริษัท/ออกจากระบบ ถามก่อน
  6. Dialog ยืนยันต้องสัญญาเฉพาะสิ่งที่ backend ทำจริง: เปิด "นำบัญชีเดิมเข้ากลุ่ม" เฉพาะ errorcode `LOGIN_EXISTS` (ไม่ใช่ `DUPLICATE`), ข้อความบอกว่าอีเมลเปลี่ยนได้เฉพาะบัญชีที่ยังไม่มีใครเข้าสู่ระบบ
  7. ไอคอนเตือนในตารางใช้ `text-primary` (8.27) ไม่ใช่ `text-amber-*`
- **กับดัก/ห้ามทำซ้ำ (Anti-pattern)**: ช่องแก้ได้แต่ค่าถูกเมินเงียบ ๆ ตอนพิมพ์; ตั้งค่าเริ่มต้นแบบยื่นเป็น ภ.ง.ด.53 เมื่อไม่รู้ (ผู้รับบุคคลธรรมดาได้ใบติ๊กผิด); ส่งประเภทว่างแต่ส่งข้อความ "ระบุ"; เปิด dialog จากการเทียบข้อความแปล; ปิดแท็บเมนูหลักแล้วค่าที่แก้หาย
- **เหตุผล (Root Cause & Rationale)**: ผู้ใช้ 40+ เชื่อสิ่งที่เห็นบนจอ — ถ้าจอกับเอกสารที่พิมพ์ไม่ตรงกันจะไม่รู้ตัวจนยื่นผิด; กฎ 40+ ข้อ 7 ห้ามทิ้งข้อมูลโดยไม่เตือน
- **วิธีตรวจ**: `npx vitest run src/app/tax/wht-certificate-panel.test.ts src/lib/thai-tax.test.ts src/app/menu/main-menu-general-ledger.test.ts src/components/system-settings/user-facing-error.test.ts`
- **อ้างอิง (Reference Implementation)**: `frontend/src/app/tax/wht-certificate-panel.tsx` (`whtCertificateFigures` :168, `whtCertificateIssueDate` :182, 403 :308, placeholder :452, hint :475); `frontend/src/app/tax/tax-filing-workbench.tsx` (`useDirtyGuard` :93); `frontend/src/app/menu/main-menu-screen.tsx` (`guardsUnsavedRoute` :407); `frontend/src/components/system-settings/user-facing-error.ts` (`offersExistingLogin` :140)

## 8.43 ช่องที่ผิดต้อง "เตือนใต้ช่อง + พาไปที่ช่อง" ไม่ใช่แค่แถบแจ้งด้านบน (2026-09-24, UAT S3)
- **แบบแผนใหม่ (New Standard Pattern)**:
  - ตรวจฝั่ง client ก่อนส่ง (เช่น เลขผู้เสียภาษี 13 หลัก + หลักตรวจสอบ `validThaiTaxId`) แล้วแสดง `<span role="alert" className="text-[0.9rem] leading-snug text-destructive">` **ใต้ช่องนั้นทันที** ด้วยข้อความไทยที่บอกวิธีแก้ (ตรวจกับหนังสือรับรอง/บัตร/ใบกำกับ แล้วพิมพ์ใหม่)
  - ทุกเซลล์ของตารางรายละเอียดมี `data-detail-field="<section>.<row>.<field>"` (row นับจาก 1)
  - เมื่อบันทึกแล้ว server ปฏิเสธพร้อม `field` → หาแถวที่ผิดเอง (`detailTaxIdTarget`) แล้ว `focusDetailField`: เปิด `<details>` ที่ครอบ, `scrollIntoView({block:"center"})` (ใช้ `behavior:"auto"` ใต้ `prefers-reduced-motion`), `focus({preventScroll:true})`; แถบแจ้งด้านบนยังบอกเหตุผลเป็นภาษาไทย
- **กับดัก/สิ่งที่ห้ามทำซ้ำ (Anti-pattern)**: `catch(e){ setError(e.message) }` อย่างเดียว — ผู้ใช้ต้องไล่หาเองว่าแถวไหนผิดในใบที่มีหลายแถว; เตือนตามความยาวอย่างเดียวโดยไม่ตรวจหลักตรวจสอบ (ผ่านหน้าจอแต่ server ปฏิเสธ); ใช้ `smooth` scroll โดยไม่เคารพ reduced-motion
- **เหตุผล (Root Cause & Rationale)**: backend บอกชื่อช่องแต่ไม่บอกแถว; คน 40+ ต้องเห็นจุดที่ผิดตรงหน้า (กฎ 40+ ข้อ 7–9: validate ก่อนส่ง, feedback ทันที, ไม่พาสายตากระโดด) — UAT 2026-09-24 เจอแถบแจ้งด้านบนอย่างเดียวแล้วผู้ทดสอบหาแถวไม่เจอ
- **วิธีตรวจ**: กรอก `0105561001238` (หลักสุดท้ายผิด) → เห็นเตือนใต้ช่องทันที; บันทึก → server ตอบไทย และโฟกัสไปที่ `partners.1.tax_id`; PG ต้องไม่มีแถวใหม่; `npx vitest run src/app/gl/gl-journal-details.test.ts`
- **อ้างอิง (Reference Implementation)**: `frontend/src/app/gl/gl-journals.tsx` (`focusDetailField` :71, ใช้ตอนบันทึก :295, :317); `frontend/src/lib/gl-journal-details.ts` (`validThaiTaxId` :68, `detailTaxIdTarget` :78); `frontend/src/app/gl/gl-journal-details.tsx` (เตือนใต้ช่อง :169, `data-detail-field` :187)

## 8.44 ช่องรหัสบัญชีห้ามเติมรหัสเดา — เว้นว่างให้ backend ถาม, เลขใบสำคัญที่ระบบสร้างมีช่องให้ระบุเอง (2026-09-24)
- **แบบแผนใหม่ (New Standard Pattern)**:
  - ช่องรหัสบัญชี GL ในฟอร์ม (สินทรัพย์, หน้าต่างจำหน่าย) เริ่มเป็น `""` และผูกค่าแบบ `value={form.x ?? ""}` — ค่าว่างคือ "ใช้ของประเภทสินทรัพย์ / ให้ระบบถาม" ไม่ใช่รหัสสำรอง
  - ทุกบัญชีที่ backend ต้องใช้ตามยอดจริง (เช่น ภาษีขายเมื่อมี VAT) ต้องมีช่องบนจอให้กรอก — backend ตอบ `fa_account_required` + `field` เป็นไทยเมื่อขาด
  - เลขใบสำคัญที่ระบบสร้างจากรหัสอื่น (รหัสสมุด + รหัสสินทรัพย์) ต้องมีช่อง "เลขที่ใบสำคัญ (เว้นว่าง = ระบบสร้างให้)" + `maxLength={30}` (ไทยทุกตัวเป็น 1 UTF-16 unit = 1 rune ตรงกับ `doc_no VARCHAR(30)`) เพื่อให้ผู้ใช้แก้เองได้เมื่อเลขที่สร้างยาวเกิน
  - ข้อความ/ค่าที่แสดงห้ามอ้างรหัสสมุด "JV" — สมุดรายวันเป็นข้อมูลที่ผู้ใช้กำหนดเอง ใช้คำว่า "สมุดรายวันประเภททั่วไปที่เปิดใช้งาน" และแสดงเลขจริงจาก response (`String(res?.journal?.docno ?? "")`)
- **กับดัก/สิ่งที่ห้ามทำซ้ำ (Anti-pattern)**: `value={x || "120101"}` (ล้างช่องไม่ได้ + ส่งรหัสเดาไปบันทึก), เติมรหัสบัญชีในค่าเริ่มต้นของฟอร์ม/ปุ่มเพิ่มใหม่, `placeholder="110101"` (ผู้ใช้ 40+ เข้าใจว่าเป็นค่าที่ระบบแนะนำ), fallback `|| "JV"` ตอนแสดงเลขใบสำคัญ
- **เหตุผล (Root Cause & Rationale)**: ผังบัญชีแต่ละบริษัทต่างกัน รหัสเดาจะลงผิดบัญชีเงียบ ๆ หรือถูก GL ปฏิเสธด้วยข้อความที่ผู้ใช้แก้ไม่ได้ (กฎ "ห้ามใช้รหัสบัญชีเป็นเงื่อนไข"); กฎ 40+ ข้อ 7 — กันพลาดก่อนส่ง
- **วิธีตรวจ**: เปิดฟอร์มสินทรัพย์ใหม่/หน้าต่างจำหน่าย ช่องบัญชีต้องว่างและลบได้; จำหน่ายที่มี VAT โดยไม่กรอกบัญชีภาษีขาย → ข้อความไทยระบุช่อง; `go test ./internal/fixedasset/` (guard AST ห้าม string ตัวเลข ≥ 4 หลัก/`"JV…"`)
- **อ้างอิง (Reference Implementation)**: `frontend/src/app/asset/fixed-assets-screen.tsx` (ค่าเริ่มต้นว่าง :68, `?? ""` :669, ช่องภาษีขาย :775, ช่องเลขที่ใบสำคัญ :786–790); `backend/internal/fixedasset/gl_poster.go` (`resolveAssetAccounts` :97, `assetAccountMissing` :116, `checkJournalDocNo` :151); bug `docs/kms/bugs/2026-09-24-fixed-asset-gl-guessed-accounts.md`

## 8.45 ฟอร์มที่อยู่หลายช่อง: ไม่ตัดข้อความเงียบ ๆ, เตือนใต้ช่อง, ใช้รายชื่อจังหวัด/อำเภอ/ตำบลเป็นตัวช่วย (2026-09-25, ที่อยู่สำหรับภาษีของบริษัท)
- **แบบแผนใหม่ (New Standard Pattern)**:
  - ช่องที่อยู่มีโครงตรงกับ key หัวแบบกรมสรรพากร (`addr_building` … `addr_postcode`, `TAX_ADDRESS_KEYS`) จัดเป็นการ์ดเดียว `rounded-2xl border border-border bg-background p-3 shadow-sm` + `h3` หัวข้อ + คำอธิบายว่าเอาไปใช้ที่ไหน (เติมหัวแบบ ภ.พ.30/ภ.ง.ด./50 ทวิ ยังแก้ได้ในแต่ละแบบ) + คำแนะนำ "ไม่ต้องพิมพ์คำนำหน้า"
  - ตาราง grid `sm:grid-cols-2 lg:grid-cols-3` (iPad ขึ้นไปวางแนวนอน), ทุกช่องมี `<label htmlFor>` ไทยจาก `tr(key, fallback)` — ป้ายที่มีแถวอยู่แล้วให้ใช้ซ้ำ (`ss_subdistrict`, `ss_district`, `address_province`, `address_zipcode`, `company_telephone`)
  - ตำบล/อำเภอ/จังหวัดใช้ `<datalist>` จาก `loadThailandAddressData` (ช่วยเลือก แต่พิมพ์เองได้ ถ้าโหลดไม่ได้ก็ยังกรอกได้); เลือกตำบลแล้วรหัสไปรษณีย์ยังว่าง → เติมให้ (แก้ได้)
  - ตรวจฝั่ง client ด้วยกฎชุดเดียวกับ backend (`taxAddressProblems` — ยาวเกิน, ขึ้นบรรทัด/แท็บ, รหัสไปรษณีย์ 5 หลัก) แล้วแสดง `<span role="alert" className="text-[0.9rem] leading-snug text-destructive">` ใต้ช่อง + `aria-invalid`/`aria-describedby`; ข้อความใช้ key เดียวกับที่ backend ตอบ (`company_tax_addr_err_*`, placeholder `{field}`/`{max}`) และข้อความของช่องแรกที่ผิดเข้าไปในแถบแจ้ง/ปุ่มบันทึกที่ปิดอยู่ (`requiredFormError`)
  - ส่งที่อยู่ครบทุก key เสมอทั้งสร้างและแก้ไข (`trimTaxAddress`) และใส่ใน optimistic update ด้วย — backend ถือว่า PUT ที่ไม่ส่ง = คงค่าเดิม, ส่งมา = แทนทั้งชุด
  - โหลดค่าแบบกัน null (`toTaxAddress(data.address)`, `String(data.phone ?? "")`) — ทุกช่องเป็น controlled string เสมอ
- **กับดัก/สิ่งที่ห้ามทำซ้ำ (Anti-pattern)**: ใส่ `maxLength` ให้ช่องที่มีเพดานตาม backend (browser ตัดข้อความที่ paste มาเงียบ ๆ ผู้ใช้ไม่รู้ว่าที่อยู่ขาด); เตือนด้วยแถบด้านบนอย่างเดียว; ช่องที่อยู่ช่องเดียวแบบข้อความยาว (เติมหัวแบบแยกช่องไม่ได้); ให้ผู้ใช้พิมพ์ "ถนน/ตำบล/จังหวัด" เองแล้วระบบเติมซ้ำเป็น "ถนนถนน…"; hard-code สีเตือน (`text-red-*`, `amber-*`) แทน `text-destructive`
