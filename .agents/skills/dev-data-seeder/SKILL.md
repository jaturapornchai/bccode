...
- Each seeded group must be a complete category tree. Create root usage/category nodes first through `POST /product/category`, capture the returned GUID, then create child/subchild nodes with `parent_guid` and `parentguidall`.
