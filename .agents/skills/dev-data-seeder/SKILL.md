...
- Resolve and validate the active `holdingcode + businesscode` first. Match the browser-selected company to backend workspace context and never seed into a guessed or Holding-only target.
...
- Every Barcode must be created with required same-company `itemcode`; use the quick-create API only when it atomically creates/links the minimal Product. Never seed an orphan Barcode or Barcode stock/balance fields.
...
- Verify `/product/category/list?group-number=<n>` returns multiple hierarchical nodes for a populated group, and verify at least one leaf category contains Product-master-only `codelist` entries shaped `{code,xorder,names}`. Every `code` must resolve to an active Mongo `products` record in the same `(holdingcode,businesscode)`; never seed Barcode/unit fields as category membership.
...
- `scripts/seed-restaurant-menu-dev.ps1`
  - Before use, verify/update it to accept the selected `businesscode`; company-owned product seeding must never run with Holding context alone.
  - Creates real DEV restaurant menu products, linked barcodes, and Product Set records for the selected `(holdingcode,businesscode)`.
