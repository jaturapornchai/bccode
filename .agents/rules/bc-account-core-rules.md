...
- Add copy button rule: every editable CRUD/list screen should expose `เพิ่ม (Copy)` beside `เพิ่ม` when a selected row can be duplicated. The Copy button must use the same primary color treatment as `เพิ่ม`, start create mode (`POST`, not edit), and prefill the new form from the selected row as completely as possible for repeated SKU/product/settings structures.
