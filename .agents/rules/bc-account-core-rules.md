...
- Product variant template rule: repeated product option structures such as color > size, size > color, storage > color, flavor > pack, or package > network belong in reusable product option set templates (`product_variant_matrix`) with user-defined option order. Product add/edit UI should let users apply a template first and then adjust product-specific choices, prices, barcodes, media, and marketplace mappings.
