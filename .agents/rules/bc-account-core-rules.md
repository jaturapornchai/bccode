...
- Product category group semantics: `productCategories.group_number` (1-20) is a usage/device/channel group, not a flat product-menu category. Seed, UI, and backend behavior must treat each group as a complete tree: root nodes describe usage contexts such as ordering device, cashier/POS, kitchen/KDS, or delivery; children/subchildren use `parent_guid` and `parentguidall`; sellable leaves or explicitly sellable category nodes carry `codelist`. Do not create one flat category per menu type as the group structure.
- Holding access scope rule: users, screen permissions, permission groups, user permission assignments, and approval rights are Holding-owned records under `holding_code`. Their applicability must be configured with explicit scope entries such as `access_scopes[]`, `scope_rules[]`, or `approval_rules[]`, using `scope_type` = `holding | company | branch`.
  - `scope_type=holding`: applies to the whole Holding and must not require `business_code` or `branch_code`.
  - `scope_type=company`: requires uppercase `business_code`, applies to all branches in that company, and is equivalent to `all_branches=true`.
  - `scope_type=branch`: requires uppercase `business_code` plus zero-padded 5-digit `branch_code`, and applies only to that branch.
  - Backend validation must confirm the company and branch exist in the same `holding_code`, reject a branch scope without `business_code`, normalize business/branch codes before duplicate checks and save, and preserve `holding_code` lowercase.
  - Effective access evaluation receives `holding_code`, `business_code`, `branch_code`, and `user_uid`; match specificity is `branch > company > holding`. For boolean screen access, the most specific matching rule overrides broader rules; within the same specificity, union explicit grants. For approvals, the most specific matching approval rule wins unless the workflow explicitly defines a stricter rule.
  - UI must use one shared Holding scope widget for all access screens: a whole-Holding checkbox, searchable company add-to-list, selected-company branch panel, branch search add-to-list, and per-company `all_branches` checkbox. The widget serializes to `scope_type`, `business_code`, `branch_code`, and `all_branches`; it must not render full company/branch checkbox trees or expose raw JSON to normal users.
  - Access screen ownership map:
    - `users`: Holding user profile and login/account status; `access_scopes[]` controls which companies/branches the user can enter.
    - `permission_definitions`: Holding screen/action dictionary; `scope_rules[]` controls where each screen/action applies.
    - `permission_groups`: Holding permission bundles; `scope_rules[]` controls where the group applies.
    - `employee_permissions` or `user_permissions`: Holding user-to-group/permission assignments; `scope_rules[]` controls where the assignment applies.
    - `approval_permissions` or `approval_rules`: Holding approval dictionary and limits; `approval_rules[]` controls document/company/branch applicability.
- Company-level master-data access selector rule: general master-data fields such as `business_codes` are company-level only. Do not show, edit, or save branch selections inside these normal company access editors. Branch-specific permissions belong only to the Holding access scope model above, or to explicit branch/legal/calendar/operation screens. Read legacy `company_guids` only as a compatibility alias.
...
- Every persisted business record must get an immutable GUID at first creation, such as `guid_fixed` / `guid`, and that GUID must never change for the lifetime of the record.
...
- Business reference fields between records must store the relevant user/business code, not the target record GUID, when the domain expects code-based references. Example: product or barcode unit references must store `unit_code` (รหัสหน่วยนับ), not the product unit `guid_fixed`.
...
- Business/reference/display code values must be normalized to uppercase before validation, duplicate checks, search, save, and downstream sync. This applies to values in `code`, `<thing>_code`, `business_code`, `business_codes[]`, company code, product code, unit code, document code, SKU, and equivalent business codes. It does not apply to persisted/API field names (still lower `snake_case`), `holding_code` (lowercase tenant key), `lang_code`, email/user login text, or GUID values.
...
- Legacy `shopUsers` collection/field names may remain only as compatibility paths until an explicit migration. New and changed access contracts must be Holding-centric (`holding_code`, `holding_users`, `user_uid`, `access_scopes[]`) while preserving dual-read/dual-write where old runtime data still exists. `shopUsers.user_uid` references `users.uid` and remains the primary join key for rename-safe membership lookup during transition, while `shopUsers.username` remains visible/searchable transition data.
...
- Branch codes are scoped by company: uniqueness and duplicate checks must use `holding_code + company_guid + code`, not `holding_code + code` or `code` alone. Backend branch create/update must require `company_guid` and verify that the company exists in the same tenant before writing `organization_branches`.
