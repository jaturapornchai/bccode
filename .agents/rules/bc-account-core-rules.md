...
- External model baseline: for ERP model/schema/API contract decisions, read `D:\bccode-model\rules.md` and the relevant model files in `D:\bccode-model` first. New/changed operational records use `holding_code` for tenant scope, `guid_fixed` for CRUD identity, and business codes such as `business_code`, `business_codes`, and `unit_code` for cross-record references. Legacy fields such as `shopid`, `company_guids`, `unitcode`, and `settings.languageconfigs` may remain only as explicit compatibility aliases during transition.
...
- Company access selector rule: company access fields such as `business_codes` are company-level only. Do not show, edit, or save branch selections inside company access editors. Branch-specific settings belong only to explicit branch/legal/calendar/operation screens; normal CRUD/master-data access is inherited from the selected company/workspace context. Read legacy `company_guids` only as a compatibility alias.
...
- `shopUsers.shopid` is the current runtime tenant/workspace compatibility id and equals `shops.guid_fixed`; do not rename legacy runtime fields without an explicit migration. New model contracts should follow the `D:\bccode-model` baseline with `holding_code` and `holding_users`. `shopUsers.user_uid` references `users.uid` and is the primary join key for rename-safe membership lookup, while `shopUsers.username` remains visible/searchable transition data.
...
- Branch codes are scoped by company: uniqueness and duplicate checks must use `shopid + company_guid + code`, not `shopid + code` or `code` alone. Backend branch create/update must require `company_guid` and verify that the company exists in the same tenant before writing `organization_branches`.
...
- `D:\bccode-model` is a local/external model-document workspace and is not part of the `D:\bccode` GitHub project. Do not stage, commit, push, or infer a GitHub remote for `D:\bccode-model` when pushing this project. Only handle `D:\bccode-model` Git separately if Jead explicitly provides a repository/remote for that folder.
