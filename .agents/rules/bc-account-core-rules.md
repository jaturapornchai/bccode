...
- Screen/menu label match rule: page headers, detail headers, empty states, aria labels, and toolbar context must match the active menu/route. Shared dictionaries can supply field labels, but screen-level labels must not leak across screens, for example `บาร์โค้ด` on `/product` or `สินค้า` on `/product_barcode`.
...
- Legacy `shopUsers` and underscore collection/field names may remain only as compatibility paths until an explicit migration or destructive reset is approved. New and changed access contracts must be Holding-centric (`holdingcode`, `holdingusers`, `useruid`, `accessscopes[]`) while preserving dual-read/dual-write only where old runtime data still exists and must be retained. `shopUsers.user_uid` is legacy compatibility; new membership lookup must use `useruid` under lowercase no-underscore contracts.
