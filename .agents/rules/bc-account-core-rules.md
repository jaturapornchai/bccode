...
- Product variant template rule: repeated product option structures such as color > size, size > color, storage > color, flavor > pack, or package > network belong in reusable product option set templates (`product_variant_matrix`) with user-defined option order. `product_variant_matrix` create/edit UI and product add/edit UI should expose a clear `แม่แบบ` action, support applying the whole option set, and support picking a SKU row from `sku_combinations` as a SKU-level template before product-specific choices, prices, barcodes, media, and marketplace mappings are adjusted.
