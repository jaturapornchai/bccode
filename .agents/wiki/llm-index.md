...
- Product option set template routing: inspect `D:\bccode\frontend\src\lib\system-setting-screens.ts` (`product_variant_matrix`), `D:\bccode\frontend\src\app\system-settings\system-settings-screen.tsx` (`ProductVariantMatrixTemplatePicker`), and `D:\bccode\frontend\src\app\menu\product-screen.tsx` (`ProductOptionsEditor`). Repeated option structures should be applied from `product_variant_matrix` before product-specific edits, preserving user-defined order such as color > size or size > color; `sku_combinations` should be exposed as selectable SKU-level templates.
