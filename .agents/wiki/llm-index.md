...
- Mobile/computer product routing: use `/productserialregistry` for `ทะเบียนเลขเครื่อง` and `/channelprice` for `ราคาตามช่องทางขาย`. These are structured Atlas-backed CRUD screens for mobile phone, SIM, computer, and serialized electronics workflows; do not replace them with raw JSON fields.
