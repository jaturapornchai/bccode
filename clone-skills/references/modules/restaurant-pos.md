...
name: restaurant-pos
description: >
  Restaurant POS system — tables, kitchen, printers, shifts, order flow, staff management.
  Use when the user mentions restaurant, POS, table, kitchen, shift, or printer.
user-invocable: true
---
...
## When to Use
- User asks about managing tables/zones in a restaurant (add, remove, edit zone/table)
- Need to debug or add logic in kitchen printer or notifier
- Questions about opening/closing shift and counting cash drawer balance
- Need to modify order flow from taking order to payment
- Problems with device pairing or POS not recognizing printer

## Anti-Patterns
- Never hardcode table status as string — use enum/constant defined in the model
- Don't forget async order sync — when kitchen confirms order, must notify all devices
- Never use kitchen printer and receipt printer as the same device — separate BLoC for each
- Never edit temp data (`pos/temp/`) directly — use API handler only
- Never mix restaurant module with general transaction module — they are separate codebases
...
## Related Skills
- [lineoa-chatbot](../lineoa-chatbot/SKILL.md) — LINE OA notifications for order alerts, kitchen alerts
- [dashboard-report](../dashboard-report/SKILL.md) — Daily sales report, shift summary, revenue analysis
- [mcp-check](../mcp-check/SKILL.md) — Verify MCP tools for sales/inventory data used in POS
