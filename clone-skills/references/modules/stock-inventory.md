...
description: >
  Inventory management: stock balances, warehouse setup, stock receive/issue/transfer/adjustment,
  and 7 costing methods (FIFO, LIFO, Weighted Average, etc.). Use when the user mentions stock
  levels, warehouse operations, inventory costing, or stock movement.
user-invocable: true
---
...
## When to Use
- User asks about current stock balance, stock level for a product, or warehouse balance
- User wants to receive goods into warehouse (Stock Receive) after a purchase order
- User needs to transfer stock between warehouses (inter-warehouse transfer)
- User wants to adjust stock balance after physical count discrepancy
- User asks which costing method the system uses or how to change it

## Anti-Patterns
- Do NOT adjust stock without providing a reason — system enforces required reason field
- Do NOT delete a warehouse that has active stock balance — orphan balance will occur
- Do NOT manually edit stock balances in DB directly — always use adjustment transactions
- Do NOT set costing method mid-period without running recalculation — costs will be wrong
- Do NOT assume inter-warehouse transfer completes instantly — sender (-) and receiver (+) must both be confirmed
...
## Related Skills
- `/product` — product data, barcodes, BOM, options, AI classification
- `/auto-packing` — pick & pack workflow using stock from warehouses
- `/master-data` — CRUD for warehouses, units, and product master data via MCP tools
- `/import-export` — import opening stock balances from Excel
