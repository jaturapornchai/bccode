...
name: procurement
description: >
  Procurement module — PR (Purchase Requisition), RFQ (Request for Quotation), PO (Purchase
  Order). Feature tracker, architecture patterns, budget check, vendor price auto-fill,
  approval workflow. Use when the user mentions procurement, PR/RFQ/PO, vendor quote, or
  budget check.
---
...
## When to Use
- User asks about or edits PR form, RFQ form, PO form (header/detail/extra fields)
- Need to debug budget check warning or vendor price auto-fill not working
- Need to add new field in PR/RFQ — must check ExtraFields pattern vs TransactionModel
- Questions about approval workflow via LINE OA or AI chatbot PR
- Need to update feature-matrix.md after adding/completing a feature

## Anti-Patterns
- Always register handler in both main.go files when adding new endpoint (GoAPI + MainAPI)
- Never delete fields in TransactionModel even if unused in PR — may be used in other modules
- Never fetch department data from `/organization/department/list` — use `global.companyBranchSelectData.departments` instead
- Never block save when budget is exceeded — only show warning banner for approver to decide
- Never add fields directly in TransactionModel for PR-specific data — use ExtraFields pattern

## References
- [Feature Matrix](references/feature-matrix.md) — full feature list, status, competitive analysis

## Related Skills
- [lineoa-chatbot](../lineoa-chatbot/SKILL.md) — PR approval via LINE OA, AI chatbot purchase ordering
- [dashboard-report](../dashboard-report/SKILL.md) — Procurement dashboard, PR→RFQ→PO tracking report
- [mcp-check](../mcp-check/SKILL.md) — Verify MCP tools for procurement data
