...
WarehouseModel
  ...
  |       |- code, names[]
  |       |- ShelfProduct[]
  |           |- barcode, itemcode, qty, minqty, maxqty, unitcode
```
...
```
1. Stock balance request received
...
6. UI shows suggestion to warehouse staff
```
...
Sale Invoice created
  -> Appears in "available-saleinvoice" list
  -> Staff creates Pick & Pack document (PENDING=0)
  -> Print packing list -> UpdatePrint() -> PROCESSING=1
  -> Pick items, scan barcodes
  -> Confirm -> ConfirmPickandpack() -> COMPLETED=2
...
```
BOM created/updated via HTTP
  -> Save to MongoDB (productBarcodeBOMs)
  -> Publish to Kafka
  -> BOM Consumer syncs to PostgreSQL
...
|--------------|---------|
| `pickandpack.created/updated/deleted` | Pick & Pack sync |
